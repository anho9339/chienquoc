...
// 函数：成品编号
int get_fin_type() { return 1819; }

// 函数：构造处理
void create()
{
        set_name( "Hạt Lạc Hoàn" );
        set_picid_1(9101);
        set_picid_2(9101);
        set_unit( "粒" );
        set_value(720);
...
string get_desc() 
{ 
        return "Một loại thuốc tương đối có hiệu quả sử dụng trong chiến đấu, lập tức hồi phục 400 điểm khí huyết, thời gian tái sử dụng 15 giây";
}
...
int get_use_effect_callout( object me )
{
	...
	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 9158, 1, OVER_BODY, PF_ONCE );
        return 1;
}
