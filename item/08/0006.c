...
string get_desc()
{
    return "Lệnh bài thần kỳ, sau khi sử dụng bạn sẽ nhận được điểm Kinh nghiệm x 2 khi tiêu diệt quái vật trong suốt "
...
int get_use_effect_callout(object who, object item)
{
    ...
    if (gone_time(who->get_login_time()) + who->get_game_time() < who->get_double_time())
    {
        send_user(who, "%c%c%w%s", ':', 3, 0,
                  sprintf("Ngươi vẫn còn thời gian nhân đôi kinh nghiệm, Muốn dùng đè lên không?\n" ESC
                          "Chấp nhận\nuse ! %x#\n" ESC "Huỷ bỏ",
                          getoid(item)));
        return 0;
    }
    ...
    if (who->get_save_2("double_time.freeze") > 0)
    {
        ...
        "/inh/user/mail"->sys_mail(who->get_id(), who->get_number(),
                                   "Bởi vì ngươi đã gửi thời gian nhân đôi tại Thần Tiên Gia Gia,nên sử dụng Chiến "
                                   "Quốc Lệnh thời gian cũng sẽ gửi lại Thần Tiên Gia Gia tại " HIR
                                   "Chu Quốc（291，175） hoặc Tân Thủ Thôn（91，164） " NOR "Ngươi phải nhớ " HIR
                                   "giải trừ thời gian đông cứng trước 23:59 kể từ khi đông cứng," NOR
                                   "，Nếu không thời gian đông cứng sẽ biến mất.");
