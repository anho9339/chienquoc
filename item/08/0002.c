...
string get_desc()
{
    return "Tù và thần kỳ chỉ có tác dụng khi Trưởng nhóm sử dụng.\n Tất cả thành viên trong nhóm đều được nhận thêm "
           "20 %  điểm Kinh nghiệm khi tiêu diệt quái vật trong suốt 1 tiếng.";
}
...
int get_use_effect_callout(object who, object item)
{
    ...
    if (time() < who->get_extra_exp_time())
    {
        send_user(who, "%c%c%w%s", ':', 3, 0,
                  sprintf("Ngươi vẫn còn thời gian sử dụng Tù Và Chiến Quốc, Muốn dùng đè lên không?\n" ESC
                          "Chấp nhận\nuse ! %x#\n" ESC "Huỷ bỏ",
                          getoid(item)));
        return 0;
    }
