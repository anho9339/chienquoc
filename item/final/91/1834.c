...
#define ITEM_USE_TIME	15 - me->get_skill(0436)
...
// 函数：成品编号
int get_fin_type() { return 1834; }

// 函数：构造处理
void create()
{
        set_name( "Thất Thải Linh Chi Hoàn" );
        set_picid_1(9923);
        set_picid_2(9923);
        set_value(1200);
        set("level", 0);
...
string get_desc() 
{ 
        return "Lập tức hồi phục 1200 điểm Pháp Lực, 1150 điểm Khí Huyết, tái sử dụng 15 giây";
}
...
int get_use_effect_callout( object me )
{
	...
	send_user( me, "%c%c%c%w", 0x30, get_item_type_2(), 0, ITEM_USE_TIME );    // 更新使用间隔
        me->add_hp( 1200 * ( 100 + me->get_healing_rate() ) / 100 );        
        me->add_mp( 1150 * ( 100 + me->get_healing_rate() ) / 100 );        
