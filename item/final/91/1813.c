...
// 函数：成品编号
int get_fin_type() { return 1813; }

// 函数：构造处理
void create()
{
        set_name( "Lục Thần Đơn" );
        set_picid_1(9165);
        set_picid_2(9165);
        set_value(1400);
        set("level", 80);
...
string get_desc() 
{ 
        return "Lập tức hồi phục 2500 điểm Pháp Lực, tái sử dụng 15 giây";
}
...
int get_use_effect_callout( object me )
{
	...
	send_user( me, "%c%c%c%w", 0x30, get_item_type_2(), 0, ITEM_USE_TIME );    // 更新使用间隔
        ...
        me->add_mp( 2500 * ( 100 + me->get_healing_rate() ) / 100 );        
