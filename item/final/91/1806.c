...
// 函数：成品编号
int get_fin_type() { return 1806; }

// 函数：构造处理
void create()
{
        set_name( "Long Tiên Thảo" );
        set_picid_1(9086);
        set_picid_2(9086);
        set_value(700);
        set("level", 60);
...
string get_desc() 
{ 
        return "Lập tức hồi phục 1800 điểm Pháp Lực, tai sử dụng 15 giây";
}
...
int get_use_effect_callout( object me )
{
	...
	send_user( me, "%c%c%c%w", 0x30, get_item_type_2(), 0, ITEM_USE_TIME );    // 更新使用间隔
        ...
        me->add_mp( 1800 * ( 100 + me->get_healing_rate() ) / 100 );        
