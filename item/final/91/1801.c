...
// 函数：成品编号
int get_fin_type() { return 1801; }

// 函数：构造处理
void create()
{
        set_name( "Thiên Tiên Ngọc Lộ" );
        set_picid_1(3154);
        set_picid_2(3154);
        set_value(200);
        set("level", 10);
...
string get_desc() 
{ 
        return "Sử dụng hồi phục 650 điểm Pháp Lực, tái sử dụng 15 giây";
}
...
int get_use_effect_callout( object me )
{
	...
	send_user( me, "%c%c%c%w", 0x30, get_item_type_2(), 0, ITEM_USE_TIME );    // 更新使用间隔
        me->add_hp( 0 * ( 100 + me->get_healing_rate() ) / 100 );        
        me->add_mp( 650 * ( 100 + me->get_healing_rate() ) / 100 );        
