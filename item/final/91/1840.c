...
// 函数：成品编号
int get_fin_type() { return 1840; }

// 函数：构造处理
void create()
{
        set_name( "Thiên Hương Tục Mệnh Tán" );
        set_picid_1(9086);
        set_picid_2(9086);
        set_value(2000);
        set("level", 0);
...
string get_desc() 
{ 
        return "Lập tức hồi phục 2200 điểm Khí Huyết và 2800 điểm Pháp Lực";
}
...
int get_use_effect_callout( object me )
{
	...
	send_user( me, "%c%c%c%w", 0x30, get_item_type_2(), 0, ITEM_USE_TIME );    // 更新使用间隔
        me->add_hp( 2200 * ( 100 + me->get_healing_rate() ) / 100 );        
        me->add_mp( 2800 * ( 100 + me->get_healing_rate() ) / 100 );        
