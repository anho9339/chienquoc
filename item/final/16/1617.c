...
#define ADD_DAMAGE      800

// 函数：叠加上限
int get_max_combined() { return 30; }

// 函数：成品编号
int get_fin_type() { return 1617; }
...
{ 
	...
	if (me->get_skill(660)<SKILL_LEVEL) color = HIR + sprintf("c %d\n", SKILL_LEVEL);
	else color = sprintf("Yêu cầu Cơ quan cấp bậc： %d\n", SKILL_LEVEL);
        return  color + "Sử dụng trong phạm vị 3X3 gây ra 800 sát thương";
}
