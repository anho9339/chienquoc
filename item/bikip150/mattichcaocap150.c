...
void create()
{
        set_name("Túi Mật Tịch Cao Cấp Môn Phái");
        set_picid_1(3105);
        set_picid_2(3105);
        set_value(5000);
		set_save("mt150",8500);
...
{ 
        return "Sau khi sử dụng sẽ nhận được "HIY "Mật Tịch Cao Cấp cấp 150"NOR " tương ứng Môn Phái";
}
...
//	weapon->add_to_user(p);
	item = new(sprintf("item/bikip150/mattichcaocap/MatTichCaoCap%d",family));
	p = item->move2(who,-1);
	item->add_to_user(p);
