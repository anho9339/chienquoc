...
void create()
{
        set_name("Túi Mật Tịch Trung Cấp Môn Phái");
        set_picid_1(3101);
        set_picid_2(3101);
        set_value(5000);
		set_save("mt150",7000);
...
{ 
        return "Sau khi sử dụng sẽ nhận được "HIY "Mật Tịch Trung Cấp cấp 150"NOR " tương ứng Môn Phái";
}
...
//	weapon->add_to_user(p);
	item = new(sprintf("item/bikip150/trungcap%d",family));
	p = item->move2(who,-1);
	item->add_to_user(p);
