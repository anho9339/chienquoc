...
void create()
{
        set_name("Túi Mật Tịch Sơ Cấp 150 phái Vân Mộng Cốc");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000);
}
...
{ 
        return "Sau khi sử dụng sẽ nhận được bí kíp cấp 150 phái Vân Mộng Cốc \n"HIY "++ Kim Giáp Thần Công ++\n";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
//	item = new(sprintf("item/skill/02815"));	
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/skill/05914"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
