...
void create()
{
        set_name("Túi Mật Tịch Trung Cấp 150 phái Đào Hoa Nguyên");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000);
}
...
{ 
        return "Sau khi sử dụng sẽ nhận được bí kíp "HIY " Trung Cấp "NOR " cấp 150 phái Đào Hoa Nguyên \n"HIY "++ Truy Viễn Thiên Hoang ++\n "HIY "++ .... ++";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
//	item = new(sprintf("item/skill/03175"));	
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/skill/03193"));	
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
