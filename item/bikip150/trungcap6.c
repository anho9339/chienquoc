...
void create()
{
        set_name("Túi Mật Tịch Trung Cấp 150 phái Côn Luân");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000);
}
...
{ 
        return "Sau khi sử dụng sẽ nhận được bí kíp "HIY " Trung Cấp "NOR " cấp 150 phái Côn Luân Kiếm  \n "HIY "++ Xoay Chuyển Càn Khôn ++ ";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/skill/04250"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
