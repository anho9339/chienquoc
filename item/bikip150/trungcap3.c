...
void create()
{
        set_name("Túi Mật Tịch Trung Cấp 150 phái Cấm Vệ Quân");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000);
}
...
{ 
        return "Sau khi sử dụng sẽ nhận được bí kíp "HIY " Trung Cấp "NOR " cấp 150 phái Cấm Vệ Quân\n"HIY "++ .... ++ \n"HIY "++ .... ++ \n"HIY "++ Bá Khí Phong Thuật ++";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
//	item = new(sprintf("item/skill/02423"));	
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p); 
//	item = new(sprintf("item/skill/02418"));
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/skill/02518"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
