...
void create()
{
        set_name( "Mật Tịch Cao Cấp phái Đào Hoa Nguyên" );
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000);
}
...
//	item = new("item/skill/03163");
	item = new("item/skill/DHN");
	gift->remove_from_user();
	destruct(gift);
	send_user(who,"%c%s",';',"Bạn nhận được "HIR " Mật Tịch Cao Cấp Đào Hoa Nguyên - Thần Công Vô Địch");
	p = item->move2(who,-1);
	item->add_to_user(p);
	return 0;
}