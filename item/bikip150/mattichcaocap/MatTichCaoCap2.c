...
void create()
{
        set_name("Mật Tịch Cao Cấp phái Thục Sơn");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000);
}
...
{ 
        return "Sau khi sử dụng sẽ nhận được kỹ năng Tuyệt Kĩ của phái Thục Sơn \n+++++ "HIY " Thần Công Vô Địch "NOR " +++++ ";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
//	item = new(sprintf("item/skill/04224"));
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/skill/TS"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/skill/TS2"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
		
		send_user(who,"%c%s",';',"Bạn nhận được "HIR " Mật Tịch Cao Cấp Thục Sơn - Thần Công Vô Địch");
