...
void create()
{
        set_name( "Mật Tịch Cao Cấp phái Vân Mộng Cốc" );
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000);
}
...
//	item = new("item/skill/05915");
	item = new("item/skill/VMC");
	gift->remove_from_user();
	destruct(gift);
	send_user(who,"%c%s",';',"Bạn nhận được "HIR " Mật Tịch Cao Cấp Vân Mộng Cốc - Thần Công Vô Địch");
	p = item->move2(who,-1);
	item->add_to_user(p);
	return 0;
}