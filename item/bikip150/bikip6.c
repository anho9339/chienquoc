...
void create()
{
        set_name("Túi Mật Tịch Sơ Cấp 150 phái Côn Luân");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000);
}
...
{ 
        return "Sau khi sử dụng sẽ nhận được bí kíp cấp 150 phái Côn Luân \n"HIY "++ .... ++ \n"HIY "++ Lôi Nộ Nhân Gian ++";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
//	item = new(sprintf("item/skill/04223"));	
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p);
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/skill/0423"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
