...
void create()
{
        set_name("Túi Mật Tịch Đặc Biệt cấp 150 phái Cấm Vệ Quân");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000000);
		set_no_give(1);		
...
{ 
        return "Sau khi sử dụng sẽ nhận được bí kíp cấp 150 phái Cấm Vệ Quân \n"HIY "++ Từ Bi Tế Vũ ++";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/skill/02426"));	
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
//	item = new(sprintf("item/skill/03487"));
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p);	
