...
void create()
{
        set_name("Túi Mật Tịch Đặc Biệt cấp 150 Môn Phái");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(4000000);
		set_no_give(1);		
...
{ 
        return "Sau khi sử dụng có thể nhận được "HIY "Mật Tịch Đặc Biệt cấp 150"NOR " tương ứng Môn Phái";
}
...
//	weapon->add_to_user(p);
	item = new(sprintf("item/bikip150/mattichchuyensinh200/bikip%d",family));
	p = item->move2(who,-1);
	item->add_to_user(p);
