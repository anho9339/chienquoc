...
void create()
{
        set_name("Túi Mật Tịch Trung Cấp 150 phái Thục Sơn");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000);
}
...
{ 
        return "Sau khi sử dụng sẽ nhận được bí kíp "HIY " Trung Cấp "NOR " cấp 150 phái Thục Sơn \n "HIY "++ Bát Quái Trận Pháp ++ \n "HIY "++ .... ++ \n "HIY "++ .... ++ ";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
//	item = new(sprintf("item/skill/03255"));	
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p);
//	item = new(sprintf("item/skill/02217"));	
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/skill/02319"));	
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
//	item = new(sprintf("item/skill/03298"));	
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p);
//	item = new(sprintf("item/skill/03299"));	
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p);
