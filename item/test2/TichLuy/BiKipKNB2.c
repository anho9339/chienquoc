...
void create()
{
        set_name("Bí Kíp Kim Nguyên Bảo(200)");
        set_picid_1(0089);
        set_picid_2(0089);
        set_unit("本");
        set_value(100);
...
int get_use_effect_callout( object me, object item )
{
        if( "sys/sys/test_db"->get_yuanbao(item) < 1 )
        {
                ...
                {
                        send_user(me, "%c%s", '!', "Không đủ 200 Kim Nguyên Bảo !");
                        return 0;
                }
                "sys/sys/test_db"->add_yuanbao(me,-2000);
                "sys/sys/test_db"->set_yuanbao(item,2000);
                write_user(me, "Bạn đem 200 Kim Nguyên Bảo cất vào %s.", item->get_name() );
        ...
        {
                "sys/sys/test_db"->add_yuanbao(me,2000);
				send_user(me,"%c%s",';',"Bạn nhận được "HIY " 200 Kim Nguyên Bảo");
                return 1;
        }
