...
void create()
{
        set_name("Bí Kíp Kim Nguyên Bảo(500)");
        set_picid_1(0089);
        set_picid_2(0089);
        set_unit("本");
        set_value(400000);
...
int get_use_effect_callout( object me, object item )
{
        if( "sys/sys/test_db"->get_yuanbao(item) < 1 )
        {
                ...
                {
                        send_user(me, "%c%s", '!', "Không đủ 500 Kim Nguyên Bảo !");
                        return 0;
                }
                "sys/sys/test_db"->add_yuanbao(me,-5000);
                "sys/sys/test_db"->set_yuanbao(item,5000);
                write_user(me, "Bạn đem 500 Kim Nguyên Bảo cất vào %s.", item->get_name() );
        ...
        {
                "sys/sys/test_db"->add_yuanbao(me,5000);
				send_user(me,"%c%s",';',"Bạn nhận được "HIY " 500 Kim Nguyên Bảo");
                return 1;
        }
