...
int get_use_effect_callout( object me, object item )
{
        ...
        {
                if( "sys/sys/test_db"->get_yuanbao(me) < 500 )
                {
                        send_user(me, "%c%s", '!', "Không đủ 50 Kim Nguyên Bảo !");
                        return 0;
                }
                "sys/sys/test_db"->add_yuanbao(me,-500);
                "sys/sys/test_db"->set_yuanbao(item,500);
                write_user(me, "Bạn đem 50 Kim Nguyên Bảo cất vào %s.", item->get_name() );
