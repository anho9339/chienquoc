...
{
	string desc;
	return desc = "Sau khi sử dụng có thể lập tức hồi phục cho Bảo Thú 2000 Pháp lực. \nTái sử dụng sau 10 giây.";
}
...
int get_use_effect_call(object me,object item)
{
	...
	if ( !me->is_pet() )
		return 0;
	owner = me->get_owner();
	if ( gone_time(me->get("svd1" )) < 10 )
	{
		send_user(owner,"%c%s",'!',sprintf("Tái sử dụng sau %d giây",10-gone_time(me->get("svd1" )) ));
		return 0;	
	}
	me->add_mp( 2000 );
	me->set("svd1", time() );	
