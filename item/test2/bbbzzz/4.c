...
string get_desc() 
{ 
        return "Sử dụng sẽ tăng Cống hiến Bang Phái thêm 536 điểm。"; 
}
...
int get_use_effect_callout( object who,object item) 
{
	...
	send_user( who, "%c%c%w%s", ':', 3, 0,sprintf("Sử dụng sẽ tăng Cống hiến Bang Phái thêm 536 điểm, muốn sử dụng không ?\n"ESC"Xác nhận\nuse ! %x#\n"ESC"Huỷ bỏ",getoid(item))); 
	return 0;
}

string get_loop_desc(object me)
{
	return "Sau khi sử dụng gia tăng Cống hiến 536 điểm";
}
// 函数：使用效果
...
int confirm_use_callout( object who, object item )
{
	...
	org->set_member_contribute(number, org->get_member_contribute(number)+536);
	org->save();
	return 1;
}