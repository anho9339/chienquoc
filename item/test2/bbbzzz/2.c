...
string get_desc() 
{ 
        return "Sử dụng sẽ tăng độ Huấn Luyện Bang Phái thêm 1 điểm。"; 
}
...
int get_use_effect_callout( object who,object item) 
{
	...
	send_user( who, "%c%c%w%s", ':', 3, 0,sprintf("Sử dụng sẽ tăng độ Huấn Luyện Bang Phái thêm 1 điểm, muốn sử dụng không ?\n"ESC"Xác nhận\nuse ! %x#\n"ESC"Huỷ bỏ",getoid(item))); 
	return 0;
}

string get_loop_desc(object me)
{
	return "Sau khi sử dụng gia tăng Huấn luyện 1 điểm";
}
// 函数：使用效果
