...
int get_use_effect_callout( object who,object gift) 
{
	...
	switch( random(2) )
		 ...
		 who->add_exp(650+random(100));
	else if (rate<60) {
		who->add_cash(1400+random(1000));
		send_user(who,"%c%s",'!',sprintf("Bạn nhận được lượng lớn ngân lượng."));
		}
	else if (rate<70)
		who->add_exp(1400+random(1000));
	else if (rate<80) {
		who->add_cash(650+random(100));
	...
	//	CHAT_D->sys_channel(0,sprintf(HIY"%s"HIG" vừa trúng sổ xố 3 ,đã nhận được 1 "HIY"%s"HIG".Xin chúc mừng！",who->get_name(),item->get_name()));
		send_user(who,"%c%s",'!',sprintf("Bạn nhận được vật phẩm %s từ sổ xố 4.",item->get_name()));
		p = item->move2(who);
		item->add_to_user(p);	
	}
	/*else if (who->add_cash(400))
	{
		send_user(who,"%c%s",'!',sprintf("Bạn nhận được %d vàng",cash));
	}		
	else if (who->add_exp(400))
	{
