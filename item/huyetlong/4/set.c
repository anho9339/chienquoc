...
int get_use_effect_callout( object who,object gift) 
{
	...
	TASK_LEGEND_D->check_task_item5(who,"Set HS",1);
		...
		item1 = new("item/huyetlong/4/ao");
	k = item1->move2(who,-1);
		item1->add_to_user(k);	
	
	item2 = new("item/huyetlong/4/giay");
	l = item2->move2(who,-1);
		item2->add_to_user(l);	
       
		item3 = new("item/huyetlong/4/non");
	m = item3->move2(who,-1);
		item3->add_to_user(m);


       item4 = new("item/huyetlong/4/hanglien");	   
	n = item4->move2(who,-1);
		item4->add_to_user(n);

		
       item5 = new("item/huyetlong/4/yeudoi");		
	q = item5->move2(who,-1);
		item5->add_to_user(q);
