...
void create()
{
    ...
    if (ch > 70)
        bb = bb3[random(sizeof(bb3))];

    else if (ch > 40)
        bb = bb4[random(sizeof(bb4))];
...
int get_use_effect_callout(object who, object item)
{
    send_user(who, "%c%c%w%s", ':', 3, 0,
              sprintf("Ngươi muốn sử dụng %s không ? \n" ESC "Chấp nhận\nuse ! %x#\n" ESC "Huỷ bỏ", item->get_name(),
                      getoid(item)));
    return 0;
}
...
int confirm_use_callout(object who, object item)
{
    ...
    if (item->get_card_id() == 800 || item->get_card_id() == 8010 || item->get_card_id() == 8011 ||
        item->get_card_id() == 8012 || item->get_card_id() == 8013 || item->get_card_id() == 402 ||
        item->get_card_id() == 352 || item->get_card_id() == 4143)
    {
    ...
    if (item->get_card_id() == 954 || item->get_card_id() == 953 || item->get_card_id() == 952 ||
        item->get_card_id() == 951 || item->get_card_id() == 950)
    {
        pp = who->get_pp() / 10;
        who->set_2("magic_card.pp", pp);
        USER_ENERGY_D->count_pp(who);
