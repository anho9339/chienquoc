...
void create()
{
        set_name("Túi Vũ Khí Bá Vương phái Cấm Vệ Quân");
        set_picid_1(3104);
        set_picid_2(3104);
        set_value(5000000);
}
...
{ 
        return "Sau khi sử dụng nhận được vũ khí Bá Vương phái Cấm Vệ Quân \n "HIR "Lưu ý: Phải gia nhập Phái"NOR ".";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/set10sao/bavuong_15/bongphap"));	
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/bavuong_15/thuongphap"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
