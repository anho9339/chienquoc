...
void create()
{
        set_name("Túi Hoàng Kim Bá Vương 15 sao Cấp 150");
        set_picid_1(3104);
        set_picid_2(3104);
        set_value(5000);
		set_no_give(0);		
...
{ 
        return "Sau khi sử dụng có thể nhận được một bộ "HIR "Trang bị"HIY " Hoàng Kim Bá Vương "NOR " 15 sao Cấp 150 \n "HIR "Lưu ý: Phải gia nhập Phái"NOR ".";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuong_15/ao"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuong_15/non"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuong_15/hanglien"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuong_15/yeudoi"));
	item->set_hide(0);
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuong_15/giay"));	
	item->set_hide(0);
	p = item->move2(who,-1);
	item->add_to_user(p);
