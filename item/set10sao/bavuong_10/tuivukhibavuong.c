...
{ 
        return "Sau khi sử dụng sẽ nhận được "HIY "vũ khí Bá Vương"NOR " theo Môn Phái";
}
...
//	weapon->add_to_user(p);
	item = new(sprintf("item/set10sao/bavuong_10/vukhi%d",family));
	p = item->move2(who,-1);
	item->add_to_user(p);
