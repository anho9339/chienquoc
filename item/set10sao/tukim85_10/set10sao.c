...
void create()
{
        set_name("Túi Tử Kim cấp 85 (10 sao)");
        set_picid_1(3104);
        set_picid_2(3104);
        set_value(5000);
		set_no_give(0);		
...
{ 
        return "Sau khi sử dụng có thể nhận được một bộ "HIR "Trang bị"HIY " Tử Kim "NOR " 10 sao cấp 85 \n "HIR "Lưu ý: Phải gia nhập Phái"NOR ".";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/set10sao/tukim85_10/ao"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/tukim85_10/non"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/tukim85_10/hanglien"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/tukim85_10/yeudoi"));
	item->set_hide(0);
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/tukim85_10/giay"));	
	item->set_hide(0);
	p = item->move2(who,-1);
	item->add_to_user(p);
