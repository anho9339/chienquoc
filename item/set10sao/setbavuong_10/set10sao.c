...
void create()
{
        set_name("Túi Hoàng Kim Bá Vương 10 sao Cấp 150");
        set_picid_1(3104);
        set_picid_2(3104);
        set_value(5000);
		set_no_give(0);		
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuong_10/ao"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuong_10/non"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuong_10/hanglien"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuong_10/yeudoi"));
	item->set_hide(0);
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuong_10/giay"));	
	item->set_hide(0);
	p = item->move2(who,-1);
	item->add_to_user(p);
