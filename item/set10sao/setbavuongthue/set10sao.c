...
void create()
{
        set_name("Túi Bá Vương 10 sao Cấp 150 (Thuê)");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000);
		set_no_give(0);		
...
{ 
        return "Sau khi sử dụng có thể nhận được một bộ "HIR "Trang bị"HIY " Hoàng Kim Bá Vương "NOR " 10 sao Cấp 150 \n "HIR "Lưu ý: Phải gia nhập Phái"NOR ".";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuongthue/ao"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuongthue/non"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuongthue/hanglien"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuongthue/yeudoi"));
	item->set_hide(0);
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/setbavuongthue/giay"));	
	item->set_hide(0);
	p = item->move2(who,-1);
	item->add_to_user(p);
