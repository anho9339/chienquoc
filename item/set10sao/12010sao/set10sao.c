...
void create()
{
        set_name("Túi Hoàng Kim 10* Cấp 120");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(5000);
}
...
{ 
        return "Sau khi sử dụng có thể nhận được một bộ "HIR "Trang bị"HIY " Hoàng Kim"NOR " và "HIR " Vũ khí"NOR " 10 sao Cấp 120 \n "HIR "Lưu ý:"HIY " Phải gia nhập Phái"NOR ".";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/set10sao/12010sao/vukhi"));	
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/12010sao/ao"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/12010sao/non"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/12010sao/hanglien"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/12010sao/yeudoi"));
	item->set_hide(0);
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/12010sao/giay"));	
	item->set_hide(0);
	p = item->move2(who,-1);
	item->add_to_user(p);
