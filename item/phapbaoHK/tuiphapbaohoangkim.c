...
void create()
{
        set_name("Túi Pháp Bảo Hoàng Kim");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(4000000);
}
...
{ 
        return "Sau khi sử dụng sẽ nhận được "HIY "Pháp Bảo Hoàng Kim"NOR " theo Môn Phái";
}
...
//	weapon->add_to_user(p);
	item = new(sprintf("item/phapbaoHK/%d000",family));
	p = item->move2(who,-1);
	item->add_to_user(p);
