...
void create()
{
        set_name( "Bá Vương Phong Thuật Chuông" );
        set_picid_1(9561);
        set_picid_2(9561);
        set_unit( "个" );
...
int get_use_effect_2_callout( object me, object who, object item, string arg )
{
        ...
        if( random(100) < item->get_active_rate() )    // 成功率
        {
                ...
                write_user( me, ECHO "Pháp bảo của %s bị bạn dùng " HIY "Bá Vương Phong Thuật Chuông" ECHO " áp chế, đã mất đi hiệu quả pháp bảo.", who->get_name() );
                write_user( who, ECHO "%s của bạn bị " HIY "Bá Vương Phong Thuật Chuông" ECHO " áp chế, đã mất đi hiệu quả pháp bảo.", me->get_name() );
        }
        else    write_user( me, ECHO "Bạn áp chế Pháp bảo của %s bằng " HIY "Bá Vương Phong Thuật Chuông" ECHO " thất bại.", who->get_name() );
...
void start_loop_perform( object me )
{
	...
	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 41322, 1,UNDER_FOOT, PF_LOOP );
...
{
	CHAR_CHAR_D->init_loop_perform(me);
	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 41322, 1, UNDER_FOOT, PF_STOP );
