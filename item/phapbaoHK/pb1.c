...
string get_desc() 
{ 
        return "Môn phái thích hợp: [Đào Hoa Nguyên] \n★★★★★★★★★ \nSau khi trang bị Pháp Bảo này, các đòn tấn công\nngoại công (Đào Hoa Quyền Pháp) có tỷ lệ gây 30％ gây choáng đối phương.";
}

void start_loop_perform( object me )
{
	CHAR_CHAR_D->init_loop_perform(me);
	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 41321, 1,UNDER_FOOT, PF_LOOP );
	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 41312, 1,UNDER_FOOT, PF_LOOP );
...
{
	CHAR_CHAR_D->init_loop_perform(me);
	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 41321, 1, UNDER_FOOT, PF_STOP );
	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 41312, 1, UNDER_FOOT, PF_STOP );
