...
int get_use_effect_2_callout( object me, object who, object item, string arg )
{
        ...
        if( random(100) < item->get_active_rate() )    // 成功率
        {
                ...
                write_user( me, ECHO "Pháp bảo của %s bị bạn dùng " HIY "Bá Vương Quỷ Chuông" ECHO " áp chế, đã mất đi hiệu quả pháp bảo.", who->get_name() );
                write_user( who, ECHO "%s của bạn bị " HIY "Bá Vương Quỷ Chuông" ECHO " áp chế, đã mất đi hiệu quả pháp bảo.", me->get_name() );
        }
        else    write_user( me, ECHO "Bạn áp chế Pháp bảo của %s bằng " HIY "Bá Vương Quỷ Chuông" ECHO " thất bại.", who->get_name() );
