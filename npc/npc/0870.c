...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_30, 12) && !who->get_legend(TASK_30, 13) && who->get_save("lbtgongjsy") < 30 )
	{
		if ( (i=who->add_save("lbtgongjsy",1)) >= 30 && who->get_save("lbtqianyong") >= 30)
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,37,"威慑对手(完成)" );
			who->set_legend(TASK_30, 13);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,37);
		send_user(who,"%c%s",'!',sprintf(HIY"Cung Tiễn Thạch Dũng %d/30",i));		
	}
	if( who->get_legend(TASK_31, 3) && !who->get_legend(TASK_31, 4) && who->get_save("qxfgongjsy") < 30 )
	{
		if ( (i=who->add_save("qxfgongjsy",1)) >= 30 && who->get_save("qxfqianyong") >= 30)
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,46,"解除疑惑(完成)" );
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6509 * rate / 100 ) ;    // NONE

/*              else if( p < 6509 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6509 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                       ...
                       case 5 : file = "/item/stuff/0051";  break;
