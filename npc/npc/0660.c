...
void create()
{
        set_name("Tần Quốc Đao Thuẫn Binh");
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_18, 16) && !who->get_legend(TASK_18, 17) && random(100) < 35)
	{
		...
		if ( (i=who->add_save_2("qgdjjjian",1)) >= 20 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,11,"大展神威（1）(完成)" );
		}
	...
	if( who->get_legend(TASK_18, 22) && !who->get_legend(TASK_18, 23) && random(100) < 35)
	{
		...
		if ( (i=who->add_save_2("qgdjjchutou",1)) >= 20 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,14,"大展神威（4）(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_QINGUO,14);
		send_user(who,"%c%s",'!',sprintf(HIY"锄头 %d/20",i));		
	}
	if( who->get_legend(TASK_19, 10) && !who->get_legend(TASK_19, 11) && random(100) < 35)
	{
		item = new("item/98/0086");
		if ( !item  )
			return ;
		p = item->move(who,-1);
		if ( !p )
		...
		item->add_to_user(p);
		who->set_legend(TASK_19, 11);
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,19,"大葫芦（5）(完成)" );
		USER_TASK_D->send_task_intro(who,2,TID_QINGUO,19);
		send_user(who,"%c%s",'!',HIY"锯子 1/1");		
	}
	if( who->get_legend(TASK_19, 13) && !who->get_legend(TASK_19, 14) && who->get_save_2("qgswdaodunbing",1) < 10)
	{
		if ( (i=who->add_save_2("qgswdaodunbing",1)) >= 10 && who->get_save_2("qgswqiangbing",1) >= 10 )
		{
			who->set_legend(TASK_19, 14);
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,20,"无耻的叛徒(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_QINGUO,20);
		send_user(who,"%c%s",'!',sprintf(HIY"Tần Quốc Đao Thuẫn Binh %d/10",i));		
	}
	if( who->get_legend(TASK_20, 4) && !who->get_legend(TASK_20, 5) )
	{
		if ( (i=who->add_save_2("sarenemo",1)) >= 20  )
		{
			who->set_legend(TASK_20, 5);
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,48,"杀人恶魔(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_QINGUO,48);
		send_user(who,"%c%s",'!',sprintf(HIY"Tần Quốc Đao Thuẫn Binh %d/20",i));		
	}
	if( who->get_legend(TASK_20, 10) && !who->get_legend(TASK_20, 11) && random(100) < 10)
	{
		...
		if ( (i=who->add_save_2("qgbxzhiqian",1)) >= 5 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,50,"重阳节（1）(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_QINGUO,50);
		send_user(who,"%c%s",'!',sprintf(HIY"纸钱 %d/5",i));		
	}
	if( who->get_legend(TASK_22, 13) && !who->get_legend(TASK_22, 14) && who->get_save_2("qzgdaodunbing",1) < 10)
	{
		if ( (i=who->add_save_2("qzgdaodunbing",1)) >= 10 && who->get_save_2("qzgqiangbing",1) >= 10 && who->get_save_2("qzggongbing",1) >= 10 )
		{
			who->set_legend(TASK_22, 14);
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,45,"牵连之罪(完成)" );
		}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5500 * rate / 100 ) ;    // NONE

                else if( p < 5750 * rate / 100 )    // 金钱
                {
