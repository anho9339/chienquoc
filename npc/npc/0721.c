...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_23, 1) && !who->get_legend(TASK_23, 2) )
	{
		if ( (i=who->add_save_2("hgdjjmitan",1)) >= 10 )
		{
			who->set_legend(TASK_23, 2);
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,44,"泄密之灾(完成)" );
		}
		send_user(who,"%c%s",'!',sprintf(HIY"Mật Thám %d /10",i));	
		USER_TASK_D->send_task_intro(who,2,TID_HANGUO,44);
	}
	i = random(100);
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 7455 * rate / 100 ) ;    // NONE

                else if( p < 9255 * rate / 100 )    // 金钱
                {
