...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_33, 1) && !who->get_legend(TASK_33, 2) && who->get_save("ltjinj") < 5 )
	{
		if ( (i=who->add_save("ltjinj",1)) >= 5 && who->get_save("ltmuj") >= 7)
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,33,"祖父的遗产（3）(完成)" );
			who->set_legend(TASK_33, 2);
		}
		USER_TASK_D->send_task_intro(who,2,TID_CHUGUO,33);
		send_user(who,"%c%s",'!',sprintf(HIY"Kim Tinh %d/5",i));		
	}
	...
	if( who->get_legend(TASK_33, 27) && !who->get_legend(TASK_33, 28) && who->get_save("lidejinjing") < 30 )
	{
		if ( (i=who->add_save("lidejinjing",1)) >= 30 && who->get_save("lidetujing") >= 30)
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Kim Tinh %d/30",i));		
	}
	if( who->get_legend(TASK_34, 25) && !who->get_legend(TASK_34, 26) && who->get_save("ssyjinjing") < 30 )
	{
		if ( (i=who->add_save("ssyjinjing",1)) >= 30 && who->get_save("ssyhuojing") >= 30)
		{
	...
	if( who->get_legend(TASK_44, 21) && !who->get_legend(TASK_44, 22) && who->get_save("qzgjinj") < 10 )
	{
		if ( (i=who->add_save("qzgjinj",1)) >= 10 && who->get_save("qzgdismzs") >= 8 && who->get_save("qzgtaijmzs") >= 8 && who->get_save("qzgdaohun") >= 4 )
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Kim Tinh %d/10",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6564 * rate / 100 ) ;    // NONE

                else if( p < 6814 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6814 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                        switch( random(4) )
                        {
                       case 0 : file = "/item/stuff/0026";  break;
