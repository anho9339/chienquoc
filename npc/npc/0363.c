...
void create()
{
        set_name("Xích Viêm Xà");

        set_char_type(FIGHTER_TYPE);    // init_level 要用到
        
        set_head_color(0x2a05);
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4924 * rate / 100 ) ;    // NONE

/*              else if( p < 4924 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 4924 * rate / 100 )    // 武器(门派)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/40/4044";  break;
