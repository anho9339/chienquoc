...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_10, 20) && !who->get_legend(TASK_10, 21) && who->get_save_2("tpzuangding") < 30)
	{
		if ( (i=who->add_save_2("tpzuangding",1)) >= 30 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,1,"逃跑的壮丁(完成)" );
			who->set_save_2("tpzuangding",30);
			who->set_legend(TASK_10, 21);
		}

		USER_TASK_D->send_task_intro(who,2,TID_QINGUO,1);
		send_user(who,"%c%s",'!',sprintf(HIY"Tráng Đinh %d/30",i));		
	}
	if( who->get_legend(TASK_23, 22) && !who->get_legend(TASK_23, 23) )
	{
		if ( (i=who->add_save_2("hgwszuangding",1)) >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,46,"武士的爱情（1）(完成)" );
			who->set_save_2("hgwszuangding",10);
			who->set_legend(TASK_23, 23);
		}

		USER_TASK_D->send_task_intro(who,2,TID_HANGUO,46);
		send_user(who,"%c%s",'!',sprintf(HIY"Tráng Đinh %d/10",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5274 * rate / 100 ) ;    // NONE

                else if( p < 5524 * rate / 100 )    // 金钱
                {
