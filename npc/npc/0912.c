...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_31, 18) && !who->get_legend(TASK_31, 19) && who->get_save("cfxieling") < 7 )
	{
		if ( (i=who->add_save("cfxieling",1)) >= 7 && who->get_save("cfkuly") >= 5)
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Tà Linh %d/7",i));		
	}
	...
	if( who->get_legend(TASK_32, 6) && !who->get_legend(TASK_32, 7) && random(100) < 22)
	{
		if ( objectp( item = present("荔枝", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save("ygsjlizi",1)) >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,53,"使节的差事（2）(完成)" );
			who->set_legend(TASK_32, 7);
		}

		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,53);
		send_user(who,"%c%s",'!',sprintf(HIY"荔枝 %d/10",i));		
	}
	...
	if( who->get_legend(TASK_32, 23) && !who->get_legend(TASK_32, 24) && who->get_save("ywxieling") < 5 )
	{
		if ( (i=who->add_save("ywxieling",1)) >= 5 && who->get_save("ywkulouy") >= 5 && who->get_save("ywhaiguj") >= 5)
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Tà Linh %d/5",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6445 * rate / 100 ) ;    // NONE

                else if( p < 6695 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6695 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                       ...
                       case 1 : file = "/item/40/4071";  break;
