...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_29, 13) && !who->get_legend(TASK_29, 14) && who->get_save("wgswshihu") < 10 )
	{
		if ( (i=who->add_save("wgswshihu",1)) >= 10 && who->get_save("wgswdaoyong") >= 8 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,34,"成为守卫的条件(完成)" );
			who->set_save("wgswshihu",10);
			who->set_legend(TASK_29, 14);
		}
		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,34);
		send_user(who,"%c%s",'!',sprintf(HIY"Thạch Hổ %d/10",i));		
	}
	if( who->get_legend(TASK_29, 18) && !who->get_legend(TASK_29, 19) && who->get_save("qzgshihu") < 30 )
	{
		if ( (i=who->add_save("qzgshihu",1)) >= 30 && who->get_save("qzgshisi") >= 30)
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Thạch Hổ %d/30",i));		
	}
	if( who->get_legend(TASK_30, 24) && !who->get_legend(TASK_30, 25) && who->get_save("qgsrshihu") < 30 )
	{
		if ( (i=who->add_save("qgsrshihu",1)) >= 30 && who->get_save("qgsrshisi") >= 30)
		{
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6416 * rate / 100 ) ;    // NONE

/*              else if( p < 6416 * rate / 100 )    // 金钱
                {
