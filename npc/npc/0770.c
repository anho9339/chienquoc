...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_24, 20) && !who->get_legend(TASK_24, 21) )
	{
		if ( (i=who->add_save_2("qgdjjyuanhun",1)) >= 20 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,17,"该死的Oan Hồn(完成)" );
			who->set_save_2("qgdjjyuanhun",20);
			who->set_legend(TASK_24, 21);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,17);
		send_user(who,"%c%s",'!',sprintf(HIY"Oan Hồn %d/20",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5728 * rate / 100 ) ;    // NONE

                else if( p < 5978 * rate / 100 )    // 金钱
                {
