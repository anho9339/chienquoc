...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_33, 7) && !who->get_legend(TASK_33, 8) && who->get_save("cgswsuijing") < 7 )
	{
		if ( (i=who->add_save("cgswsuijing",1)) >= 7 && who->get_save("cgswtujing") >= 6)
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Thuỷ Tinh %d/7",i));		
	}
	...
	if( who->get_legend(TASK_33, 22) && !who->get_legend(TASK_33, 23) && who->get_save("cggjsuij") < 10 )
	{
		if ( (i=who->add_save("cggjsuij",1)) >= 10 && who->get_save("cggjhuoj") >= 8)
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Thuỷ Tinh %d/10",i));		
	}
	if( who->get_legend(TASK_34, 11) && !who->get_legend(TASK_34, 12) && who->get_save("wlhsuijing") < 30 )
	{
		if ( (i=who->add_save("wlhsuijing",1)) >= 30 && who->get_save("wlhhuojing") >= 30)
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Thuỷ Tinh %d/30",i));		
	}
	if( who->get_legend(TASK_35, 7) && !who->get_legend(TASK_35, 8) && who->get_save("ztsuijing") < 30 )
	{
		if ( (i=who->add_save("ztsuijing",1)) >= 30 && who->get_save("zthuojing") >= 30)
		{
	...
	if( who->get_legend(TASK_35, 17) && !who->get_legend(TASK_35, 18) && who->get_save("wlhsuijing1") < 30 )
	{
		if ( (i=who->add_save("wlhsuijing1",1)) >= 30 && who->get_save("wlhtujing1") >= 30)
		{
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6891 * rate / 100 ) ;    // NONE

/*              else if( p < 6891 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6891 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                       ...
                       case 2 : file = "/item/stuff/0028";  break;
