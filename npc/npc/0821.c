...
// 函数：获取人物造型
int get_char_picid() { return 4145; }

// 函数：构造处理
void create()
{
        set_name("Đậu Tướng Quân");
        ...
        NPC_ENERGY_D->init_level( this_object(), 82+ random(4) );    // 
        set_max_seek(4);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2314 * rate / 100 ) ;    // NONE

                else if( p < 7834 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 9050 * rate / 100 )    // 杂物１
                ...
                else if( p < 9500 * rate / 100 )    // 杂物２
                {
                        switch( random(4) )
                        {
                       case 0 : file = "/item/std/0401";  break;
