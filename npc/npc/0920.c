...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_32, 11) && !who->get_legend(TASK_32, 12) && who->get_save("ygswsimo") < 8 )
	{
		if ( (i=who->add_save("ygswsimo",1)) >= 8 && who->get_save("ygswyuanling") >= 6)
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Thi Ma %d/8",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6482 * rate / 100 ) ;    // NONE

                else if( p < 6732 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6732 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(3) )
                        {
                       case 0 : file = "/item/40/4072";  break;
