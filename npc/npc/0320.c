...
void check_legend_task( object who )
{
	int p,i;
	object item;
	if( who->get_legend(TASK_07, 13) && !who->get_legend(TASK_07, 14) && random(100) < 50)
	{
		if ( objectp( item = present("南瓜", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save_2("wgjnangua",1)) >= 20 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,8,"魏管家的劫难（1）(完成)" );
			who->set_save_2("wgjnangua",20);
			who->set_legend(TASK_07, 14);
		}

		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,8);
		send_user(who,"%c%s",'!',sprintf(HIY"南瓜 %d/20",i));		
	}
	if( who->get_legend(TASK_07, 22) && !who->get_legend(TASK_07, 23) && random(100) < 28 && who->get_save_2("ssyluobo") < 4)
	{
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5110 * rate / 100 ) ;    // NONE

                else if( p < 5360 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 5510 * rate / 100 )    // 防具(门派)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(2) )
                        {
                       case 0 : file = "/item/40/4040";  break;
                       case 1 : file = "/item/40/4039";  break;
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                        switch( random(2) )
                        {
                       case 0 : file = "/item/91/9101";  break;
                       case 1 : file = "/item/stuff/0191";  break;
