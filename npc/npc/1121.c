...
// 函数：获取人物造型
int get_char_picid() { return 1101; }
...
void check_legend_task( object who ) 
{
	...
	if( who->get_legend(TASK_44, 13) && !who->get_legend(TASK_44, 14) && who->get_save("qgjjianhun") < 8 )
	{
		if ( (i=who->add_save("qgjjianhun",1)) >= 8 && who->get_save("qgjdaohun") >= 8 )
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Kiếm Hồn %d /8",i));		
	}	
	...
	if( who->get_legend(TASK_45, 7) && !who->get_legend(TASK_45, 8) && who->get_save("qwjianhun") < 10 )
	{
		if ( (i=who->add_save("qwjianhun",1)) >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,69,"无上的荣耀（1）(完成)" );
			who->set_legend(TASK_45, 8);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,69);
		send_user(who,"%c%s",'!',sprintf(HIY"Kiếm Hồn %d /10",i));		
	}
	...
	if( who->get_legend(TASK_46, 15) && !who->get_legend(TASK_46, 16) && who->get_save("qysjianhun") < 30 )
	{
		if ( (i=who->add_save("qysjianhun",1)) >= 30 && who->get_save("qysqianhun") >= 30 )
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Kiếm Hồn %d /30",i));		
	}
	if( who->get_legend(TASK_47, 23) && !who->get_legend(TASK_47, 24) && who->get_save("xhdjianhun") < 30 )
	{
		if ( (i=who->add_save("xhdjianhun",1)) >= 30 && who->get_save("xhdqianhun") >= 30 )
		{
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 7367 * rate / 100 ) ;    // NONE

/*              else if( p < 7367 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 7367 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/40/4081";  break;
...
/*              else if( p < 1100000 * rate / 100 )    // 杂物５
                {
                        switch( random(0) )
                        {
