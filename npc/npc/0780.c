...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_24, 26) && !who->get_legend(TASK_24, 27) )
	{
		if ( (i=who->add_save_2("qgdygyigui",1)) >= 20 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,19,"可怕的发现(完成)" );
			who->set_save_2("qgdygyigui",20);
			who->set_legend(TASK_24, 27);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,19);
		send_user(who,"%c%s",'!',sprintf(HIY"Dịch Quỷ %d/20",i));		
	}
	if( who->get_legend(TASK_26, 7) && !who->get_legend(TASK_26, 8) && who->get_save_2("lfsyigui") < 30 )
	{
		if ( (i=who->add_save_2("lfsyigui",1)) >= 30 && who->get_save_2("lfsjiangsi") >= 30)
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,30,"最后的考验(完成)" );
			who->set_save_2("lfsyigui",30);
			who->set_legend(TASK_26, 8);
		}
		USER_TASK_D->send_task_intro(who,2,TID_HANGUO,30);
		send_user(who,"%c%s",'!',sprintf(HIY"Dịch Quỷ %d/30",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5814 * rate / 100 ) ;    // NONE

                else if( p < 6064 * rate / 100 )    // 金钱
                {
