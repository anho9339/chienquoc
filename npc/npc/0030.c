...
void create()
{
        ...
        set_head_color(0x29a5);

        NPC_ENERGY_D->init_level( this_object(), 3+ random(2) );    // 
        set_max_seek(4);
...
void check_legend_task( object who ) 
{
	...
	if ( who->get_legend(TASK_NEWBIE_02, 18) && !who->get_legend(TASK_NEWBIE_02, 19)  )
	{
		if ( objectp( item = present("小麦", who, 1, MAX_CARRY) ) && item->is_xiaomai() == 1 ) 
		{
		...
		if ( (i=who->add_save_2("baqiu_xiaomai",1)) >= 3 )
		{
			who->set_legend(TASK_NEWBIE_02, 19);
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,44,"酿酒的原材料(已经完成)" );
		}
		else
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,44,"酿酒的原材料" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_NEWBIE_01,44);
		send_user(who,"%c%s",'!',sprintf(HIY"小麦 %d/3",i));
