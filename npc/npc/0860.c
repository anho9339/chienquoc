...
// 函数：获取人物造型
int get_char_picid() { return 0850; }
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_29, 13) && !who->get_legend(TASK_29, 14) && who->get_save("wgswdaoyong") < 8 )
	{
		if ( (i=who->add_save("wgswdaoyong",1)) >= 8 && who->get_save("wgswshihu") >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,34,"成为守卫的条件(完成)" );
			who->set_save("wgswdaoyong",8);
			who->set_legend(TASK_29, 14);
		}
		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,34);
		send_user(who,"%c%s",'!',sprintf(HIY"Đao Dũng %d/8",i));		
	}
	if( who->get_legend(TASK_29, 28) && !who->get_legend(TASK_29, 29) && who->get_save("qgbxdaoyong") < 30 )
	{
		if ( (i=who->add_save("qgbxdaoyong",1)) >= 30 && who->get_save("qgbxqianyong") >= 30)
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,33,"考验自己(完成)" );
			who->set_legend(TASK_29, 29);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,33);
		send_user(who,"%c%s",'!',sprintf(HIY"Đao Dũng %d/30",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6475 * rate / 100 ) ;    // NONE

/*              else if( p < 6475 * rate / 100 )    // 金钱
                {
