...
void create()
{
        set_name("Yên Tiên Phong Bộ Binh");
        ...
        NPC_ENERGY_D->init_level( this_object(), 35+ random(4) );    // 
        set_max_seek(4);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2643 * rate / 100 ) ;    // NONE

                else if( p < 3043 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 3043 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9997 * rate / 100 )    // 杂物２
                {
                       ...
                       case 2 : file = "/item/91/9110";  break;
