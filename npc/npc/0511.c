...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_14, 15) && !who->get_legend(TASK_14, 16) && random(100) < 15)
	{
		...
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,28,"二胡(Hoàn thành)" );
		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,28);
		send_user(who,"%c%s",'!',sprintf(HIY"二胡 %d/1",1));		
	}
	if( who->get_legend(TASK_14, 18) && !who->get_legend(TASK_14, 19) && who->get_save_2("tqbengxiong") < 15)
	{
		if ( (i=who->add_save_2("tqbengxiong",1)) >= 15 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,29,"偷钱的笨熊(Hoàn thành)" );
			who->set_save_2("tqbengxiong",15);
			who->set_legend(TASK_14, 19);
		}

		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,29);
		send_user(who,"%c%s",'!',sprintf(HIY"Đại Bổn Hùng %d/15",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5002 * rate / 100 ) ;    // NONE

/*              else if( p < 5002 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 5002 * rate / 100 )    // 武器(门派)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(2) )
                        {
                       case 0 : file = "/item/40/4051";  break;
                       case 1 : file = "/item/40/4016";  break;
