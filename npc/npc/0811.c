...
// 函数：获取人物造型
int get_char_picid() { return 4131; }

// 函数：构造处理
void create()
{
        set_name("Thị Vệ Vong Linh");

        set_char_type(FIGHTER_TYPE);    // init_level 要用到
        
        set_head_color(0xa145);

        NPC_ENERGY_D->init_level( this_object(), 75+ random(4) );    // 
        set_max_seek(4);
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_24, 23) && !who->get_legend(TASK_24, 24) && random(100) < 38)
	{
		...
		if ( (i=who->add_save_2("qgbxjiujiwan",1)) >= 20 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,18,"有备无患(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,18);
		send_user(who,"%c%s",'!',sprintf(HIY"救急丸 %d/20",i));		
	}
	if( who->get_legend(TASK_25, 19) && !who->get_legend(TASK_25, 20) )
	{
		if ( (i=who->add_save_2("hgnzgswwl1",1)) >= 20 && who->get_save_2("hgnzgtongjs",1) >= 20 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,22,"挑战自己(完成)" );
			who->set_save_2("hgnzgswwl1",20);
			who->set_legend(TASK_25, 20);
		}
		USER_TASK_D->send_task_intro(who,2,TID_HANGUO,22);
		send_user(who,"%c%s",'!',sprintf(HIY"Thị Vệ Vong Linh %d/20",i));		
	}
	if( who->get_legend(TASK_26, 14) && !who->get_legend(TASK_26, 15) && who->get_save_2("hgnzgswwl") < 30)
	{
		if ( (i=who->add_save_2("hgnzgswwl",1)) >= 30 && who->get_save_2("hgnzgeling") >= 30 )
		{
			who->set_save_2("hgnzgswwl",30);
			who->set_legend(TASK_26, 15);
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,33,"试探实力(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_HANGUO,33);
		send_user(who,"%c%s",'!',sprintf(HIY"Thị Vệ Vong Linh %d/30",i));		
	}
	if( who->get_legend(TASK_27,4) && !who->get_legend(TASK_27,5) )
	{
		if ( (i=who->add_save_2("qgswshiwwl",1)) >= 25 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,21,"Thị Vệ Vong Linh(完成)" );
			who->set_save_2("qgswshiwwl",25);
			who->set_legend(TASK_27, 5);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,21);
		send_user(who,"%c%s",'!',sprintf(HIY"Thị Vệ Vong Linh %d/25",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6104 * rate / 100 ) ;    // NONE

                else if( p < 6354 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6354 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(2) )
                        {
                       case 0 : file = "/item/40/4067";  break;
