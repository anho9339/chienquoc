...
void create()
{
        ...
        NPC_ENERGY_D->init_level( this_object(), 17+ random(3) );    // 
        set_max_seek(4);
...
{ 
	int i,p;
	object item;
	if ( who->get_legend(TASK_NEWBIE_03,15) && !who->get_legend(TASK_NEWBIE_03,16) && who->get_save_2("gongsumutou") < 20 && random(100) < 50 )
	{
		if ( objectp( item = present("木头", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save_2("gongsumutou",1)) >= 20 )
		{
			who->set_save_2("gongsumutou",20);
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHOUGUO,4,"买椟还珠(3)(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_ZHOUGUO,4);
		send_user(who,"%c%s",'!',sprintf(HIY"木头 %d/20",i));		
	}
}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 7067 * rate / 100 ) ;    // NONE

                else if( p < 9067 * rate / 100 )    // 金钱
                {
