...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_07, 16) && !who->get_legend(TASK_07, 17) && who->get_save_2("wgjcaigou") < 10)
	{
		if ( (i=who->add_save_2("wgjcaigou",1)) >= 10 && who->get_save_2("wgjzhuyao") >= 20 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,9,"魏管家的劫难（2）(完成)" );
			who->set_save_2("wgjcaigou",10);
			who->set_legend(TASK_07, 17);
		}

		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,9);
		send_user(who,"%c%s",'!',sprintf(HIY"豺狗 %d/10",i));		
	}	
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 7195 * rate / 100 ) ;    // NONE

                else if( p < 9195 * rate / 100 )    // 金钱
                {
