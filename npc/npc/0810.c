...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_25, 19) && !who->get_legend(TASK_25, 20) && who->get_save_2("hgnzgtongjs") < 20 )
	{
		if ( (i=who->add_save_2("hgnzgtongjs",1)) >= 20 && who->get_save_2("hgnzgswwl1") >= 20 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,22,"挑战自己(完成)" );
			who->set_save_2("hgnzgtongjs",20);
			who->set_legend(TASK_25, 20);
		}
		USER_TASK_D->send_task_intro(who,2,TID_HANGUO,22);
		send_user(who,"%c%s",'!',sprintf(HIY"Đồng Giáp Thi %d/20",i));		
	}
	if( who->get_legend(TASK_27, 19) && !who->get_legend(TASK_27, 20) )
	{
		if ( (i=who->add_save("qgsjtongjiasi",1)) >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,22,"灵谷洞之旅(完成)" );
			who->set_save("qgsjtongjiasi",10);
			who->set_legend(TASK_27, 20);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,22);
		send_user(who,"%c%s",'!',sprintf(HIY"Đồng Giáp Thi %d/10",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5929 * rate / 100 ) ;    // NONE

                else if( p < 6179 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6179 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                       ...
                       case 1 : file = "/item/40/4062";  break;
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                       ...
                       case 1 : file = "/item/41/4109";  break;
