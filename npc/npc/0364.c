...
// 函数：获取人物造型
int get_char_picid() { return 0152; }

// 函数：构造处理
void create()
{
        set_name("Đường Lang Yêu");
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_09, 28) && !who->get_legend(TASK_09, 29) && who->get_save_2("swtanglang") < 10)
	{
		if ( (i=who->add_save_2("swtanglang",1)) >= 10 && who->get_save_2("swheixiong") >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,5,"欺诈的天罚(完成)" );
			who->set_save_2("swtanglang",10);
			who->set_legend(TASK_09, 29);
		}

		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,5);
		send_user(who,"%c%s",'!',sprintf(HIY"螳螂妖 %d/10",i));		
	}	
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4951 * rate / 100 ) ;    // NONE

/*              else if( p < 4951 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 5093 * rate / 100 )    // 防具(门派)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(2) )
                        {
                       case 0 : file = "/item/40/4045";  break;
                       case 1 : file = "/item/40/4011";  break;
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                       ...
                       case 1 : file = "/item/stuff/0144";  break;
