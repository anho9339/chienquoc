...
void create()
{
        set_name("Võng Lượng Quỷ");

        set_char_type(FIGHTER_TYPE);    // init_level 要用到
        
        set_head_color(0x1e0);
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_25, 1) && !who->get_legend(TASK_25, 2) )
	{
		if ( (i=who->add_save_2("qgswwangliang",1)) >= 20 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,20,"决定性的打击(完成)" );
			who->set_save_2("qgswwangliang",20);
			who->set_legend(TASK_25, 2);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,20);
		send_user(who,"%c%s",'!',sprintf(HIY"魍魉鬼 %d/20",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5759 * rate / 100 ) ;    // NONE

                else if( p < 6009 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6009 * rate / 100 )    // 武器(门派)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(2) )
                        {
                       case 0 : file = "/item/40/4059";  break;
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                       ...
                       case 1 : file = "/item/32/3282";  break;
