...
// 函数：获取人物造型
int get_char_picid() { return 4141; }

// 函数：构造处理
void create()
{
        set_name("Tinh Binh Oan Linh");

        set_char_type(FIGHTER_TYPE_2);    // init_level 要用到
        
        set_head_color(0x40f2);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2364 * rate / 100 ) ;    // NONE

                else if( p < 3004 * rate / 100 )    // 金钱
                {
                        ...
                        if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
                        {
                                ...
                                item->set_value( 1800 + random(350) );
