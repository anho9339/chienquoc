...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_24, 9) && !who->get_legend(TASK_24, 10) && random(100) < 38)
	{
		if ( objectp( item = present("茉莉花", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save_2("cgbxmolihua",1)) >= 20 )
		{
			who->set_save_2("cgbxmolihua",20);
			who->set_legend(TASK_24, 10);
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,29,"茉莉花茶（2）(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_CHUGUO,29);
		send_user(who,"%c%s",'!',sprintf(HIY"茉莉花 %d/20",i));		
	}
	if( who->get_legend(TASK_24, 17) && !who->get_legend(TASK_24, 18) && random(100) < 18)
	{
		item = new("item/98/0116");
		if ( !item  )
			return ;
		p = item->move(who,-1);
		if ( !p )
		...
		item->add_to_user(p);
		who->set_legend(TASK_24, 18);
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,16,"搜寻官印(完成)" );
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,16);
		send_user(who,"%c%s",'!',sprintf(HIY"官印 %d/1",1));		
	}
	...
	if( who->get_legend(TASK_25, 11) && !who->get_legend(TASK_25, 12) && who->get_save_2("hzgdoubing") < 20 )
	{
		if ( (i=who->add_save_2("hzgdoubing",1)) >= 20 && who->get_save_2("hzgguanzy") >= 20 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,19,"证明自己(完成)" );
			who->set_save_2("hzgdoubing",20);
			who->set_legend(TASK_25, 12);
		}
		USER_TASK_D->send_task_intro(who,2,TID_HANGUO,19);
		send_user(who,"%c%s",'!',sprintf(HIY"Đậu Binh %d/20",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 8042 * rate / 100 ) ;    // NONE

                else if( p < 9242 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 9350 * rate / 100 )    // 杂物１
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                       ...
                       case 1 : file = "/item/stuff/0160";  break;
                       case 2 : file = "/item/91/9101";  break;
                       case 3 : file = "/item/stuff/0148";  break;
