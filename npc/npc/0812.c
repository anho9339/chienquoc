...
void create()
{
        set_name("Oan Hồn Tinh Linh");

        set_char_type(FIGHTER_TYPE_2);    // init_level 要用到
        
        set_head_color(0x21c1);
        
        set_walk_speed(2);
        set_attack_speed(10);

        NPC_ENERGY_D->init_level( this_object(), 81+ random(4) );    // 
        set_max_seek(4);
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 2954 * rate / 100 )    // 金钱
                {
                        ...
                        if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
                        {
                                ...
                                item->set_value( 1600 + random(320) );
