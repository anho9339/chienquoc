...
void check_legend_task( object who ) 
{
	int p,i;
	object item;
	if( who->get_legend(TASK_09, 23) && !who->get_legend(TASK_09, 24) && random(100) < 30)
	{
		if ( objectp( item = present("稻子", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
			if ( item->get_amount() < 9 )
				item->add_amount(1);
		}
		else
		{
			item = new("item/98/0039");
			if ( !item  )
				return ;
			p = item->move(who,-1);
			if ( !p )
		...
		if ( (i=who->add_save_2("zouzidaozi",1)) >= 9 )
		{
			who->set_save_2("zouzidaozi",9);
			who->set_legend(TASK_09, 24);
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,3,"唇亡齿寒（3）(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,3);
		send_user(who,"%c%s",'!',sprintf(HIY"稻子 %d/9",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 7192 * rate / 100 ) ;    // NONE

                else if( p < 9192 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 9350 * rate / 100 )    // 杂物１
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                       ...
                       case 1 : file = "/item/41/4106";  break;
                       case 2 : file = "/item/stuff/0184";  break;
