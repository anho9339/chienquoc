...
void check_legend_task( object who )
{
	int p,i;
	object item;
	if( who->get_legend(TASK_06, 25) && !who->get_legend(TASK_06, 26) )
	{
		if ( (i=who->add_save_2("zuietufei",1)) >= 15 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,2,"罪恶的土匪(完成)" );
			who->set_save_2("zuietufei",15);
			who->set_legend(TASK_06, 26);
		}

		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,2);
		send_user(who,"%c%s",'!',sprintf(HIY"土匪 %d/15",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 7212 * rate / 100 ) ;    // NONE

                else if( p < 9212 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 9350 * rate / 100 )    // 杂物１
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                        switch( random(2) )
                        {
                       case 0 : file = "/item/91/9100";  break;
                       case 1 : file = "/item/91/9110";  break;
