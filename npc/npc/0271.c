...
void create()
{
        set_name("Yên Quốc Tuần Binh");

        set_char_type(FIGHTER_TYPE);    // init_level 要用到
        
        set_head_color(0x0);

        NPC_ENERGY_D->init_level( this_object(), 27+ random(3) );    // 
        set_max_seek(4);
...
void check_legend_task( object who )
{
	int p,i;
	object item;
	if( who->get_legend(TASK_07, 1) && !who->get_legend(TASK_07, 2) && random(100) < 40)
	{
		if ( objectp( item = present("女儿红", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save_2("haopengyou",1)) >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,4,"送给丁兄的酒(完成)" );
			who->set_save_2("haopengyou",10);
			who->set_legend(TASK_07, 2);
		}

		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,4);
		send_user(who,"%c%s",'!',sprintf(HIY"女儿红 %d/10",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5040 * rate / 100 ) ;    // NONE

                else if( p < 5290 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 5290 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(2) )
                        {
                       case 0 : file = "/item/40/4031";  break;
                       case 1 : file = "/item/40/4030";  break;
