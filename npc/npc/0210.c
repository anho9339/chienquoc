...
void check_legend_task( object who )
{
	int p,i;
	object item;
	if( who->get_legend(TASK_07, 6) && !who->get_legend(TASK_07, 7) && random(100) < 40)
	{
		if ( objectp( item = present("山芍药", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save_2("sansaoyao",1)) >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,6,"换盘缠的芍药(完成)" );
			who->set_save_2("sansaoyao",10);
			who->set_legend(TASK_07, 7);
		}

		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,6);
		send_user(who,"%c%s",'!',sprintf(HIY"山芍药 %d/10",i));		
	}
...
{
				if(who->add_save_2("soquaicandiet1.score")>=3)
				{
								send_user(who,"%c%s",'!',sprintf("Nhiệm vụ Sứ giả đã hoàn thành"));
								return 0;
				}
				who->add_save_2("soquaicandiet1.score",1);
				send_user(who,"%c%s",'!',sprintf("Đã giết được %d/3",who->get_save_2("soquaicandiet1.score")));
}*/
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4650 * rate / 100 ) ;    // NONE

/*              else if( p < 4650 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 4650 * rate / 100 )    // 武器(门派)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/40/4023";  break;
