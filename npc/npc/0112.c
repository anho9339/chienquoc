...
void create()
{
        ...
        set_head_color(0x5228);

        NPC_ENERGY_D->init_level( this_object(), 11+ random(3) );    // 
        set_max_seek(4);
...
{ 
	if( who->get_legend(TASK_NEWBIE_03,9)  && !who->get_legend(TASK_NEWBIE_03,10) && !who->get_save_2("bandiangou") )
	{
		send_user( who, "%c%s", '!', HIY"Chó Đốm 1/1");
		who->set_save_2("bandiangou",1);
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,5,"斑点狗的血液(完成)" );
		USER_TASK_D->send_task_intro(who,2,TID_CHUGUO,5);		
	}	
/*	if( who->get_legend(TASK_23,11)  && !who->get_legend(TASK_23,12) )
	{
		who->set_legend(TASK_23,12);
		send_user( who, "%c%s", '!', HIY"Chó Đốm 1/1");
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_MAOSHAN,3,"游魂（4）(完成)" );
	}*/
}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4018 * rate / 100 ) ;    // NONE

/*              else if( p < 4018 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 4360 * rate / 100 )    // 防具(门派)
                ...
                else if( p < 9900 * rate / 100 )    // 杂物２
                {
                       ...
                       case 1 : file = "/item/stuff/0185";  break;
