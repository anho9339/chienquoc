...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_31, 13) && !who->get_legend(TASK_31, 14) && who->get_save("wwjianyong") < 12 )
	{
		if ( (i=who->add_save("wwjianyong",1)) >= 12 && who->get_save("wwqianyong") >= 9)
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,35,"通缉犯(完成)" );
			who->set_legend(TASK_31, 14);
		}
		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,35);
		send_user(who,"%c%s",'!',sprintf(HIY"Kiếm Dũng %d/12",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6580 * rate / 100 ) ;    // NONE

/*              else if( p < 6580 * rate / 100 )    // 金钱
                {
