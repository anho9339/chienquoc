...
void create()
{
        set_name("Đốc Công Kệnh Trịnh Quốc");
        ...
        NPC_ENERGY_D->init_level( this_object(), 51+ random(4) );    // 
        set_max_seek(4);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2660 * rate / 100 ) ;    // NONE

                else if( p < 3300 * rate / 100 )    // 金钱
                {
