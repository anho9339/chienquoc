...
// 函数：获取人物造型
int get_char_picid() { return 0953; }
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_33, 22) && !who->get_legend(TASK_33, 23) && who->get_save("cggjhuoj") < 8 )
	{
		if ( (i=who->add_save("cggjhuoj",1)) >= 8 && who->get_save("cggjsuij") >= 10)
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,40,"源起于食物(完成)" );
			who->set_legend(TASK_33, 23);
		}
		USER_TASK_D->send_task_intro(who,2,TID_CHUGUO,40);
		send_user(who,"%c%s",'!',sprintf(HIY"Hoả Tinh %d/8",i));		
	}
	if( who->get_legend(TASK_34, 11) && !who->get_legend(TASK_34, 12) && who->get_save("wlhhuojing") < 30 )
	{
		if ( (i=who->add_save("wlhhuojing",1)) >= 30 && who->get_save("wlhsuijing") >= 30)
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,43,"证实自己(完成)" );
			who->set_legend(TASK_34, 12);
		}
		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,43);
		send_user(who,"%c%s",'!',sprintf(HIY"Hoả Tinh %d/30",i));		
	}
	if( who->get_legend(TASK_34, 25) && !who->get_legend(TASK_34, 26) && who->get_save("ssyhuojing") < 30 )
	{
		if ( (i=who->add_save("ssyhuojing",1)) >= 30 && who->get_save("ssyjinjing") >= 30)
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,49,"姻缘之路3(完成)" );
			who->set_legend(TASK_34, 26);
		}
		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,49);
		send_user(who,"%c%s",'!',sprintf(HIY"Hoả Tinh %d/30",i));		
	}
	if( who->get_legend(TASK_35, 7) && !who->get_legend(TASK_35, 8) && who->get_save("zthuojing") < 30 )
	{
		if ( (i=who->add_save("zthuojing",1)) >= 30 && who->get_save("ztsuijing") >= 30)
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,54,"试练身手(完成)" );
			who->set_legend(TASK_35, 8);
		}
		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,54);
		send_user(who,"%c%s",'!',sprintf(HIY"Hoả Tinh %d/30",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6929 * rate / 100 ) ;    // NONE

/*              else if( p < 6929 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6929 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                        switch( random(5) )
                        {
                       case 0 : file = "/item/stuff/0029";  break;
                       case 1 : file = "/item/stuff/0130";  break;
                       case 2 : file = "/item/stuff/0106";  break;
                       case 3 : file = "/item/stuff/0053";  break;
                       case 4 : file = "/item/stuff/0195";  break;
...
/*              else if( p < 1000000 * rate / 100 )    // 杂物５
                {
                        switch( random(0) )
                        {
