...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_32, 11) && !who->get_legend(TASK_32, 12) && who->get_save("ygswyuanling") < 6 )
	{
		if ( (i=who->add_save("ygswyuanling",1)) >= 6 && who->get_save("ygswsimo") >= 8)
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,44,"净化灵魂(完成)" );
			who->set_legend(TASK_32, 12);
		}
		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,44);
		send_user(who,"%c%s",'!',sprintf(HIY"Oán Linh %d/6",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6520 * rate / 100 ) ;    // NONE

                else if( p < 6770 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6770 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                       ...
                       case 2 : file = "/item/40/4071";  break;
