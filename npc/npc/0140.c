...
void create()
{
        ...
        set_head_color(0x329c);
        
        set_walk_speed(2);
        set_attack_speed(10);

        NPC_ENERGY_D->init_level( this_object(), 6+ random(4) );    // 
        set_max_seek(4);
...
/*              else if( p < 4440 * rate / 100 )    // 防具(门派)
                ...
                else if( p < 10000 * rate / 100 )    // 杂物２
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/91/9110";  break;
