...
// 函数：获取人物造型
int get_char_picid() { return 1100; }
...
void check_legend_task( object who ) 
{
	...
	if( who->get_legend(TASK_44, 13) && !who->get_legend(TASK_44, 14) && who->get_save("qgjdaohun") < 8 )
	{
		if ( (i=who->add_save("qgjdaohun",1)) >= 8 && who->get_save("qgjjianhun") >= 8 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,64,"热心的建议（1）(完成)" );
			who->set_legend(TASK_44, 14);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,64);
		send_user(who,"%c%s",'!',sprintf(HIY"Đao Hồn %d /8",i));		
	}	
	if( who->get_legend(TASK_44, 21) && !who->get_legend(TASK_44, 22) && who->get_save("qzgdaohun") < 4 )
	{
		if ( (i=who->add_save("qzgdaohun",1)) >= 4 && who->get_save("qzgdismzs") >= 8 && who->get_save("qzgjinj") >= 10 && who->get_save("qzgtaijmzs") >= 8 )
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Đao Hồn %d /4",i));		
	}
	...
	if( who->get_legend(TASK_45, 18) && !who->get_legend(TASK_45, 19) && random(100) < 18)
	{
		item = new("item/98/0225");
		if ( !item  )
			return ;
		p = item->move(who,-1);
		if ( !p )
		...
		item->add_to_user(p);
		who->set_legend(TASK_45, 19);
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,60,"求医（1）(完成)" );
		USER_TASK_D->send_task_intro(who,2,TID_ZHAOGUO,60);
		send_user(who,"%c%s",'!',HIY"如意玉牌 1/1");		
	}
	...
	if( who->get_legend(TASK_46, 8) && !who->get_legend(TASK_46, 9) && who->get_save("mxdaohun") < 30 )
	{
		if ( (i=who->add_save("mxdaohun",1)) >= 30 && who->get_save("mxqianhun") >= 30 )
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Đao Hồn %d /30",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 7322 * rate / 100 ) ;    // NONE

/*              else if( p < 7322 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 7322 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/40/4080";  break;
...
/*              else if( p < 1090000 * rate / 100 )    // 杂物５
                {
                        switch( random(0) )
                        {
