...
void create()
{
        set_name("Khỉ Đầu Chó");

        set_char_type(FIGHTER_TYPE);    // init_level 要用到
        
        set_head_color(0x0);

        NPC_ENERGY_D->init_level( this_object(), 12+ random(3) );    // 
        set_max_seek(4);
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4426 * rate / 100 ) ;    // NONE

/*              else if( p < 4426 * rate / 100 )    // 金钱
                {
