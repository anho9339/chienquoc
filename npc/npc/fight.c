...
int set_hp() 
{
	...
	if (get_z(me)>0)
	{
		...
		if (who)
		{
			me->set_name(who->get_name()+"Đích đạo thảo nhân");
		}
	}
	return 1;
}
...
// 函数：是否允许移动
   int is_moveable() { return 0; }    // 禁止Sử dụng go 命令

// 函数：Căn tuỳ đối thủ(Tâm khiêu chi trung điều dụng)
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 3000 * rate / 100 ) ;    // NONE

/*              else if( p < 3000 * rate / 100 )    // kim钱
                {
...
/*              else if( p < 3000 * rate / 100 )    // Vũ khí(基本)
                {
                ...
                else if( p < 3500 * rate / 100 )    // 防具(基本)
                {
                        if( equip >= total2 ) continue;

                        file = ARMOR_FILE->get_armor_file_0(1);
                        if( load_object(file) && ( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) ) )
                        {
                                item = new(file);
...
/*              else if( p < 3500 * rate / 100 )    // Vũ khí(门派)
                {
                        if( equip >= total2 ) continue;
...
/*              else if( p < 3500 * rate / 100 )    // 防具(门派)
                ...
                else if( p < 10000 * rate / 100 )    // 杂物２
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/41/4102";  break;
...
int accept_money(object me, int gold){return 1;}

int accept_object(object me, object item){return 1;}