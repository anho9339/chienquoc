...
// 函数：获取人物造型
int get_char_picid() { return 0100; }

// 函数：构造处理
void create()
{
        set_name("Tinh Tinh Vương");

        set_char_type(FIGHTER_TYPE_2);    // init_level 要用到
        
        set_head_color(0x1a6);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2578 * rate / 100 ) ;    // NONE

/*              else if( p < 2578 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 4360 * rate / 100 )    // 防具(门派)
                ...
                else if( p < 9400 * rate / 100 )    // 杂物１
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/40/4017";  break;
