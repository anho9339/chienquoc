...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_10, 23) && !who->get_legend(TASK_10, 24) && who->get_save_2("plkuyi") < 30)
	{
		if ( (i=who->add_save_2("plkuyi",1)) >= 30 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,2,"叛乱的苦役(完成)" );
			who->set_save_2("plkuyi",30);
			who->set_legend(TASK_10, 24);
		}

		USER_TASK_D->send_task_intro(who,2,TID_QINGUO,2);
		send_user(who,"%c%s",'!',sprintf(HIY"Khổ Dịch %d/30",i));		
	}
	if( who->get_legend(TASK_40, 10) && !who->get_legend(TASK_40, 11) && random(100) < 12)
	{
		if ( objectp( item = present("Rương Chứa Sừng Trâu Vàng", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save("lysjinsnj",1)) >= 3 )
		{
			who->set_save("lysjinsnj",3);
			who->set_legend(TASK_40, 11);
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHOUGUO,12,"Sừng Trâu Vàng (Hoàn thành)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_ZHOUGUO,12);
		send_user(who,"%c%s",'!',sprintf(HIY"Sừng Trâu Vàng %d /3",i));		
	}	
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5297 * rate / 100 ) ;    // NONE

                else if( p < 5547 * rate / 100 )    // 金钱
                {
