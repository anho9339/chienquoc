...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_35, 27) && !who->get_legend(TASK_35, 28) && who->get_save("qgsrguanzx") < 6 )
	{
		if ( (i=who->add_save("qgsrguanzx",1)) >= 6 && who->get_save("qgsrguanzy") >= 18 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,50,"罐中妖与Quán Trung Tiên(完成)" );
			who->set_legend(TASK_35, 28);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,50);
		send_user(who,"%c%s",'!',sprintf(HIY"Quán Trung Tiên %d/6",i));		
	}
	if( who->get_legend(TASK_36, 11) && !who->get_legend(TASK_36, 12) && who->get_save("qgdjjguanzx") < 8 )
	{
		if ( (i=who->add_save("qgdjjguanzx",1)) >= 8 && who->get_save("qgdjjguixr") >= 11 )
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Quán Trung Tiên %d/8",i));		
	}
	...
	if( who->get_legend(TASK_36, 23) && !who->get_legend(TASK_36, 24) && who->get_save("qgjguanzx") < 10 )
	{
		if ( (i=who->add_save("qgjguanzx",1)) >= 10  )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,57,"叛徒(完成)" );
			who->set_legend(TASK_36, 24);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,57);
		send_user(who,"%c%s",'!',sprintf(HIY"Quán Trung Tiên %d/10",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6762 * rate / 100 ) ;    // NONE

                else if( p < 7012 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 7012 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/40/4076";  break;
...
/*              else if( p < 1020000 * rate / 100 )    // 杂物５
                {
                        switch( random(0) )
                        {
