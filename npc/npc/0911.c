...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_32, 1) && !who->get_legend(TASK_32, 2) && random(100) < 20)
	{
		if ( objectp( item = present("单刀", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save("ygswdandao",1)) >= 8 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,43,"单刀(完成)" );
			who->set_legend(TASK_32, 2);
		}

		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,43);
		send_user(who,"%c%s",'!',sprintf(HIY"单刀 %d/8",i));		
	}
	...
	if( who->get_legend(TASK_32, 17) && !who->get_legend(TASK_32, 18) && who->get_save("ygswhaiguj") < 8 )
	{
		if ( (i=who->add_save("ygswhaiguj",1)) >= 8 && who->get_save("ygswkulouy") >= 8)
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Hài Cốt Tinh %d/8",i));		
	}
	if( who->get_legend(TASK_32, 23) && !who->get_legend(TASK_32, 24) && who->get_save("ywhaiguj") < 5 )
	{
		if ( (i=who->add_save("ywhaiguj",1)) >= 5 && who->get_save("ywkulouy") >= 5 && who->get_save("ywxieling") >= 5)
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Hài Cốt Tinh %d/5",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6409 * rate / 100 ) ;    // NONE

                else if( p < 6659 * rate / 100 )    // 金钱
                {
