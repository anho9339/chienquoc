...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_27, 7) && !who->get_legend(TASK_27, 8) && who->get_save_2("zgdjjjiandkl") < 10)
	{
		if ( (i=who->add_save_2("zgdjjjiandkl",1)) >= 10 && who->get_save_2("zgdjjdaodkl") >= 20 )
		{
			who->set_save_2("zgdjjjiandkl",10);
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Kiếm Thuẫn Khô Lâu %d/10",i));		
	}
	...
	if( who->get_legend(TASK_28, 26) && !who->get_legend(TASK_28, 27) && who->get_save("zgdjjjiandunkl") < 10)
	{
		if ( (i=who->add_save("zgdjjjiandunkl",1)) >= 10 && who->get_save("zgdjjdaodunkl") >= 10 && who->get_save("zgdjjsuangdaokl") >= 8)
		{
			who->set_save("zgdjjjiandunkl",10);
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 6038 * rate / 100 ) ;    // NONE

                else if( p < 6288 * rate / 100 )    // 金钱
                {
