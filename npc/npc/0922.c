...
void create()
{
        set_name("Thủ Mộ Thạch Hổ");

        set_char_type(FIGHTER_TYPE_2);    // init_level 要用到
        
        set_head_color(0x4a27);
        
        set_walk_speed(2);
        set_attack_speed(10);

        NPC_ENERGY_D->init_level( this_object(), 92+ random(4) );    // 
        set_max_seek(4);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2382 * rate / 100 ) ;    // NONE

/*              else if( p < 2382 * rate / 100 )    // 金钱
                {
