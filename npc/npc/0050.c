...
void create()
{
        set_name("Bù Nhìn Tinh");

        set_char_type(FIGHTER_TYPE);    // init_level 要用到
        
        set_head_color(0x39f4);

        NPC_ENERGY_D->init_level( this_object(), 5+ random(2) );    // 
        set_max_seek(4);
...
void check_legend_task( object who )
{
	...
	if ( who->get_legend(TASK_49, 24) && !who->get_legend(TASK_49, 25) && who->get_save("xiuli_cscr")==0 )
	{
		i = who->set_save("xiuli_cscr",1);
		if ( i && who->get_save("xiuli_chy") && who->get_save("xiuli_zimuwa") )
		{
			who->set_legend(TASK_49, 25);
	                send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,48,"行路难(完成)" );
		}
		send_user(who,"%c%s",'!',sprintf("Bù Nhìn Tinh %d/1",i));
	...
	if ( who->get_legend(TASK_49,6) && !who->get_legend(TASK_49,7) && random(100)<65 )
	{
		...
		{		
			if ( (i=who->add_save_2("huoyanshi",1)) >= 5 )
			{
				...
				send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,116,"当铺老板的交易(已经完成)" );
				send_user(who,"%c%s",';',"火焰石已经收集好,请找新手村的门派向导完成任务");
			}
	...
	if ( who->get_legend(TASK_48, 28) && !who->get_legend(TASK_48, 29) && random(100) < 50 )
	{
		...
		{		
			if ( (i=who->add_save("wdzbaihh",1)) >= 3 )
			{
				...
				send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,112,"装饰房屋(完成)" );
			}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4921 * rate / 100 ) ;    // NONE

/*              else if( p < 4921 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 5263 * rate / 100 )    // 防具(门派)
                ...
                else if( p < 9900 * rate / 100 )    // 杂物１
                {
                       ...
                       case 1 : file = "/item/40/4010";  break;
