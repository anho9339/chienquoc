...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_16, 11) && !who->get_legend(TASK_16, 12) && who->get_save_2("ygqianbing") < 20)
	{
		if ( (i=who->add_save_2("ygqianbing",1)) >= 20 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,36,"逍遥法外(Hoàn thành)" );
			who->set_save_2("ygqianbing",20);
			who->set_legend(TASK_16, 12);
		}

		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,36);
		send_user(who,"%c%s",'!',sprintf(HIY"Yên Quốc Thương Binh %d/20",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5390 * rate / 100 ) ;    // NONE

                else if( p < 5640 * rate / 100 )    // 金钱
                {
