...
// 函数：获取人物造型
int get_char_picid() { return 0102; }
...
{
	int i;
	if( who->get_legend(TASK_NEWBIE_03,5) && !who->get_legend(TASK_NEWBIE_03,6) && who->get_save_2("sijiaose") < 18  )
	{
		i = who->add_save_2("sijiaose",1);
		send_user( who, "%c%s", '!', sprintf(HIY"消灭四脚蛇 %d/18",i));
		if ( i >= 18 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,3,"消灭四脚蛇(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_CHUGUO,3);
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4418 * rate / 100 ) ;    // NONE

/*              else if( p < 4418 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 4760 * rate / 100 )    // 防具(门派)
                ...
                else if( p < 9400 * rate / 100 )    // 杂物１
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/40/4019";  break;
                ...
                else if( p < 9900 * rate / 100 )    // 杂物２
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/32/3282";  break;
