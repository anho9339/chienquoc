...
// 函数：获取人物造型
int get_char_picid() { return 1601; }
...
void create()
{
        ...
        NPC_ENERGY_D->init_level( this_object(), 118+ random(4) );    // 
        set_max_seek(6);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2494 * rate / 100 ) ;    // NONE

/*              else if( p < 2494 * rate / 100 )    // 金钱
                {
                        ...
                        if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
                        {
                                ...
                                item->set_value( 3200 + random(300) );
