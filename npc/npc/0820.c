...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_27, 22) && !who->get_legend(TASK_27, 23) )
	{
		if ( (i=who->add_save("qgswtiejiasi",1)) >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,23,"悔恨的守卫(完成)" );
			who->set_save("qgswtiejiasi",10);
			who->set_legend(TASK_27, 23);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,23);
		send_user(who,"%c%s",'!',sprintf(HIY"Thiết Giáp Thi %d/10",i));		
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5901 * rate / 100 ) ;    // NONE

                else if( p < 6151 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 6151 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                       ...
                       case 1 : file = "/item/40/4061";  break;
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                       ...
                       case 2 : file = "/item/stuff/0192";  break;
