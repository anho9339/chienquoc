...
void create()
{
        set_name("Độc Nhãn Thú");

        set_char_type(FIGHTER_TYPE);    // init_level 要用到
        
        set_head_color(0x28e3);

        NPC_ENERGY_D->init_level( this_object(), 16+ random(3) );    // 
        set_max_seek(4);
...
void check_legend_task( object who ) 
{
	...
	{
		if ( (i=who->add_save_2("nzdujiaosou",1)) >= 20 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,5,"宫廷的喧闹(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_HANGUO,5);
		send_user(who,"%c%s",'!',sprintf(HIY"Độc Nhãn Thú %d/20",i));
	} 
	if ( who->get_legend(TASK_NEWBIE_03,20) && !who->get_legend(TASK_NEWBIE_03,21) && who->get_save_2("souweisoujiao") < 20 && random(100) < 50 )
	{
		if ( objectp( item = present("兽角", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save_2("souweisoujiao",1)) >= 20 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,10,"孝顺的守卫(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_CHUGUO,10);
		send_user(who,"%c%s",'!',sprintf(HIY"兽角 %d/20",i));		
	}
}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4436 * rate / 100 ) ;    // NONE

/*              else if( p < 4436 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 4760 * rate / 100 )    // 防具(门派)
                ...
                else if( p < 9400 * rate / 100 )    // 杂物１
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/40/4018";  break;
