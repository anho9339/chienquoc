...
void create()
{
        ...
        NPC_ENERGY_D->init_level( this_object(), 6+ random(2) );    // 
        set_max_seek(4);
...
void check_legend_task( object who ) 
{
	...
	if ( who->get_legend(TASK_49, 24) && !who->get_legend(TASK_49, 25) && who->get_save("xiuli_chy")==0 )
	{
		i = who->set_save("xiuli_chy",1);
		if ( i && who->get_save("xiuli_cscr") && who->get_save("xiuli_zimuwa") )
		{
			who->set_legend(TASK_49, 25);
	                send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,48,"行路难(完成)" );
		}
		send_user(who,"%c%s",'!',sprintf("Kiến Đen %d/1",i));
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4918 * rate / 100 ) ;    // NONE

/*              else if( p < 4918 * rate / 100 )    // 金钱
                {
