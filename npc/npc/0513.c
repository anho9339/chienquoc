...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_11, 24) && !who->get_legend(TASK_11, 25) && who->get_save_2("huabaojing") < 10)
	{
		if ( (i=who->add_save_2("huabaojing",1)) >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,18,"挑战(Hoàn thành)" );
			who->set_save_2("huabaojing",10);
			who->set_legend(TASK_11, 25);
		}

		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,18);
		send_user(who,"%c%s",'!',sprintf(HIY"Hoa Báo Tinh %d/10",i));		
	}
	...
	if( who->get_legend(TASK_24, 12) && !who->get_legend(TASK_24, 13) && random(100) < 18)
	{
		...
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,30,"茉莉花茶（3）(Hoàn thành)" );
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 7424 * rate / 100 ) ;    // NONE

                else if( p < 9224 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 9350 * rate / 100 )    // 杂物１
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                       ...
                       case 1 : file = "/item/91/9101";  break;
