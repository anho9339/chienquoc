...
void create()
{
        ...
        NPC_ENERGY_D->init_level( this_object(), 46+ random(4) );    // 
        set_max_seek(6);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2202 * rate / 100 ) ;    // NONE

                else if( p < 2842 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 2842 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9996 * rate / 100 )    // 杂物２
                {
                       ...
                       case 4 : file = "/item/91/9135";  break;
