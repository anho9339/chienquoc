...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_22, 27) && !who->get_legend(TASK_22, 28) )
	{
		if ( (i=who->add_save_2("hgbxsuigui",1)) >= 10 )
		{
			who->set_legend(TASK_22, 28);
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,43,"害人的水鬼(完成)" );
		}
		send_user(who,"%c%s",'!',sprintf(HIY"Thủy Quỷ %d /10",i));	
		USER_TASK_D->send_task_intro(who,2,TID_HANGUO,43);
	}	
	i = random(100);
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 5652 * rate / 100 ) ;    // NONE

                else if( p < 5902 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 5902 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9350 * rate / 100 )    // 杂物１
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/40/4057";  break;
