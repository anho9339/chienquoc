...
// 函数：获取人物造型
int get_char_picid() { return 8011; }

// 函数：构造处理
void create()
{
        set_name("Thủ Lĩnh Thích Khách");
        ...
        NPC_ENERGY_D->init_level( this_object(), 76+ random(4) );    // 
        set_max_seek(6);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2312 * rate / 100 ) ;    // NONE

                else if( p < 7752 * rate / 100 )    // 金钱
                {
                        ...
                        if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
                        {
                                ...
                                item->set_value( 1500 + random(260) );
