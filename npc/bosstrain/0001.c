...
void create()
{
	object me = this_object();
        set_name("Khuyển Thái Lang Tinh Vuong Tinh Vuong");
        set_char_picid(1230); 
        set_level(90);
        set_max_hp(20000);
        set_max_mp(500);
	...
	set_skill(0423,40);
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"Nghe nói Khuyển Thái Lang Tinh Vuong lại đi lại ở vùng khe núi Tân Thủ Thôn đợi khiêu chiến với dũng sĩ." ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	log_file("boss.txt",short_time()+" Khuyển Thái Lang Tinh Vuong xuất hiện\n");
}
...
void win_bonus_callout( object me, object who )
{
	...
	exp = 15000;
        for( i = 0; i < size; i ++ ) if( team[i] )
        {
				...
				"sys/sys/test_db"->add_yuanbao(team[i],60);
