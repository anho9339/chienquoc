...
void create()
{
	object me = this_object();
        set_name("Tinh Vương Khuyển Thái Lang");
        set_char_picid(1230); 
        set_level(80);
        set_max_hp(300000);
        set_max_mp(50000);
	...
	set_skill(0423,40);
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"Nghe nói Tinh Vương Khuyển Thái Lang lại đi lại ở vùng khe núi Tân Thủ Thôn đợi khiêu chiến với dũng sĩ." ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	log_file("boss.txt",short_time()+" Tinh Vương Khuyển Thái Lang xuất hiện\n");
}
...
void win_bonus_callout( object me, object who )
{
	...
	log_file("boss.txt",short_time()+" Khuyển Thái Lang Tinh Vuong bị tiêu diệt\n");

        if( !objectp( who = me->get_enemy_4() ) ) return;

        if( owner = who->get_owner() ) who = owner;
        if( who->is_npc() ) return;
	CHAT_D->rumor_channel( 0, CHAT+"Khuyển Thái Lang Tinh Vuong đã bị "+who->get_name()+" tiêu diệt!" ); 
	...
	exp = 15000;
        for( i = 0; i < size; i ++ ) if( team[i] )
        {
				...
				"sys/sys/test_db"->add_yuanbao(team[i],20);
				team[i]->set_fam_contribute(me->get_fam_contribute()+60);
		...
		for(i=0;i<2;i++)
	         {
	                if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
	                {
	                        item = new( "/item/test2/tuitukim8x" );
	                  //      item->set_value( 10000 );
	                        TEAM_D->drop_item(item,who);
				item->set_user_id(owner);
	                        item->add_to_scene(z, p / 1000, p % 1000);
...
{
	send_user( who, "%c%c%w%s", ':',3,0352,"Khuyển Thái Lang Tinh Vuong:\n    怎么你们老爱找我麻烦！？\n"+
