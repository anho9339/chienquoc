...
int get_hair_color() { return 0x00E0FFFF; }
...
string get_short() 
{ 
	return get_name() + "||||" + "Sư Tử Cuồng Nộ";
}
...
//                team[i]->add_exp(exp2);
				...
				team[i]->add_title("Y205");
				team[i]->show_title("Y205");
...
void drop_items( object me, object who )
{
	...
	z = get_z(me);  x = get_x(me);  y = get_y(me);
        id = who->get_leader();
			USER_D->user_channel(sprintf(HIC"Nghe nói Bằng hữu "HIY "%s"  HIC" đã tiêu diệt "HIY "Vệ Thần "HIC"...!", who->get_name() ));
