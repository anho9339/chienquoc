...
string get_short() 
{ 
	return get_name() + "||||" + "Thiên Y Cái Thế";
}
...
//                team[i]->add_exp(exp2);
				...
				team[i]->add_title("Y208");
				team[i]->show_title("Y208");
...
void drop_items( object me, object who )
{
	...
	z = get_z(me);  x = get_x(me);  y = get_y(me);
        id = who->get_leader();
			USER_D->user_channel(sprintf(HIC"Nghe nói Bằng hữu "HIY "%s"  HIC" đã tiêu diệt "HIY "Vương Thần "HIC"...!", who->get_name() ));
