...
//        set("no_sk04237",1);	//玄龙决对其没有效果
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+" nghe nói Phi Tặc Vương lại xuất hiện tại Địa Ngục Ma Quật Tầng 4" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	...
	{		
		if ( gone_time(saytime) >=60 )
		{
			...
			if ( random(2) )
			{
				...
				{
					case 0:
						SAY_CMD->say(me, "Ngươi nghĩ lấy lại được báu vật à? Hạ ta trước đi!"); 
						break;
					case 1:
						SAY_CMD->say(me, "Haha, loài người thật nhỏ bé!"); 
						break;
					default:
						SAY_CMD->say(me, enemy->get_name()+", mau tiếp chiêu!!!"); 
						break;
				}
			}
		}
...
int perform_action( object who ) 
{
	...
	if ( lowhp == 0 && get_hp() * 100 / get_max_hp() < 40 )	//生命值为40％以下
	{
		...
		if ( summon == 0 )
			summon_guard();
		set_lowhp_status();
		SAY_CMD->say(me, "Loài người ngu xuấn! Nhất định các ngươi phải chịu chết vì dám chọc giận ta!"); 	
	}
...
{
	send_user( who, "%c%c%w%s", ':',3,0202,"Phi Tặc Vương:\n    Loài người ngu xuẩn! Dám theo ta đến tận đây! Xem ra các ngươi chán sống rồi!\n"+
