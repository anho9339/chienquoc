...
//        set("no_sk04237",1);	//玄龙决对其没有效果
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	//CHAT_D->rumor_channel( 0, CHAT+"Nghe nói Thiên Sơn Đại Tướng Quân lên cơn say muốn khiêu chiến các vị anh hùng Chiến Quốc Pro hắn xuất hiện ở gần Chu Quốc, hãy mau thu phục hắn!" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	log_file("boss.txt",short_time()+" Thiên Sơn Tướng Quân xuất hiện\n");
}
	...
	{		
		if ( gone_time(saytime) >=60 )
		{
			...
			if ( random(2) )
			{
				...
				{
					case 0:
						SAY_CMD->say(me, "Ta sẽ cho các ngươi biết, thế nào là lễ độ!"); 
						break;
					case 1:
						SAY_CMD->say(me, "Ngươi chưa phải đối thủ của ta đâu. Ha ha ha!"); 
						break;
					case 2:
						SAY_CMD->say(me, enemy->get_name()+", thần binh mau mau ra giúp ta!!"); 
						break;
					default:
						SAY_CMD->say(me, "Thân thể ta là kim thiên bất bại!!"); 
						break;
				}
			}
		}
...
int perform_action( object who ) 
{
	...
	if ( lowhp == 0 && get_hp() * 100 / get_max_hp() < 40 )	//生命值为40％以下
	{
		...
		if ( summon == 0 )
			summon_guard();
		set_lowhp_status();
		SAY_CMD->say(me, "Thiên hạ là của ta!"); 
	}
...
void win_bonus_callout( object me, object who )
{
	...
	CHAT_D->rumor_channel( 0, CHAT+"Nghe nói Thiên Sơn Đại Tướng Quân đã bị các anh hùng CQ Tâm Diện tiêu diệt" ); 
	log_file("boss.txt",short_time()+" Thiên Sơn Đại Tướng Quân bị tiêu diệt\n");
	...
	"sys/sys/test_db"->add_yuanbao(team[i],200/size);
	}
}
...
void drop_items( object me, object who )
{
	...
	if ( rate < rate1 )
	{
		...
		{
			nTmp = WEAPON_FILE->get_family_equip(nFamily[random(7)],0,random(2)?90:80,HEAD_TYPE+random(6));
			if ( sizeof(nTmp) )
			{	
		...
		{
			item = new( "/item/test/TuiQuaVoSong" );
			if ( !item )
				return;
			TEAM_D->drop_item(item,who);
			item->set_user_id(owner);
	...
	{	     
	                if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
	                {
	                        item = new( "/item/nangcap/canhpp/tuilenhtt20" );
	                        TEAM_D->drop_item(item,who);
				item->set_user_id(owner);
	                        item->add_to_scene(z, p / 1000, p % 1000);
	                        item->set("winner", who);
	...
	else if ( rate < 9500 )		
	{
	                if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
	                {
	                        item = new( "/item/nangcap/canhpp/tuingoctiende20" );
	                        TEAM_D->drop_item(item,who);
				item->set_user_id(owner);
	                        item->add_to_scene(z, p / 1000, p % 1000);
	                        item->set("winner", who);
	...
	else if ( rate < 8000)		
	{
	                ...
	                if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
	                {
	                        item = new( "/item/nangcap/canhpp/tuicanhtt20" );
	                        TEAM_D->drop_item(item,who);
				item->set_user_id(owner);
	                        item->add_to_scene(z, p / 1000, p % 1000);
	                        item->set("winner", who);
...
{
	send_user( who, "%c%c%w%s", ':',3,5201,"Thiên Sơn Tướng Quân:\n    Ta chưa say đâu, các người cứ kéo vào hết 1 lượt đi. Ha ha ha....!!\n"+
