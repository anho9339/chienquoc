...
void create()
{
	object me = this_object();
        set_name("Nguyễn Hải Dương");
   //     set_char_picid(0101); 
       set_char_picid(1221); 
        set_level(60);
        set_max_hp(10000000);
        set_max_mp(2000000);
		  set_head_color(0x008B0000);
	init_fight_status();
        set_walk_speed(2);
        set_attack_speed(10);
        set_walk_step(2);
...
//        set("no_sk04237",1);	//玄龙决对其没有效果
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"Nghe nói Nguyễn Hải Dương đã xuất hiện tại Trường Bạch Sơn. Các vị hãy mau đi thu phục nó!" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	...
	{		
		if ( gone_time(saytime) >=60 )
		{
			...
			if ( random(2) )
			{
				...
				{
					case 0:
						SAY_CMD->say(me, "Một mình cân cả nhà là ta!"); 
						break;
					case 1:
						SAY_CMD->say(me, "Thù hận, tình yêu, giết hết!!!"); 
						break;
				/*	case 2:
						SAY_CMD->say(me, enemy->get_name()+", thần binh mau mau ra giúp ta!!"); 
						break; */
					default:
						SAY_CMD->say(me, "Đừng cầu xin ta!! Giết, giết hết!!"); 
						break;
				}
			}
		}
...
void win_bonus_callout( object me, object who )
{
	...
	CHAT_D->rumor_channel( 0, CHAT+"Nghe nói Nguyễn Hải Dương đã bị tiêu diệt" ); 
...
{
	send_user( who, "%c%c%w%s", ':',3,0101,"Nguyễn Hải Dương:\n    Mấy cưng biết anh là ai rồi chứ? Bình Phước đấy! Nhớ không?!!!\n"+
