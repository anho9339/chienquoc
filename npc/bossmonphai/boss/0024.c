...
void heart_beat_walk()
{
	...
	{		
		...
		if ( gone_time(saytime) >=60 )
		{
			...
			if(random(100) < 40)
			{
				SAY_CMD->say(me, "Người nào đấy, dám cả gan vào hoàng lăng"); 
			}
		}
	}
	else if ( RelaxTime == 0 )
...
int perform_action( object who ) 
{
	...
	if ( gone_time(pTime) > 15 )
	{
		...
		{
			SAY_CMD->say(me, "Hãy đỡ ngàn dặm ngựa phi");
	...
	{
		pTime1 = time();
		SAY_CMD->say(me, "Thời gian chết của ngươi đây, hãy xem Trực Đảo Hoàng Long");
...
void stop_fight()
{
	...
	{
		SAY_CMD->say(this_object(), "Haha, yếu đuối mà vào Hoàng Lăng à!!!");
		add_hp(2000);
	}
	::stop_fight();	
}
