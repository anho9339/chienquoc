...
void create()
{
        set_name( "Ác Phách" + NPC_NAME_D->get_thief_name() );
...
void win_bonus_callout( object me, object who )
{
	...
	if ( !objectp(user) || !inside_screen_2(me, user) || user->is_die() )
	{
		...
		destruct(me);
		send_user(who,"%c%s",'!',"Nhiệm vụ vợ chồng thất bại！");
		return;	
	}
	...
	send_user( who, "%c%c%c%w%w%s", 0x51, 1, 2,TID_HONGNIANG,1,"Dạy Dỗ Ác Phách(Hoàn Thành)" ); 
	send_user( user, "%c%c%c%w%s", 0x51, 1, 1,TID_HONGNIANG,"Nhiệm Vụ Hồng Nương" ); 
	send_user( user, "%c%c%c%w%w%s", 0x51, 1, 2,TID_HONGNIANG,1,"Dạy Dỗ Ác Phách(Hoàn Thành)" ); 
