...
{ 
        ...
        {
//        	send_user(owner, "%c%s", ':', sprintf("%s：\n何方鼠辈，胆敢在本门所在撒野！\n%s：大爷我在这碍你了？讨打～", owner->get_name(), get_name() ) );
                return 1;
        }
	if (me->get("user")==who->get_number())
	{	
//		send_user(who, "%c%s", ':', sprintf("%s：\n何方鼠辈，胆敢在本门所在撒野！\n%s：大爷我在这碍你了？讨打～", who->get_name(), get_name() ) );
		return 1;            
	}
...
void win_bonus_callout( object me, object who )
{
	...
	if (!who) return;
	if (who->get_save_2("mastertask.type")==9)
	{
...
//			who->set_save_2("mastertask.time", 0);
			...
			send_user(who, "%c%s", '!', "Ngươi ra rời mục tiêu quá xa, nhiệm vụ thất bại !");
			return ;			
		}		
