...
void create()
{
        set_name("Vệ Vương Tàn Phách");

        set_char_type(FIGHTER_TYPE_2);    // init_level 要用到
        
        set_head_color(0x100);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2870 * rate / 100 ) ;    // NONE

                else if( p < 3510 * rate / 100 )    // 金钱
                {
