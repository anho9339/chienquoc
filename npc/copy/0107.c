...
void create()
{
        set_name("Trần Vương Tàn Phách");

        set_char_type(FIGHTER_TYPE_2);    // init_level 要用到
        
        set_head_color(0x3016);
        
        set_walk_speed(2);
        set_attack_speed(10);

        NPC_ENERGY_D->init_level( this_object(), 70+ random(4) );    // 
        set_max_seek(4);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2556 * rate / 100 ) ;    // NONE

                else if( p < 3196 * rate / 100 )    // 金钱
                {
                        ...
                        if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
                        {
                                ...
                                item->set_value( 1250 + random(250) );
...
/*              else if( p < 3196 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 3871 * rate / 100 )    // 武器(门派)
                {
                        if( equip >= total2 ) continue;

                        switch( random(3) )
                        {
                       case 0 : file = WEAPON_FILE->get_weapon_file_2(45);  break;
                ...
                else if( p < 4546 * rate / 100 )    // 防具(门派)
                {
                        if( equip >= total2 ) continue;

                        switch( random(3) )
                        {
                       case 0 : file = ARMOR_FILE->get_armor_file_2(45);  break;
                ...
                else if( p < 9796 * rate / 100 )    // 杂物２
                {
                       ...
                       case 4 : file = "/item/stuff/0049";  break;
                       case 5 : file = "/item/32/3285";  break;
