...
void create()
{
        set_name("Cơ Quan Nhân");

        set_char_type(FIGHTER_TYPE);    // init_level 要用到
        
        set_head_color(0x7a6e);
	set_walk_speed(2);
	set_attack_speed(20);
        NPC_ENERGY_D->init_copy_npc( this_object(), 85+ random(2),({"无","强壮","暗器"}) );    // 
        set_max_seek(6);
	set_walk_step(2);
	set_enmity_flag(1);
        setup();
