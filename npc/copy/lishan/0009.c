...
void create()
{
	...
	set_walk_speed(2);
	set_attack_speed(15);
        NPC_ENERGY_D->init_copy_npc( this_object(), 75+ random(2),({"人形","强壮","武器"}) );    // 
        set_max_seek(4);
	set_walk_step(2);
//	set_enmity_flag(1);
        setup();
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_2_00, 28) && !who->get_legend(TASK_2_00, 29) && who->get_save("fb_sx_qlgj") < 12 )
	{
		if ( (i=who->add_save("fb_sx_qlgj",1)) >= 12 && who->get_save("fb_sx_qlky") >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,102,"大肆破坏(完成)" );
			who->set_legend(TASK_2_00, 29);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QINGUO,102);
		send_user(who,"%c%s",'!',sprintf(HIY"Tần Lăng Công Tượng %d/12",i));		
	}
	if( who->get_legend(TASK_2_01, 7) && !who->get_legend(TASK_2_01, 8) &&
		USER_INVENTORY_D->count_empty_carry(who) >=1 && random100() < 5 )
	{
		TASK_LEGEND_D->give_item(who,"item/98/0244",1);
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,101,"千里寻亲(完成)" );
		who->set_legend(TASK_2_01, 8);
		USER_TASK_D->send_task_intro(who,2,TID_HANGUO,101);
		send_user(who,"%c%s",'!',HIY"兄长的玉佩 1/1");		
	}
}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4252 * rate / 100 ) ;    // NONE

                else if( p < 5252 * rate / 100 )    // 金钱
                {
