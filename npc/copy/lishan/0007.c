...
void create()
{
	...
	set_walk_speed(2);
	set_attack_speed(20);
        NPC_ENERGY_D->init_copy_npc( this_object(), 79+ random(3),({"人形","强壮","五雷"}) );    // 
        set_max_seek(6);
	set_walk_step(2);
	set_enmity_flag(1);
        setup();
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4570 * rate / 100 ) ;    // NONE

                else if( p < 5570 * rate / 100 )    // 金钱
                {
