...
void create()
{
	...
	set_walk_speed(2);
	set_attack_speed(15);
        NPC_ENERGY_D->init_copy_npc( this_object(), 83+ random(2),({"人形","强壮","武器"}) );    // 
        set_max_seek(4);
	set_walk_step(2);
	set_enmity_flag(1);
        setup();
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_2_01, 7) && !who->get_legend(TASK_2_01, 8) &&
		USER_INVENTORY_D->count_empty_carry(who) >=1 && random100() < 21 )
	{
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4870 * rate / 100 ) ;    // NONE

                else if( p < 5870 * rate / 100 )    // 金钱
                {
