...
void create()
{
        set_name("Ba Vương Tàn Phách");

        set_char_type(FIGHTER_TYPE_2);    // init_level 要用到
        
        set_head_color(0x8410);
        
        set_walk_speed(2);
        set_attack_speed(10);

        NPC_ENERGY_D->init_level( this_object(), 65+ random(4) );    // 
        set_max_seek(4);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2566 * rate / 100 ) ;    // NONE

                else if( p < 3206 * rate / 100 )    // 金钱
                {
                        ...
                        if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
                        {
                                ...
                                item->set_value( 1220 + random(220) );
