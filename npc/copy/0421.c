...
void create()
{
	...
	set_walk_speed(2);
	set_attack_speed(15);
        NPC_ENERGY_D->init_copy_npc( this_object(), 44+ random(2),({"人形","强壮","空手"}) );    // 
        set_max_seek(4);
	set_walk_step(2);
	set_enmity_flag(1);
        setup();
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_2_00, 1) && !who->get_legend(TASK_2_00, 2) && who->get_save("fb_001_003") < 4 )
	{
		if ( (i=who->add_save("fb_001_003",1)) >= 4 && who->get_save("fb_001_001") >= 4 && who->get_save("fb_001_002") >= 4 && who->get_save("fb_001_004") >= 10 && who->get_save("fb_001_005") >= 10 )
		{
		...
		send_user(who,"%c%s",'!',sprintf(HIY"Phản Quân Mã Phu %d/4",i));		
	}
}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 3790 * rate / 100 ) ;    // NONE

                else if( p < 4790 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 4790 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9390 * rate / 100 )    // 杂物１
                {
                       ...
                       case 1 : file = "/item/40/4048";  break;
