...
void create()
{
        ...
        set_head_color(0x300f);
        
        set_walk_speed(2);
        set_attack_speed(10);

        NPC_ENERGY_D->init_level( this_object(), 105+ random(4) );    // 
        set_max_seek(6);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2736 * rate / 100 ) ;    // NONE

                else if( p < 3376 * rate / 100 )    // 金钱
                {
