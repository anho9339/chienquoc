...
void create()
{
	...
	set_walk_speed(2);
	set_attack_speed(15);
        NPC_ENERGY_D->init_copy_npc( this_object(), 54+ random(3),({"亡灵","强壮","武器"}) );    // 
        set_max_seek(4);
	set_walk_step(2);
	set_enmity_flag(1);
        setup();
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_2_00, 7) && !who->get_legend(TASK_2_00, 8) && random(100) < 21 )
	{
		item = new("item/98/0239");
		if ( !item  )
			return ;
		p = item->move(who,-1);
		if ( !p )
		...
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,103,"怨鬼之灵(完成)" );
		who->set_legend(TASK_2_00, 8);

		USER_TASK_D->send_task_intro(who,2,TID_QIGUO,103);
		send_user(who,"%c%s",'!',HIY"怨鬼之灵 1/1");
	}
}
