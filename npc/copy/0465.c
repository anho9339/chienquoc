...
void create()
{
	...
	set_walk_speed(2);
	set_attack_speed(20);
        NPC_ENERGY_D->init_copy_npc( this_object(), 54+ random(3),({"妖","强壮","五雷"}) );    // 
        set_max_seek(6);
	set_walk_step(2);
	set_enmity_flag(1);
        setup();
...
/*              else if( p < 5360 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9990 * rate / 100 )    // 杂物２
                {
                        switch( random(2) )
                        {
                       case 0 : file = "/item/stuff/0032";  break;
