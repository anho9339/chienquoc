...
void create()
{
        set_name("Quắc Vương Tàn Phách");

        set_char_type(FIGHTER_TYPE_2);    // init_level 要用到
        
        set_head_color(0x106d);
        
        set_walk_speed(2);
        set_attack_speed(10);

        NPC_ENERGY_D->init_level( this_object(), 50+ random(4) );    // 
        set_max_seek(6);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 2002 * rate / 100 ) ;    // NONE

                else if( p < 2642 * rate / 100 )    // 金钱
                {
...
/*              else if( p < 2642 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 9276 * rate / 100 )    // 杂物１
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/40/4047";  break;
