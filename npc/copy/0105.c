...
void create()
{
        set_name("Cảnh Vương Tàn Phách");

        set_char_type(FIGHTER_TYPE_2);    // init_level 要用到
        
        set_head_color(0x1004);
        
        set_walk_speed(2);
        set_attack_speed(10);

        NPC_ENERGY_D->init_level( this_object(), 60+ random(4) );    // 
        set_max_seek(6);
...
//      p = random(100);  size = ( p > 2 ) ? 1 : ( p > 0 ) ? 2 : 8;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 1939 * rate / 100 ) ;    // NONE

                else if( p < 2579 * rate / 100 )    // 金钱
                {
                        ...
                        if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
                        {
                                ...
                                item->set_value( 1150 + random(150) );
...
/*              else if( p < 2579 * rate / 100 )    // 防具(基本)
                ...
                else if( p < 3247 * rate / 100 )    // 武器(门派)
                {
                        if( equip >= total2 ) continue;

                        switch( random(3) )
                        {
                       case 0 : file = WEAPON_FILE->get_weapon_file_2(35);  break;
                       case 1 : file = WEAPON_FILE->get_weapon_file_2(45);  break;
                       case 2 : file = WEAPON_FILE->get_weapon_file_2(55);  break;
                ...
                else if( p < 3916 * rate / 100 )    // 防具(门派)
                {
                        if( equip >= total2 ) continue;

                        switch( random(3) )
                        {
                       case 0 : file = ARMOR_FILE->get_armor_file_2(35);  break;
                       case 1 : file = ARMOR_FILE->get_armor_file_2(45);  break;
                       case 2 : file = ARMOR_FILE->get_armor_file_2(55);  break;
                ...
                else if( p < 9800 * rate / 100 )    // 杂物３
                {
                        switch( random(1) )
                        {
                       case 0 : file = BOOK_FILE->get_book_file();  break;
                ...
                else if( p < 10000 * rate / 100 )    // 杂物４
                {
                        switch( random(1) )
                        {
                       case 0 : file = "/item/44/4488";  break;
...
/*              else if( p < 10000 * rate / 100 )    // 杂物５
	...
	"sys/copy/mirror"->drop_shenqi(me,who);	//神器材料
}
