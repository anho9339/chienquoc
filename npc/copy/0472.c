...
void create()
{
        set_name("Phản Quân Tiểu Đội Trưởng");
	...
	set_walk_speed(2);
	set_attack_speed(15);
        NPC_ENERGY_D->init_copy_npc( this_object(), 49+ random(2),({"人形","强壮","武器"}) );    // 
        set_max_seek(4);
	set_walk_step(2);
	set_enmity_flag(1);
        setup();
...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_2_00, 1) && !who->get_legend(TASK_2_00, 2) && who->get_save("fb_001_005") < 10 )
	{
		if ( (i=who->add_save("fb_001_005",1)) >= 10 && who->get_save("fb_001_001") >= 4 && who->get_save("fb_001_002") >= 4 && who->get_save("fb_001_003") >= 4 && who->get_save("fb_001_004") >= 10 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,101,"剿灭叛军(完成)" );
			who->set_legend(TASK_2_00, 2);
		}
		USER_TASK_D->send_task_intro(who,2,TID_QINGUO,101);
		send_user(who,"%c%s",'!',sprintf(HIY"Phản Quân Tiểu Đội Trưởng %d/10",i));		
	}
}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                if( p < 4286 * rate / 100 ) ;    // NONE

                else if( p < 5286 * rate / 100 )    // 金钱
                {
