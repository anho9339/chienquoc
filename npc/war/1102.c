...
void win_bonus2( object me, object who )
{
	...
	object orgnpc, map, item, leader, item2, item3, item4, item5, item6, item7, item8, item9, item10, item11, item12, item13;
...
/*	if (rand<1500*rate/100)
	...
	if (rand<8500*rate/100)
	{
		item = new ("/item/TuiLinhThach/NgauNhien/2");		// Linh thạch 2
 //       item6 = new ("/item/TuiLinhThach/NgauNhien/3");		
//		item3 = new ("/item/sell/0033");		
	}	
	else
	if (rand<9500*rate/100)
	{
		...
		item2 = new ("/item/sell/2032");
		//item->set_amount(5);
	} 
 
	if (rand<10000*rate/100)
	{
		item2 = new ("/item/44/4489");	// Tinh Nguyên Đơn
		item3 = new ("item/manhghep/bdp");	// Mảnh ghép bảo để phù
		item4 = new ("item/huyetlong/ghep/mg");	 // Ghép đồ 105
		item5 = new (BOOK_FILE->get_book_file_2());			
		item7 = new ("/item/exp/tinhdonexp3");
		item8 = new ("item/exp/tinhdonexp");	
		item9 = new ("item/exp/tinhdonexp2");	
		item10 = new ("/item/manhghep/dauan");	 // Dầu ăn
		item11 = new ("/item/manhghep/deplao"); // Dép lào
		item12 = new ("/item/manhghep/mgphnb"); // Mảnh ghép phnb
	}	
	if (item && ( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) ) )
	{
		TEAM_D->drop_item(item,who);
	...
	if (item10 && ( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) ) )
	{
		...
		item10->set_amount(2+random(3));
		item10->set_user_id(owner);
		item10->add_to_scene(z, p / 1000, p % 1000);
		item10->set("winner", who);
		item10->set( "time", time() );
	}	
	if (item11 && ( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) ) )
	{
		TEAM_D->drop_item(item11,who);
		item11->set_amount(2+random(3));
		item11->set_user_id(owner);
		item11->add_to_scene(z, p / 1000, p % 1000);
		item11->set("winner", who);
		item11->set( "time", time() );
	}
	if (item12 && ( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) ) )
	{
		TEAM_D->drop_item(item12,who);
		item12->set_amount(3+random(2));
		item12->set_user_id(owner);
		item12->add_to_scene(z, p / 1000, p % 1000);
		item12->set("winner", who);
		item12->set( "time", time() );
	}
}
