...
void heart_beat_walk()
{
	...
	{		
		...
		if ( gone_time(saytime) >=60 )
		{
			...
			if(random(100) < 40)
			{
				SAY_CMD->say(me, "Ngươi thật quá vô dụng!"); 
			}
		}
	}
	else if ( RelaxTime == 0 )
...
int perform_action( object who ) 
{
	...
	if ( lowhp && summon == 0 && ( p = get_valid_xy(z=get_z(me), get_x(me), get_y(me), IS_ITEM_BLOCK) ) )
	{
		...
		SAY_CMD->say(me, "Ngươi hãy chịu chết đi!"); 	
		summon = 1;
	...
	{
		pTime = time();
		SAY_CMD->say(me, "Yaaa, ta chém...ta chém!");
	...
	{
		pTime1 = time();
		SAY_CMD->say(me, "Ta là yêu quái mạnh nhất từ trước đến nay!");
