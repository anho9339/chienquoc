...
void heart_beat_walk()
{
	...
	{		
		...
		if ( gone_time(saytime) >=60 )
		{
			...
			if(random(100) < 40)
			{
				SAY_CMD->say(me, "Đến đây đi Đến đây đi，các ngươi biết cái quái gì về yêu và hận?"); 
			}
		}
	}
	else if ( RelaxTime == 0 )
...
int perform_action( object who ) 
{
	...
	if ( gone_time(pTime) > 30 )
	{
		...
		if ( size=sizeof(char) )
		{
			SAY_CMD->say(me, "Hãy làm chậm mọi thứ nào");
	...
	if ( gone_time(pTime1) > 60 )
	{
		...
		{
			SAY_CMD->say(me, "Nếu như mọi thứ đều dừng lại");
...
void win_bonus_callout( object me, object who )
{
	...
	exp = 15000;
		...
		if( team[i]->get_legend(TASK_2_01, 1) && !team[i]->get_legend(TASK_2_01, 2) 
			&& USER_INVENTORY_D->count_empty_carry(team[i]) >=1)
		{
			...
			item->add_to_user(p);
			send_user( team[i], "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,103,"Hồn Hề Trở Về (Hoàn Thành)" );
			team[i]->set_legend(TASK_2_01, 2);
			USER_TASK_D->send_task_intro(team[i],2,TID_QINGUO,103);
			send_user(team[i],"%c%s",'!',HIY"Tam Hồn Lục Phách 1/1");
			send_user(team[i],"%c%s",';',"Trong không khí loáng thoáng giọng nói bi thương.");
			item->say_hello(1);		
		}
	}
}
...
void stop_fight()
{
	...
	{
		SAY_CMD->say(this_object(), "Haha，Đây chính là hận đấy……");
		add_hp(2000);
	}
	::stop_fight();	
}
