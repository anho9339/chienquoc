...
void heart_beat_walk()
{
	...
	{		
		...
		if ( gone_time(saytime) >=60 )
		{
			...
			if(random(100) < 40)
			{
				SAY_CMD->say(me, "Ngươi nào can đảm, dám xông vào Hoàng Lăng"); 
			}
		}
	}
	else if ( RelaxTime == 0 )
...
int perform_action( object who ) 
{
	...
	if ( gone_time(pTime) > 15 )
	{
		...
		{
			SAY_CMD->say(me, "Hãy xem chiêu Ngàn Dặm Ngựa Phi");
	...
	{
		pTime1 = time();
		SAY_CMD->say(me, "Hãy xem ngươi tới đâu, Trực Đảo Hoàng Long");
...
void stop_fight()
{
	...
	{
		SAY_CMD->say(this_object(), "Haha! Đây gọi là trừng phạt!!!");
		add_hp(2000);
	}
	::stop_fight();	
}
