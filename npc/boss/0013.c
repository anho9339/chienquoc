...
void heart_beat_walk()
{
	...
	{		
		...
		if ( gone_time(saytime) >=60 )
		{
			...
			if(random(100) < 40)
			{
				SAY_CMD->say(me, "Ta ngửi thấy mùi người sống."); 
			}
		}
	}
	else if ( RelaxTime == 0 )
...
int perform_action( object who ) 
{
	...
	if ( call_npc == 0 )
	{
		call_npc = 1;	
		map = me->get("map");
		SAY_CMD->say(me, "Xông vào hoàng lăng của ta，hãy mau tới bắt hắn lại.");
	...
	if ( gone_time(pTime) > 25 )
	{
		...
		{
			SAY_CMD->say(me, "Tiếp nhận Chi Long Hỏa Ngục của ta đây");
	...
	if ( gone_time(pTime1) > 25 )
	{
		...
		if ( size=sizeof(nTarget) )
		{
			SAY_CMD->say(me, "Xem Kim Quang Pháp Trận đây");
...
void stop_fight()
{
	...
	{
		SAY_CMD->say(this_object(), "Còn dám xông vào，còn không mau đầu hàng đi!");
		add_hp(2000);
	}
	::stop_fight();	
}
