...
void drop_items_callout( object me, object who )
{
		...
		who->add_exp(600);
		who->add_cash(500);
		item = new("item/event/nhagiao/tuiquatonsu");
		tuiquatonsu = item->move(who,-1);
			if ( !tuiquatonsu )
			{
				destruct(item);
				return ;	
			}
			item->add_to_user(tuiquatonsu);
			send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
			USER_D->user_channel(sprintf(HIC"Bằng hữu "HIY "%s"  HIC" vừa đập tên "HIY "Lưu Manh Giả Danh Tri Thức"HIC" giải oan cho bao người tốt.", who->get_name() ));	
