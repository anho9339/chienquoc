...
string get_short() 
{ 
	return get_name() + "||||" + "Tiên Cảnh Ma Vương";
}
...
//        USER_ENERGY_D->count_ap(me);
 //       USER_ENERGY_D->count_dp(me);
    ...
    //    if (who->get("anti_faint")) return 2;
        if (who->get("anti_faint")) return 1;
...
//				team[i]->add_save("testbonus",3000);
				...
				team[i]->set_fam_contribute(me->get_fam_contribute()+10);
				team[i]->add_exp(1000);
				team[i]->add_cash(1000);		
	}
}
...
void drop_items( object me, object who )
{
	...
	z = get_z(me);  x = get_x(me);  y = get_y(me);
        id = who->get_leader();
			USER_D->user_channel(sprintf(HIC"Nghe nói Bằng hữu "HIY "%s"  HIC" đã tiêu diệt "HIY "Đông Phương Bất Bại "HIC"...!", who->get_name() ));
	...
	if ( rate < rate1 )
	{
		/*if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	//一张虎皮
		{
			...
			item->set_value( 100000+random(100000) );
			TEAM_D->drop_item(item,who);
			item->set_user_id(owner);
			item->add_to_scene(z, p / 1000, p % 1000);
			item->set("winner", who);
			...
			return;
			rate = random(10000);
			if ( rate < 9500  * rate1 / 100 )		//完美宝石矿 1个
			{
