...
{ 
	...
	if ( who->is_user() && who->get_level() >= 105 ) {
	set_enemy(0);
	send_user( who, "%c%c%w%s", ':', 3, get_char_picid(),  "Cấp bậc của ngươi đã đạt 105 trở lên, không thể tấn công ta được đâu!" );
	return 0;
	}
...
void drop_items( object me, object who )
{
	...
	z = get_z(me);  x = get_x(me);  y = get_y(me);
        id = who->get_leader();
			USER_D->user_channel(sprintf(HIY"Bằng hữu "HIC "%s"  HIY" đã đánh bại "HIG "Sát Thần"HIC" ! ", who->get_name() ));
	...
	if ( rate < rate1 )
	{
		...
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
					...
					for(i=0;i<2;i++)
	        {
					...
					switch( random(2) )
		                {
		               case 0 : item = new( "/item/test2/TuiQuaVoSong" );  break; 
		               case 1 : item = new( "/item/test2/TuiPhapBao" );  break; 	
		...
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
	                {
                       ...
                       case 2 : item = new( "/item/sell/0032" );  break; 	
                       case 3 : item = new( "/item/sell/2032" );  break; 	
                       case 4 : item = new( "/item/sell/0032" ); break;					   
		                }
	                        TEAM_D->drop_item(item,who);
				item->set_user_id(owner);
	                        item->add_to_scene(z, p / 1000, p % 1000);
		...
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
	                {
		               ...
		               case 0 : item = new( "/item/sell/0032" );  break; 
		               case 1 : item = new( "/item/sell/1032" );  break; 	
                       case 2 : item = new( "/item/sell/2032" );  break; 	                   				   
		                }
	                        TEAM_D->drop_item(item,who);
				item->set_user_id(owner);
	                        item->add_to_scene(z, p / 1000, p % 1000);
