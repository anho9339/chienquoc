...
{ 
	...
	if ( who->is_user() && who->get_level() >= 90 ) {
	set_enemy(0);
	return 0;
	}
...
{ 
	...
	if ( who->is_user() && who->get_level() >= 90 ) {
	set_enemy(0);
	send_user( who, "%c%c%w%s", ':', 3, get_char_picid(),  "Cấp bậc của ngươi đã đạt 90 trở lên, không thể tấn công ta được đâu!" );
	return 0;
	}
...
void heart_beat_walk()
{
	...
	{		
		...
		if ( gone_time(saytime) >=60 )
		{
			...
			if(random(100) < 40)
			{
				SAY_CMD->say(me, "Nhìn ngươi tư chất rất tốt, hãy ở lại với tao để đi cướp Vàng rồi giết người đi. ha ha ha !!"); 
			}
		}
	}
	else if ( RelaxTime == 0 )
...
int perform_action( object who ) 
{
	...
	if ( !lowhp )
	{
		if ( gone_time(pTime) > 20 )
		{
			...
			{
				pTime = time();
				SAY_CMD->say(me, "Tổ cha tụi bậy giám bắt tao à ? Tao giết người thì kệ tao ai biểu tụi bây bắt tao chi !");
				return 1;
			}
		}
		...
		{
			pTime1 = time();
			SAY_CMD->say(me, "Ta sẽ cướp vàng của ngươi");
...
void drop_items( object me, object who )
{
	...
	z = get_z(me);  x = get_x(me);  y = get_y(me);
        id = who->get_leader();
		USER_D->user_channel(sprintf("Nghe nói Bằng hữu "HIY "%s"  NOR " đã tiêu diệt "HIY "Lê Văn Luyện "NOR "...!", who->get_name() ));
	...
	{			
			...
			if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
			{
			item = new( "quest/product2"->get_random_level_yellow_neck_product(76,86) );
			TEAM_D->drop_item(item,who);
			item->set_user_id(owner);
			item->add_to_scene(z, p / 1000, p % 1000);
			item->set("winner", who);
...
object summon_guard(object who)
{
	...
	npc->set_level(level);
	npc->set_name( "Đồng Đảng Lê Văn Luyện" );
	npc->set_owner(who);
	return npc;
}