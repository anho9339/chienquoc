...
void do_look(object who)
{
		...
		{
			send_user(who, "%c%s", ';', "Cấp bậc 40-60 mới có thể hái quả ở Cây thường !");
			return 0;
		}
...
int into_effect( object who , string arg)
{
	...
	send_user(who, "%c%c%c", 0x5a, 1, 1);
		...
		if( objectp( item = present("Quả thường", who, 1, MAX_CARRY*4) ) && item->get_amount() < 10000 )
		{
			item->add_amount(1);				
		}
		else
		{
				item = new("/item/event/quathuong");
				p = item->move2(who,-1);
				item->add_to_user(p);
		}		
