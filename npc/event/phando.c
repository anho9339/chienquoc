...
int get_hair_color() { return 0x6300; }
...
string get_short() 
{ 
	return get_name() + "||||" + "Phản Đồ Cấm Vệ Quân";
}
...
//                team[i]->add_exp(exp2);
			...
			//	"sys/sys/test_db"->add_yuanbao(team[i],50);
				team[i]->set_fam_contribute(team[i]->get_fam_contribute()+10); // Điểm cống hiến phái +10 
				team[i]->add_exp(1000);
				team[i]->add_cash(50000);		
	}
}
...
void drop_items( object me, object who )
{
	...
	z = get_z(me);  x = get_x(me);  y = get_y(me);
        id = who->get_leader();
			USER_D->user_channel(sprintf(HIC"Nghe nói Bằng hữu "HIY "%s"  HIC" đã tiêu diệt "HIY "Phản Đồ Cấm Vệ Quân "HIC"...!", who->get_name() ));
	...
	if ( rate < rate1 )
	{
		...
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	//Tử Kim Bảo Hộp
		{
	...
	if ( !p )
		return;
	rate = random(10000);
	if ( rate < 9800  * rate1 / 100 )		//Công thức vàng 97 hoặc 107
	{
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
	    {
		...
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
	    {
			item = new( "quest/product2"->get_random_level_yellow_product(107,117) );
			TEAM_D->drop_item(item,who);
			item->set_user_id(owner);
			item->add_to_scene(z, p / 1000, p % 1000);
			item->set("winner", who);
