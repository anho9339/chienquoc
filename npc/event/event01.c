...
void drop_items_callout( object me, object who )
{
        ...
        int p, rate, i, size, gold, equip, total, total2, e1, e2, e3, e4, e5 ;
        int z, x, y;

        z = get_z(me);  x = get_x(me);  y = get_y(me);
        id = who->get_leader();
		item = new("item/event/DanPhucHy");
		e1 = item->move2(who,-1);
		item1 = new("item/event/DaNuOa");
		e2 = item1->move2(who,-1);
		item2 = new("item/event/DinhThanNong");
		e3 = item2->move2(who,-1);
		item3 = new("item/event/AnKhongDong");
		e4 = item3->move2(who,-1);
		item4 = new("item/event/KinhConLon");
		e5 = item4->move2(who,-1);
			if ( !e1 || !e2 || !e3 || !e4 || !e5 )
			{
				destruct(item);
				return ;	
			...
			item->add_to_user(e1);
			item1->add_to_user(e2);
			item2->add_to_user(e3);
			item3->add_to_user(e4);
			item4->add_to_user(e5);
			send_user(who,"%c%s",';',"Bạn nhận được "HIY " 5 món Thần Khí thượng cổ");
			USER_D->user_channel(sprintf(HIC"Bằng hữu "HIY "%s"  HIC" vừa tiêu diệt "HIY "Sói Vương "HIC" và lấy được 5 món Thần Khí thượng cổ vô cùng quý hiếm ", who->get_name() ));
			USER_D->user_channel(sprintf(HIC"Bằng hữu "HIY "%s"  HIC" vừa tiêu diệt "HIY "Sói Vương "HIC" và lấy được 5 món Thần Khí thượng cổ vô cùng quý hiếm ", who->get_name() ));
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 8940 * rate / 100 )    // 武器(门派)
                {
                        ...
                        switch( random(3) )
                        {
                       case 0 : file = WEAPON_FILE->get_weapon_file_2(130);  break;
                       case 1 : file = WEAPON_FILE->get_weapon_file_2(130);  break;
                       case 2 : file = WEAPON_FILE->get_weapon_file_2(130);  break;
                ...
                else if( p < 9100 * rate / 100 )    // 防具(门派)
                {
                        ...
                        switch( random(3) )
                        {
                       case 0 : file = ARMOR_FILE->get_armor_file_2(130);  break;
                       case 1 : file = ARMOR_FILE->get_armor_file_2(130);  break;
                       case 2 : file = ARMOR_FILE->get_armor_file_2(130);  break;
...
/*              else if( p < 9100 * rate / 100 )    // 杂物１
                ...
                else if( p < 9850 * rate / 100 )    // 杂物２
                {
                       ...
                       case 1 : file = "/item/91/9135";  break;
