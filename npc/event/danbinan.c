...
{ 
		if ( this_object()->get("danbinan2")!=who->get_number() ) return 0;
		...
		who->add_save("diemcuudoi",1);
		write_user(who,"Bạn cứu đói 1 dân bị nạn, nhận được 1 điểm tích luỹ Cứu Thế.");
...
void heart_beat_loop_callout( object me, int effect1, int effect2, int effect3, int effect4, int effect5, int effect6, int effect7, int effect8 )    // 每１秒处理
{
        ...
        if( p % 1 == 0 )   
        {
                if( p % 2 == 0 )    
                {
						...
						SAY_CMD->say( me, sprintf("%s, tôi đói quá, xin cho tôi %s #49",owner->get_name(),me->get("thucan")) );
                }
