...
// 函数：获取人物造型
int get_char_picid() { return 5504; }
...
void create()
{
        set_name("Công tượng Rèn Chu Quốc");
        set_real_name("Công tượng Rèn 1~20");
...
void do_look( object who )
{
	...
	if ( who->get_legend(TASK_2_00,20) && !who->get_legend(TASK_2_00,21) && who->get_save("baifang_js_5") != 1)
		tmp = sprintf(ESC HIY "Bái sư\ntalk %x# welcome.1\n",getoid(me));
	tmp += sprintf(ESC"Học tập kỹ năng\nlearn ? %s\n", me->get_char_id());
	tmp += sprintf(ESC"Mua công thức kỹ năng Rèn Đúc đẳng cấp1-20\nlist %s\n", me->get_char_id());
	if ( sizeof(tmp) )
		send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf( " %s :\n Ở chỗ của ta Ngươi có thể học được hết những kỹ năng của Công tượng sắt mà Ngươi học ở nước Tần, nhưng phương pháp pha chế cao cấp thì Ngươi phải mua ở chỗ những Công tượng của nước Tần.\n",get_name(),getoid(me)) +
		tmp +
		ESC"Rời khỏi");
	else
		send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf( " %s :\n Ở chỗ của ta Ngươi có thể học được hết những kỹ năng của Công tượng sắt mà Ngươi học ở nước Tần, nhưng phương pháp pha chế cao cấp thì Ngươi phải mua ở chỗ những Công tượng của nước Tần.\n",get_name()));
