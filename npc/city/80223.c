...
void do_look( object who )
{
	...
	if ( who->get_legend(TASK_2_00,20) && !who->get_legend(TASK_2_00,21) && who->get_save("baifang_js_3") != 1)
		tmp = sprintf(ESC HIY "Bái sư\ntalk %x# welcome.1\n",getoid(me));
	tmp += sprintf(ESC"Học tập kỹ năng\nlearn ? %s\n", me->get_char_id());
	if ( sizeof(tmp) )
		send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf( " %s :\n Ở chỗ ta Ngươi có thể học được kỹ năng Dược Sư mà Ngươi học được ở nước Triệu.\n",get_name(),getoid(me)) +
		tmp +
		ESC"Rời khỏi");
	else
		send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf( " %s :\n Ở chỗ ta Ngươi có thể học được kỹ năng Dược Sư mà Ngươi học được ở nước Triệu.\n",get_name()));
