...
// 函数：获取人物造型
int get_char_picid() { return 5506; }
...
void create()
{
        set_name("Công tượng Mỏ Chu Quốc");
...
void do_look( object who )
{
	...
	if ( who->get_legend(TASK_2_00,20) && !who->get_legend(TASK_2_00,21) && who->get_save("baifang_js_7") != 1)
		tmp = sprintf(ESC HIY "Bái sư\ntalk %x# welcome.1\n",getoid(me));
	tmp += sprintf(ESC"Học tập kỹ năng\nlearn ? %s\n", me->get_char_id());
	if ( sizeof(tmp) )
		send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf( " %s :\n Ở chỗ ta ngươi có thể học được toàn bộ kỹ năng luyện quặng ở Yên Quốc có thể học được.\n",get_name(),getoid(me)) +
		tmp +
		ESC"Rời khỏi");
	else
		send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf( " %s :\n Ở chỗ ta ngươi có thể học được toàn bộ kỹ năng luyện quặng ở Yên Quốc có thể học được.\n",get_name()));
