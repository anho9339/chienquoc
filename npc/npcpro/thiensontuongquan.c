...
void create()
{
        set_name("Thiên Sơn Tướng Quân");
		set_real_name(HIR "Bá Đạo");
        set_2( "good", ([
           /*     "01" : "/item/sell/0032",
				"02" : "/item/sell/1032",
				"03" : "/item/sell/2032",
...
//		ESC HIY"Ta muốn thuê trang bị Bá Vương\ntalk %x# gift.1\n"
        ESC HIY"Truyền thuyết Chiến Y và Chiến Mão\ntalk %x# gift.80\n"
		ESC HIY"Ta muốn đổi Chiến Y hoặc Chiến Mão lấy phần thưởng\ntalk %x# gift.13\n"
...
void do_gift2( object me, string arg )
{
 ...
 /*     	case 1:
			...
			break;
		case 2:
				send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf(" %s :\n Bạn muốn hợp thành vật phẩm nào?? \nNgoại Trang Bá Vương: cần 8 mảnh ghép Ngoại Trang \n"
...
//					ESC HIY"[Cánh Nữ]\ntalk %x# gift.32\n"
		...
		case 4:
			...
			break;
		case 5:
				send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf(" %s :\n Bạn muốn hợp thành vật phẩm nào?? \nKì Lân Ngọc Bội: cần 4 mảnh ghép Ngọc Bội \nPhượng Hoàng Ngọc Bội: cần 4 mảnh ghép Ngọc Bội \nThỏ Thần Ngọc Bội: cần 4 mảnh ghép Ngọc Bội \nĐại Thánh Ngọc Bội: cần 4 mảnh ghép Ngọc Bội \nThiên Long Ngọc Bội 1: cần 8 mảnh ghép Ngọc Bội \nThiên Long Ngọc Bội 2: cần 8 mảnh ghép Ngọc Bội \nThiên Long Ngọc Bội 3: cần 8 mảnh ghép Ngọc Bội \nThiên Long Ngọc Bội 4: cần 8 mảnh ghép Ngọc Bội \nThiên Long Ngọc Bội 5: cần 8 mảnh ghép Ngọc Bội \n"
...
//					ESC HIY"3000 điểm Tiềm Năng\ntalk %x# gift.54\n"
		...
		case 12:
			...
			break; */
		case 13:
				send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf(" %s :\n Ta đã mơ ước từ lâu là có thể sưu tầm những Chiến Y và Chiến Mão thật nhiều để nhìn ngắm thật lâu, nhưng ta lại bận quá nhiều công việc quan trọng. Nếu bạn có thể tìm giùm ta 20 Chiến Y và 40 Chiến Mão ta sẽ tặng lại cho bạn Lễ vật !! \n"
					ESC HIY"[Giao nộp 20 Chiến Y]\ntalk %x# gift.68\n"
					ESC HIY"[Giao nộp 30 Chiến Mão]\ntalk %x# gift.69\n"
					ESC"Rời khỏi",get_name(),id,id,id));
			break;	
	/*	case 20:
		...
		break;
		case 29:
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Vương Giả") < 8 )
			{
				notify( "Ngươi không có 8 Mảnh Ghép Vương Giả" );
				return ;	
			}
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Vương Giả",8);	
		item = new(sprintf("/item/test2/Ghep/PPvaCanh/PhiPhong/%d", 1001+random(10)));
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 8 Mảnh Ghép Vương Giả hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
		break;
		case 30:
		...
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Vương Giả") < 8 )
			...
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Vương Giả",8);	
		item = new(sprintf("/item/test2/Ghep/PPvaCanh/Canh/%d", 1001+random(10)));
		item->set_hide(0);
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 8 Mảnh Ghép Vương Giả hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		...
		break;
		case 33:
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Ngoại Trang") < 8 )
			{
				notify( "Ngươi không có 8 Mảnh Ghép Ngoại Trang" );
				return ;	
			}
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Ngoại Trang",8);	
		item = new(sprintf("/item/test2/Ghep/NgoaiTrang/%d", 1001+random(10)));
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 8 Mảnh Ghép Ngoại Trang hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
		break;
		case 34:
		...
		break;
		case 35:
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Ngọc Bội") < 4 )
			{
				notify( "Ngươi không có 4 Mảnh Ghép Ngọc Bội" );
				return ;	
			}
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Ngọc Bội",4);	
	//	item = new("/item/01/0007");
		item = new(sprintf("/item/test2/Ghep/NgocBoi/0007/%d", 1001+random(10)));
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 4 Mảnh Ghép Ngọc Bội hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
		break;
		case 36:
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Ngọc Bội") < 4 )
			...
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Ngọc Bội",4);	
		item = new(sprintf("/item/test2/Ghep/NgocBoi/0008/%d", 1001+random(10)));
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 4 Mảnh Ghép Ngọc Bội hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
		...
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Ngọc Bội") < 4 )
			...
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Ngọc Bội",4);	
		item = new(sprintf("/item/test2/Ghep/NgocBoi/0009/%d", 1001+random(10)));
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 4 Mảnh Ghép Ngọc Bội hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
		...
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Ngọc Bội") < 4 )
			...
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Ngọc Bội",4);	
		item = new(sprintf("/item/test2/Ghep/NgocBoi/0010/%d", 1001+random(10)));
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 4 Mảnh Ghép Ngọc Bội hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
		break;
		case 39:
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Ngọc Bội") < 8 )
			{
				notify( "Ngươi không có 8 Mảnh Ghép Ngọc Bội" );
				return ;	
			}
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Ngọc Bội",8);	
		item = new(sprintf("/item/test2/Ghep/NgocBoi/0011/%d", 1010+random(10)));
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 8 Mảnh Ghép Ngọc Bội hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
		break;
		case 40:
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Ngọc Bội") < 8 )
			...
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Ngọc Bội",8);	
		item = new(sprintf("/item/test2/Ghep/NgocBoi/0012/%d", 1010+random(10)));
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 8 Mảnh Ghép Ngọc Bội hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
		...
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Ngọc Bội") < 8 )
			...
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Ngọc Bội",8);	
		item = new(sprintf("/item/test2/Ghep/NgocBoi/0013/%d", 1010+random(10)));
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 8 Mảnh Ghép Ngọc Bội hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
		...
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Ngọc Bội") < 8 )
			...
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Ngọc Bội",8);	
		item = new(sprintf("/item/test2/Ghep/NgocBoi/0014/%d", 1010+random(10)));
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 8 Mảnh Ghép Ngọc Bội hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
		...
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Mảnh Ghép Ngọc Bội") < 8 )
			...
			TASK_LEGEND_D->check_task_item1(who,"Mảnh Ghép Ngọc Bội",8);	
		item = new(sprintf("/item/test2/Ghep/NgocBoi/0015/%d", 1010+random(10)));
		i = item->move(who,-1);
		item->add_to_user(i);
			USER_D->user_channel(sprintf(HIG"Bằng hữu "HIY "%s" HIG" vừa lấy 8 Mảnh Ghép Ngọc Bội hợp thành: " HIY"%s " HIG " .", who->get_name(),item->get_name() ));
		send_user(who,"%c%s",';',"Bạn nhận được "HIR+item->get_name());
		...
		item = new(sprintf("item/08/0001"));
		item->set_amount(5);		
		i = item->move2(who,-1);
		item->add_to_user(i);
//		item = new(sprintf("item/test/phieuthuongnhan"));
//		item->set_amount(1);		
//		i = item->move2(who,-1);
//		item->add_to_user(i);
		who->add_cash(500000);
		who->add_exp(who->get_level()*1000);
//		who->set_level(who->get_level()+1);
//		who->add_save_2("diemtienvang.score",1000);
//		"sys/sys/test_db"->add_yuanbao(who,100); 
		send_user(who,"%c%s",':',me->get_name()+"：\nNgân lượng +500.000 lượng \nChiến Quốc Lệnh x5 \n"ESC"Rời khỏi.");
		break;
		case 57:
...
//		who->add_save_2("diemtienvang.score",1000);
		...
		break;
		case 66:
		if ( who->get_save_2("diemsumon.score") < 2 )
			{	
		...
		who->add_save_2("diemsumon.score",-2);
		who->add_exp(who->get_level()*5000);
		send_user(who,"%c%s",';',"Bạn nhận được "HIY " kinh nghiệm chúc phúc của Sứ giả hoạt động !!!");
		break;
		case 67:
		if ( TASK_LEGEND_D->check_task_item_amount(who,"Bí kíp Kim Nguyên Bảo") < 1 )
			{
				notify( "Ngươi không có Bí kíp Kim Nguyên Bảo" );
				return ;	
			}
			TASK_LEGEND_D->check_task_item1(who,"Bí kíp Kim Nguyên Bảo",1);	
		"sys/sys/test_db"->add_yuanbao(who,500);
		send_user(who,"%c%s",';',"Bạn nhận được "HIR "50 "NOR "Kim Nguyên Bảo");
		break; */
		//Event chiến y chiến mão
		case 68:
		...
		break;
		case 80:
			send_user(who,"%c%s",':',sprintf("Thiên Sơn Tướng Quân:\n Ngày xưa ta là tướng quân vĩ đại trên thiên đình, chỉ vì phạm tội tế nhị như nhìn gái nên quần áo bị ném xuống dưới đây.\n Nếu các ngươi có thể tìm lại 20 Chiến Y và 30 Chiến Mão do ngọc hoàng ném xuống đây thì chắc chắn ta sẽ ban tặng phần thưởng không nhỏ.\n"ESC"Phản hồi"));
			break;
		}
}
