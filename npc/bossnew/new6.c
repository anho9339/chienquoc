...
//        set("no_sk04237",1);	//玄龙决对其没有效果
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"Nghe nói Sói Võ Sư đã xuất hiện tại Trường Bạch Sơn. Các vị hãy mau đi thu phục nó!" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	...
	{		
		if ( gone_time(saytime) >=60 )
		{
			...
			if ( random(2) )
			{
				...
				{
					case 0:
						SAY_CMD->say(me, "Đừng ép ta phải tặng quà thầy cô nữa!"); 
						break;
					case 1:
				...
				/*	case 2:
						...
						break; */
					default:
						SAY_CMD->say(me, "Đánh bại được ta rồi hãy nói!!"); 
						break;
				}
			}
		}
...
void win_bonus_callout( object me, object who )
{
	...
	CHAT_D->rumor_channel( 0, CHAT+"Nghe nói Sói Võ Sư đã bị tiêu diệt" ); 
...
{
	send_user( who, "%c%c%w%s", ':',3,8250,"Sói Võ Sư:\n    Đừng hòng ép ta đi tặng quà 20-11 yayaya!!!\n"+
