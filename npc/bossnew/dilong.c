...
//        set("no_sk04237",1);	//玄龙决对其没有效果
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->vip_channel( 0, CHAT+"Ầm ầm! Sấm chớp nổi dậy ở "HIY"Yên Trường Thành 1"NOR" xuất hiện yêu khí kì quái, phải chăng đó là "HIB"Dị Long"NOR" !!!" );
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	log_file("boss.txt",short_time()+" Dị Long xuất hiện\n");
}
	...
	{		
		if ( gone_time(saytime) >=60 )
		{
			...
			if ( random(2) )
			{
				...
				{
					case 0:
						SAY_CMD->say(me, "Nộ khí Thần Long, hào khí quật khởi!"); 
						break;
					case 1:
						SAY_CMD->say(me, "Long Thần là ta, không ai được coi ta như quái vật dị dạng!"); 
						break;
				/*	case 2:
						SAY_CMD->say(me, enemy->get_name()+", thần binh mau mau ra giúp ta!!"); 
						break; */
					default:
						SAY_CMD->say(me, "Tới đây lũ người ngu ngốc!"); 
						break;
				}
			}
		}
...
//		add_to_scene(174,190,160);
	...
	if ( objectp(enemy=get_enemy()) && gone_time(leiTime) > 40 )
	{
		...
		if ( !lowhp&&random(100)<40 )	
 			return "skill/11/11019"->perform_action_npc(me, 0);
		if ( lowhp&&random(100)<60 )	
 			return "skill/11/11018"->perform_action_npc(me, 0);
...
/*void summon_guard()
{
	...
	for(i=0;i<100;i++)
	{
		...
		if ( !p )
			continue;
		npc = new("npc/boss/bossthienson/thienbinhhotro");
	...
	for(i=0;i<100;i++)
	{
		...
		if ( !p )
			continue;
		npc = new("npc/boss/bossthienson/thienlangthantuong");		
		if ( !npc )
			break;
		npc->add_to_scene( z, p / 1000, p % 1000);
		npc->set_owner(me);
...
/*	if ( lowhp == 0 && get_hp() * 100 / get_max_hp() < 40 )	//生命值为40％以下
	{
		...
		if ( summon == 0 )
			summon_guard();
		set_lowhp_status();
		SAY_CMD->say(me, "Hãy mau ra trợ giúp ta!"); 
	} */
...
void win_bonus_callout( object me, object who )
{
	...
	CHAT_D->vip_channel( 0, CHAT+"Nghe nói Dị Long đã bị các anh hùng Chiến Quốc Tâm Diện tiêu diệt!" ); 
	log_file("boss.txt",short_time()+" Dị Long bị tiêu diệt\n");
...
//	if ( rate < rate1 )
	{
		...
		// Random tỉ lệ, số nhỏ là ra nhiều
		...
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
	                {
		                switch( random(5) )
		                {
		               case 0 : item = new( "/item/nangcap/dathan" );  break; 
		               case 1 : item = new( "/item/nangcap/banhngot" );  break; 	
                       case 2 : item = new( "/item/nangcap/canhpp/ngoctiende" );  break; 	
                       case 3 : item = new( "/item/nangcap/canhpp/lenhthienton" );  break; 	
                       case 4 : item = new( "/item/nangcap/canhpp/canhthienthan" ); break;					   
		                }
	...
	else if ( rate < 4400  * rate1 / 100 )		//TQVS
	{
	...
	else if ( rate < 8888  * rate1 / 100)	//随机技能进阶三本
	{
	         for(i=0;i<1;i++)
	         {
	                if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
...
{
	send_user( who, "%c%c%w%s", ':',3,8287,"Dị Long:\n    Ngươi nghĩ ngươi biết nhiều sao tên Phàm Trần kia? Tính tới đây thách đố ta à?!!!\n"+
