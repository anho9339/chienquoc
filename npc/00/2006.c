...
inherit OFFICER;
mapping mpBox = ([
			141127080: ([ 32*5 +19 : ([ "legend2" : 32*5 +20,"obj" : "item/98/0019" ,"taskid":TID_WEIGUO,"taskno":15,"taskname":"裁缝的梦想(完成)"])]),
		]);
// 函数：获取人物造型
int get_char_picid() { return 2500; }
...
void do_look( object who )
{
	...
	z = get_z(who);x=get_x(this_object());y=get_y(this_object());
	{
		...
		for(i=0,size=sizeof(nLegend);i<size;i++)
		{
			...
			send_user(who, "%c%c%w%s", 0x5a, 0, 10, "采集中……");
