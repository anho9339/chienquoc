...
// 函数：获取人物造型
int get_char_picid() { return 2101; }
...
void do_look( object who )
{
	...
	{
		send_user(who, "%c%s", ';', "Thời gian không khoẻ mạnh không thể thu thập");
		return;
	}	
	...
	if (!item) 
	{
		send_user( who, "%c%s", '!', sprintf("Bạn không có Công Cụ Thái Dã %d cấp !", TOOL_LEVEL));
		return;
	}
