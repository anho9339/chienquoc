...
void do_look( object who )
{
	...
	{
		send_user( who, "%c%s", '!', sprintf("Kỹ năng %s không đủ %d cấp!", PF_NAME, SKILL_LEVEL));
		return;
	}
	if (who->get_skill_degree(THIS_SKILL)<SKILL_EXP) 
	{
		send_user( who, "%c%s", '!', sprintf("Kỹ năng %s độ thuần thục không đủ %d!", PF_NAME, SKILL_EXP));
		return;
	}
	...
	if (!item) 
	{
		send_user( who, "%c%s", '!', sprintf("Không có %d cấp công cụ %s!", TOOL_LEVEL, PF_NAME));
		return;
	}
	...
	send_user(who, "%c%c%w%s", 0x5a, 0, 6, PF_NAME+"Đang...");	
...
void into_effect( object who, string arg )
{
	...
	if (item)
	{
		...
		printf( ECHO "Bạn nhận được %d cái %s.", item->get_amount(), item->get_name() );
