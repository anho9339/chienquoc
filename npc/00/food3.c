...
#define THIS_SKILL	610
#define SKILL_LEVEL	1
#define SKILL_EXP	0
#define RESET_TIME	600
#define TOOL_LEVEL	0
#define STUFF_NUMBER	144
#define PF_NAME		"Đầu Bếp"
#define TOOL_TYPE	21
#define RESET_FILE	"/sys/item/stuff3"

// 函数：获取人物造型
int get_char_picid() { return 2603; }
...
void do_look( object who )
{
	...
	{
		send_user( who, "%c%s", '!', sprintf("Kĩ năng %s của bạn không đủ %d cấp !", PF_NAME, SKILL_LEVEL));
		return;
	}
	if (who->get_skill_degree(THIS_SKILL)<SKILL_EXP) 
	{
		send_user( who, "%c%s", '!', sprintf("Kĩ năng %s của bạn không đủ %d thuần thục !", PF_NAME, SKILL_EXP));
		return;
	}
	...
	if (!item) 
	{
		send_user( who, "%c%s", '!', sprintf("Bạn không có %d cấp công cụ %s !", TOOL_LEVEL, PF_NAME));
		return;
	}
	...
	send_user(who, "%c%c%w%s", 0x5a, 0, 6, PF_NAME+"Lấy....");	
