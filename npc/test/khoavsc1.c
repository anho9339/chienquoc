...
void do_look(object who)
{
	...
	ESC"%s\ntalk %x# gift.2\n"
	ESC"Rời khỏi\nCLOSE\n",khoa1,khoa1,khoa2,khoa3,khoa,
	"/sys/party/vstfix"->get_mokhoa1() == 0 ? "Ta muốn khởi động Khoá" : "Ta chỉ đến xem",getoid(me)));
}
...
void do_gift2( object me, string arg )
{
	...
	object who;	
		...
		if ( "/sys/party/vstfix"->get_mokhoa1() > 0 ) return;
...
int into_effect( object who , string arg)
{
	...
	send_user(who, "%c%c%c", 0x5a, 1, 1);
		...
		if ( "/sys/party/vstfix"->get_mokhoa1() < 1 )
		{
			if ( "/sys/party/vstfix"->checkctmk() != 1 ) return 0;
			if ( "/sys/party/vstfix"->check_war_time() != 1 ) return 0;
		"/sys/party/vstfix"->add_mokhoa(1);
		"/sys/party/vstfix"->set_mokhoa1(1);
		notify("Bạn đã mở chìa khoá Ất thành công !");
		USER_D->user_channel(sprintf("Chìa khoá Ất đã bị %s "HIR "khởi động "NOR "!", who->get_name() ));
