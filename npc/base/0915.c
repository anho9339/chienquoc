...
void do_talk_exchange()
{
	TASK_COLLECT_D->do_talk_exchange( this_player(), this_object(),1015 );
}
...
//        tmp += sprintf(ESC"换取食物\ntalk %x# exchange\n",id);
        ...
        tmp += ESC"Rời khỏi\nCLOSE\n";
        send_user(who,"%c%s",':'," "+get_name()+" :\n Mở Tửu Lâu trên đảo này cũng được mà! Đồ ngon quí giá, thứ gì cũng có!\nKhách quan, cần đổi gì không?\n"+ tmp);
}
...
void do_exchange(string sArg)
{
	TASK_COLLECT_D->do_exchange(sArg,1015);
}
...
void do_inputx(string sArg)
{
	object me,who;
	me = this_object(); who = this_player();
	TASK_COLLECT_D->do_inputx(me,who,sArg,1015);
}