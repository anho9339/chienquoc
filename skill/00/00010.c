...
#define THIS_PERFORM    00010
#define PF_START       "00010"
#define PF_NAME        "Tuyết Nội Nhi"
...
string get_desc( object me )
{
 ...
 //       if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 60 cấp\n Vật phẩm tiêu hao: Bùa Thần Tiên\n    (Nội) Triệu hồi Tuyết Nhi hỗ trợ, có tỉ lệ nhất định làm choáng mục tiêu.";
//        cur_level = (level - SKILL_LEVEL)/10 + 1;
        result = sprintf(HIC" %s (Cấp 60 )\n " NOR "Võ công yêu cầu: Cấp 60 \n Vật phẩm tiêu hao: Bùa Thần Tiên\n Tái sử dụng sau: %d giây\n   (Nội) Triệu hồi Tuyết Nhi hỗ trợ chiến đấu. Khi Tuyết Nhi đánh có tỉ lệ nhất định làm choáng mục tiêu.\n ",
        	PF_NAME, 2 );
	return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
	...
	{
	    printf( ECHO "Phái của bạn là phái ngoại mà?" );
		return 1;
	} 
	if (me->get_fam_name()=="Cấm Vệ Quân")
	{
	    printf( ECHO "Phái của bạn là phái ngoại mà?" );
		return 1;
	} 
