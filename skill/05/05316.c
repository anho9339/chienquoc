...
#define THIS_PERFORM    05316
#define PF_START       "05316"
#define PF_TIME        "05316#"
#define PF_NAME        "Thần Công Vô Địch"
...
string get_desc( object me )
{
        ...
        return sprintf( HIY"[" PF_NAME "] \nPháp lực tiêu hao: %d \nTái sử dụng sau: %d giây \nYêu cầu sử dụng : %s%d cấp \n Liên tục trong 60 giây áp sát mục tiêu gây ra sát thương..Lực nội công càng cao thì sát thương càng lớn. \n",
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("05316")==0) return 1;
...
void effect_hurt_done( object me )
{
        ...
        int z, x, y;
        int level, mp, mp2, ap, cp, damage, damage2, rate, i, size, p;
...
//              send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 36321, 3, UNDER_FOOT, PF_ONCE );
		...
		if (p = me->get_skill(0432)) damage = damage * (100+p*2)/100+7*p;		// 帮派的伤害强化
                damage2 = CHAR_FIGHT_D->correct_damage_2(me, who, damage, cp);
		if (p = who->get_skill(0433)) damage2 = damage2 * (100 - p*2)/100 - 7*p;		// 帮派的抵抗强化
