...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("05918")==0) return 1;
...
//		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 2710, 1, OVER_BODY, PF_ONCE );
		...
		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 40821, 2, OVER_BODY, PF_ONCE );
	//	send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 23211, 2, OVER_BODY, PF_ONCE );
...
//	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 5912, 1, OVER_BODY, PF_ONCE );
    send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 40821, 2, OVER_BODY, PF_ONCE );
...
//	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 23211, 1, OVER_TILE, PF_LOOP );
	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 23211, 1, OVER_BODY, PF_ONCE );
