...
#define THIS_PERFORM    05915
#define PF_START       "05915"
#define PF_TIME        "05915#"
#define PF_NAME        "Kiếm Trận"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   30
#define SUB_MP          ( 1000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 1 )
#define ADD_CP          ( 400 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 700 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: 150 cấp\n Trận pháp kiếm trận,Tăng 400 Nội Công.Có xác xuất 40％ gây choáng 2 giây.", PF_NAME);
        cur_level = (level - SKILL_LEVEL)/150 + 1;
		size = 9 + (level/ 80 )*400;
        result = sprintf(HIY" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Kiếm Khí tung hoành giải phóng phạm vi rộng,Tăng %d Nội công sát thương tất cả mục tiêu xung quanh.Có xác xuất 40％ gây choáng 2 giây.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n  Tăng %d Nội Công。", 
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_CP, cur_level*150+SKILL_LEVEL, ADD_CP + 700, 9 + ((cur_level*150+SKILL_LEVEL)/ 80 )*400 );
        return result;  
}
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("05915")==0) return 1;
...
int cast_done( object me )
{
        ...
        me->set_enemy(0);        
        send_user(me, "%c%c%w%s", 0x5a, 0, 10, "Kiếm Trận……");
        return 2;    // 执行成功
}
	...
	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%d%c", 0x50, getoid(me), time2(), MAGIC_ACT_END );
		...
		if (count<=100)
                	/*send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%w%c%c%c", 0x6f, getoid(who), 
                        	42421, 2, OVER_BODY, 42422, 2, OVER_BODY, PF_ONCE );*/
		send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 2710, 2, OVER_BODY, PF_ONCE );
		...
		send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 51141, 3, OVER_BODY, PF_ONCE );
