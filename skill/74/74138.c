...
#include <action.h>
#include <effect.h>
#define PF_START       "74138"
#define TIME_INTERVAL   20

// 函数：宠物技能
int is_pet_skill() { return 1; }
// 函数：获取技能ID
int get_id() { return 74138; }
// 函数：获取技能类型
string get_name() { return "Đóng Băng"; }
// 函数：获取技能类型（0、被动；1、主动）
...
{
	return HIC+get_name()+"\n Kỹ năng của Bảo thú, Sử dụng có thể làm mục tiêu bị đóng băng trong 3 giây \ntái sử dụng 20 giây";
}
// 函数：获取指令类型
...
int perform_action_npc( object me, object who )
{
	...
	if( gone_time( me->get_perform(PF_START) ) < TIME_INTERVAL ) return 0;
        ...
        if ( gone_time(me->get("sk74138")) < 20 ) return 0;
...
int cast_done_npc( object me )
{
	...
	string arg;
        ...
        send_user( user, "%c%d%w%c%c%c", 0x6f, getoid(who), 23131, 1, OVER_BODY, PF_ONCE );
	if (who->is_anti_effect()) return 2;
        if(     get_effect(who, EFFECT_CHAR_LAZY)
        ||      get_effect(who, EFFECT_CHAR_LAZY_0) ) return 2;    // 执行成功
        set_effect(who, EFFECT_CHAR_LAZY, 3);
        send_user( who, "%c%w%w%c", 0x81, 9013, get_effect_3(who, EFFECT_CHAR_LAZY), EFFECT_BAD );
        send_user( user - ({ who }), "%c%d%w%c%c", 0x83, getoid(who), 9013, 1, EFFECT_BAD );
