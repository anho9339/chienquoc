...
// 函数：获取技能ID
int get_id() { return 74103; }
// 函数：获取技能类型
string get_name() { return "Ngự Độc Thuật"; }
// 函数：获取技能类型（0、被动；1、主动）
...
{
	return HIC+get_name()+"\n Kỹ năng của Bảo thú, miễn nhiễm với các loại Độc.\n";
}
// 函数：获取指令类型
...
{
	pet->set("sk74103",pet->get_anti_poison());
	pet->set_anti_poison(100);
}
//删除技能的影响
void remove_skill_effect(object pet,object owner)
{
	pet->set_anti_poison(pet->get("sk74103"));
	pet->delete("sk74103");
}