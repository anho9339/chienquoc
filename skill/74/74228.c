...
// 函数：获取技能ID
int get_id() { return 74228; }
// 函数：获取技能类型
string get_name() { return "Phiên Vân Phúc Vũ"; }
// 函数：获取技能类型（0、被动；1、主动）
...
{
	return HIC+get_name()+"\n Kỹ năng của Bảo thú, Sử dụng thuỷ công tấn công mục tiêu phạm vị 3x3.";
}
// 函数：获取指令类型
	...
	string arg;
        ...
        if ( !(size=sizeof(char)) )
        {
                send_user( get_scene_object(z, x, y, USER_TYPE), "%c%w%w%w%c%c%w%c%c%c", 0x4f, x, y, 
                        2515, 1, OVER_BODY, 2515, 1, OVER_BODY, PF_ONCE );
		return ;        	
        }
	for(i=0;i<size;i++)
	{
		...
		if( !inside_screen_2(me, who) ) continue;
	        ...
	        send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%w%c%c%c", 0x6f, getoid(who), 
	                2515, 1, OVER_BODY, 2515, 1, UNDER_FOOT, PF_ONCE );
		if ( rate=who->get("anti_water") )	//抗水
