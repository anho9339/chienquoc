...
#define THIS_SKILL      11018
#define THIS_PERFORM    11018
#define PF_NAME        "Huyền Chân Hỏa"
...
int perform_action_npc( object me, object who )
{
	...
	SAY_CMD->say(me, "Hãy đỡ Huyền Chân Hỏa của ta, loài người phàm đáng chết!!"); 
	...
	for(i=0;i<121;i++)		//发送效果
        {
        	...
        	size++;
                send_user( get_scene_object(z, x0, y0, USER_TYPE), "%c%w%w%w%c%c%w%c%c%c", 0x4f, x0, y0, 
                        9026, 1, OVER_BODY, 9026, 1, OVER_BODY, PF_ONCE );
		if ( size>=5 )
			break;                  
        }
