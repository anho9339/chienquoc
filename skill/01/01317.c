...
#define THIS_PERFORM    01317
#define PF_START       "01317"
#define PF_NAME        "Bery Thập Bát Chưởng"
...
string get_desc( object me )
{
        ...
        result = sprintf(HIY" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Tuyệt kỹ độc môn do Ben và Bery sáng tạo,gia tăng %d ngoại công khi gọi thập bát long công kích đối thủ và có 50％ gây chậm chạp.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Gia tăng %d ngoại công。",
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("01317")==0) return 1;
...
//        send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 5116, 1, OVER_BODY, PF_ONCE );
		...
		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 42332, 3, OVER_BODY, PF_ONCE );
		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 42351, 3, OVER_BODY, PF_ONCE );
		...
		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 25161, 1, OVER_BODY, PF_ONCE );
...
//		send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 5116, 1, OVER_BODY, PF_ONCE );
		...
		send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 42351, 3, OVER_BODY, PF_ONCE );
