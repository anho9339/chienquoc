...
#define THIS_PERFORM    01312
#define PF_START       "01312"
#define PF_NAME        "Kiếm Bay Vèo Vèo"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 1\n Tấn công nhiều mục tiêu và có 20％ xác xuất gây choáng 1 giây.", PF_NAME);
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIY" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Tuyệt kĩ độc môn do Ben và Bery nghiên cứu sáng tạo,Tăng %d nội công dùng kiếm khí tấn công nhiều mục tiêu và có xác xuất 20％ gây choáng 1 giây.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Gia tăng %d nội công。",
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("01312")==0) return 1;
...
//		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 2710, 1, OVER_BODY, PF_ONCE );
//		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 5912, 1, OVER_BODY, PF_ONCE );
		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 41721, 2, OVER_BODY, PF_ONCE );
//		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 41722, 2, OVER_BODY, PF_ONCE );
        send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 10101, 1, OVER_BODY, PF_ONCE );
	...
	count = 0;
        for( i = 0, size = sizeof(char); i < size; i ++ ) 
                if( who = char[i] ) 
                {
                	...
                	count ++;
                	if (count<=20)
//   	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 2710, 1, OVER_BODY, PF_ONCE );
//	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 5912, 1, OVER_BODY, PF_ONCE );
    send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 41721, 2, OVER_BODY, PF_ONCE );
//	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 41722, 2, OVER_BODY, PF_ONCE );
	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 10101, 1, OVER_BODY, PF_ONCE );
	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 5912, 1, OVER_TILE, PF_LOOP );
	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%d%w%w%d%w%c%c%w%c%c", 0x8a, getoid(me), time2(),
                get_x(me), get_y(me), getoid(who), 5116, 1, OVER_BODY  );
