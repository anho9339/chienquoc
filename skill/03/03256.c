...
#define THIS_PERFORM    03256
#define PF_START       "03256"
#define PF_NAME        "Thiên Địa Kiếm Khí"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 150\n Thục Sơn tuyệt kỹ, triệu hồi kiếm khí tấn công liên hoàn mục tiêu và có 22%% gây choáng 2 giây (Thiên Địa Kiếm Khí đánh rộng 6x6 ô).", PF_NAME);
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIY" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Thục Sơn tuyệt kỹ, tăng %d nội công khi triệu hồi kiếm khí tấn công liên hoàn mục tiêu và có 22%% gây choáng 2 giây (Thiên Địa Kiếm Khí đánh rộng 6x6 ô).\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Gia tăng %d nội công.",
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("03256")==0) return 1;
...
//		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 42321, 3, OVER_BODY, PF_ONCE );
		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 5011, 1, OVER_BODY, PF_ONCE );
		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 50119, 2, OVER_BODY, PF_ONCE );
	...
	count = 0;
        for( i = 0, size = sizeof(char); i < size; i ++ ) 
                if( who = char[i] ) 
                {
                	...
                	count ++;
                	if (count<=1)
//		send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 51144, 1, OVER_BODY, PF_ONCE );
...
//		send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 42321, 3, OVER_BODY, PF_ONCE );
		...
		send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 50119, 2, OVER_BODY, PF_ONCE );
	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%d%w%w%d%w%c%c%w%c%c", 0x8a, getoid(me), time2(),
                get_x(me), get_y(me), getoid(who), 5011, 1, OVER_BODY  );//42511
