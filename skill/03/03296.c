...
#define THIS_PERFORM    03296
#define PF_START       "03296"
#define PF_TIME        "03296#"
#define PF_NAME        "Vận Chuyển Càn Khôn"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   1
#define SUB_MP          ( 0 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 1 )
#define ADD_EFFECT      ( 10000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 120000 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n    Tuyệt kĩ xoay chuyển Càn khôn,Gia tăng 150000 Khí Huyết trong 60 phút";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Tuyệt kĩ xoay chuyển Càn khôn,Gia tăng %d Khí Huyết trong 60 phút。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Gia tăng %d Khí Huyết trong 60 phút。",
...
int main( object me, object who, int x, int y, string arg )
{
	...
	object owner;
		...
		if (me->get_save("03296")==0) return 1;
