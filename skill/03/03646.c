...
#define THIS_PERFORM    03646
#define PF_START       "03646"
#define PF_TIME        "03646#"
#define PF_NAME        "Hoành Hoành Phi Phi"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   60
#define SUB_MP          50 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/150*50
#define ADD_AP		150000 + (me->get_skill(THIS_SKILL) - 8)/150*10000
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n    Gia tăng tối đa Ngoại Kháng và Nội Kháng để giảm sát thương từ người khác";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây \n    Lăng Ba Cường Hoá gia tăng %d Ngoại Kháng và Nội Kháng của bản thân trong 4 giây\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Lăng Ba Cường Hoá gia tăng %d Ngoại Kháng và Nội Kháng của bản thân trong 4 giây",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_AP, cur_level*150+SKILL_LEVEL, ADD_AP+22 );
	return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
        ...
        if (me->get_save("03646")==0) return 1;
...
int cast_done( object me )
{
        ...
        char = get_range_object(z, x, y, 0, USER_TYPE) + get_range_object(z, x, y, 0, CHAR_TYPE);        
        size = sizeof(char);
