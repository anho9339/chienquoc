...
#define THIS_PERFORM    03645
#define PF_START       "03645"
#define PF_TIME        "03645#"
#define PF_NAME        "Từ Bi Tế Vũ"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   10
#define SUB_MP          ( 200 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 2 )
#define ADD_EFFECT      ( 500 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 500 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n    Dùng ngân châm đả thông kinh mạch, trong 60 phút có thể tăng thêm 500 điểm Ngoại Công cho bản thân và đồng đội";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIR " %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Dùng ngân châm đả thông kinh mạch, trong 60 phút có thể tăng thêm %d điểm Ngoại Công cho bản thân và đồng đội.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Liên tục 60 phút Tăng thêm %d Ngoại Công.",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_EFFECT, cur_level*150+SKILL_LEVEL, ADD_EFFECT+500 );
	return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
	...
	object owner;
		...
		if (me->get_save("03645")==0) return 1;
...
int cast_done( object me )
{
	...
	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 32411, 1, OVER_BODY, PF_ONCE );
