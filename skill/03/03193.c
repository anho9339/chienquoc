...
#define THIS_PERFORM    03193
#define PF_START       "03193"
#define PF_TIME        "03193#"
#define PF_NAME        "Truy Viễn Thiên Hoang"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 150\n Chiêu thức độc môn của Hoàng Lão Tà,xuất chiêu như ảo ảnh tấn công nhiều mục tiêu xung quanh.", PF_NAME);
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        size = 9 + (level/ 80 )*50;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Chiêu thức độc môn của Hoàng Lão Tà,xuất chiêu như ảo ảnh với %d nội công tấn công %d mục tiêu xung quanh.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng %d nội công tấn công %d mục tiêu xung quanh. ", 
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_CP, size, cur_level*150+SKILL_LEVEL, ADD_CP + 10,  9 + ((cur_level*150+SKILL_LEVEL)/ 80 )*50 );
        return result;	}
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("03193")==0) return 1;
...
int cast_done( object me )
{
        ...
        for( i = 0, size = sizeof(char); i < size; i ++ ) 
                if( who = char[i] ) 
                {
					ret = CHAR_FIGHT_D->attack_done(me, who, HIT_NONE, ADD_CP);
					ret += CHAR_FIGHT_D->attack_done(me, who, HIT_NONE, ADD_CP);
//					ret += CHAR_FIGHT_D->attack_done(me, who, HIT_NONE, ADD_CP);
//					ret += CHAR_FIGHT_D->attack_done(me, who, HIT_NONE, ADD_CP);
