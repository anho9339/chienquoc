...
#define THIS_PERFORM    03174
#define PF_START       "03174"
#define PF_TIME        "03174#"
#define PF_NAME        "Bất Tử Hộ Thể Linh Giáp"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   30
#define SUB_MP        	0
#define SUB_YUAN        50
#define ADD_EFFECT    ( 100000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 300000 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n    Dùng chân nguyên linh khí để hộ thể, trong 30 phút có thể phòng thủ được 100000 điểm sát thương";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Nguyên khí tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Dùng chân nguyên linh khí để hộ thể, trong 30 phút có thể phòng thủ được %d điểm sát thương。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Trong 30 phút có thể phòng thủ được %d điểm sát thương。",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_EFFECT, cur_level*150+SKILL_LEVEL, ADD_EFFECT+300000 );
	return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("03174")==0) return 1;
...
int cast_done( object me )
{
	...
	if (who->is_user()) who->set_save("05911", ADD_EFFECT);
        ...
        set_effect(who, EFFECT_CHAR_MISS2, 1800);
