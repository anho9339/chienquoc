...
#define THIS_PERFORM    02714
#define PF_START       "02714"
#define PF_NAME        "Thiên Ngoại Chi Tinh"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 150\n Mao Sơn tuyệt kĩ độc môn chi truyền,dùng nội kình đẩy mục tiêu có 25%% xác xuất gây choáng 2 giây.", PF_NAME);
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIY" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Mao Sơn tuyệt kĩ độc môn chi truyền,Tăng %d nội công dùng nội kình đẩy hồn nhiều mục tiêu và có 25%% xác xuất gây choáng 2 giây.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Gia tăng %d nội công.",
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("02714")==0) return 1;
...
//		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 51141, 1, OVER_BODY, PF_ONCE );
        send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 51142, 1, OVER_BODY, PF_ONCE );
		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 41322, 1, OVER_BODY, PF_ONCE );
	//	send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 27111, 2, OVER_BODY, PF_ONCE );
...
//   	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 5116, 1, OVER_BODY, PF_ONCE );
	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 51142, 1, OVER_BODY, PF_ONCE );
    send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 41322, 1, OVER_BODY, PF_ONCE );
	//send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 27111, 2, OVER_BODY, PF_ONCE ); // 41322
