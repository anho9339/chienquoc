...
#define THIS_PERFORM    02318
#define PF_START       "02318"
#define PF_TIME        "02318#"
#define PF_NAME        "Thiên Địa Vô Cực"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   5
#define SUB_MP          ( 1000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 1 )
#define ADD_AP          ( 350 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 1500 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n    Thiên địa tuần hoàn,vô cực hữu ý.Tăng 350 điểm Nội Công sát thương một mục tiêu liên tục và làm mục tiêu choáng 1 giây khi đánh trúng。";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIY" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Thiên địa tuần hoàn,vô cực hữu ý.Tăng %d điểm Nội Công sát thương một mục tiêu liên tục và làm mục tiêu choáng 1 giây khi đánh trúng。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng thêm %d điểm Nội Công。", 
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_AP, cur_level*150+SKILL_LEVEL, ADD_AP + 1500);     	
        return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("02318")==0) return 1;
...
int cast_done( object me )
{
		...
		if( sscanf(arg, "%d,%d,%d", z, x, y) == 3 )
        {
               send_user( get_scene_object(z, x, y, USER_TYPE), "%c%w%w%w%c%c%w%c%c%c", 0x4f, x, y, 
                        42421, 8, OVER_BODY, 42511, 6, UNDER_FOOT, PF_ONCE );
...
/*        send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%w%c%c%w%c%c%c", 0x6f, getoid(who), 
                42421, 3, OVER_BODY, 5116, 2, UNDER_FOOT, 50119, 4, UNDER_FOOT, PF_ONCE );*/
//		send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 4171, 2, OVER_BODY, PF_ONCE );
