...
#define THIS_PERFORM    02221
#define PF_START       "02221"
#define PF_NAME        "Lãnh Nguyệt Tâm Pháp"
...
//        send_user( me, "%c%w%c%w", 0x50, 0, 0xff, 2 );    // 所有出招２秒后能使用
        ...
        send_user( me, "%c%w%c%w", 0x50, THIS_PERFORM, 0xff, 0 );
        me->set_perform( PF_START, time() );        
	send_user(me, "%c%c%w%s", 0x5a, 0, 3, "Đang thi hành……");
        return 1;
}
...
int into_effect( object me , string arg)
{
	...
	me->set_perform("02221#", time() );
        CHAR_CHAR_D->init_loop_perform(me);
        send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me),
                3292, 1, OVER_BODY, PF_ONCE );
        send_user( me, "%c%w%w%c", 0x81, 2221, 30000, EFFECT_GOOD );
        send_user( get_scene_object_2(me, USER_TYPE) - ({ me }), "%c%d%w%c%c", 0x83, getoid(me), 2221, 1, EFFECT_GOOD);
        return 2;    // 执行成功
}
...
{
        return  PF_NAME ":\n　　Miễn nhiếm với trạng thái trầm mặc\n";
}
