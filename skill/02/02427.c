...
#define THIS_PERFORM    02427
#define PF_START       "02427"
#define PF_NAME        "Vân Long Kích"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 150\n Tuyệt kỹ độc môn cuối cùng của Cấm Vệ Quân, thương pháp như rồng bay hút lấy đối thủ và có 10%% gây choáng 2 giây.", PF_NAME);
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIY" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm \n Nguyên khí tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Tuyệt kỹ độc môn cuối cùng của Cấm Vệ Quân,tăng %d sát thương và thương pháp như rồng bay hút lấy đối thủ và có 10%% gây choáng 2 giây.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Gia tăng %d ngoại công.",
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("02427")==0) return 1;
        ...
        {
                printf( ECHO "Sử dụng \"" PF_NAME "\" phải trang bị binh khí." );
                return 1;
        }
...
//		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 51141, 1, OVER_BODY, PF_ONCE );
        send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 3642, 1, OVER_BODY, PF_ONCE );
//		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 2416, 1, OVER_BODY, PF_ONCE );
//		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 24121, 1, OVER_BODY, PF_ONCE );
...
//		send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 42332, 2, OVER_BODY, PF_ONCE );
		send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 3642, 1, OVER_BODY, PF_ONCE );
//		send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 2416, 1, OVER_BODY, PF_ONCE );
