...
#define THIS_PERFORM    02417
#define PF_START       "02417"
#define PF_TIME        "02417#"
#define PF_NAME        "Ám Ngoại Chi Tinh"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   5
#define SUB_MP          0
#define SUB_YUAN        30
#define ADD_RATE        100 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/150*100
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 150\n Thương pháp lừng danh giang hồ một thời, tăng thêm 100％ độ chính xác khi tấn công mục tiêu.", PF_NAME);
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Nguyên khí tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Thương pháp lừng danh giang hồ một thời, tăng thêm %d ％ độ chính xác khi tấn công mục tiêu.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: "HIR "Cấp %d \n" NOR " Tăng thêm %d％ độ chính xác。", 
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_YUAN, TIME_INTERVAL, ADD_RATE, cur_level*150+SKILL_LEVEL, ADD_RATE + 100);
        return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("02417")==0) return 1;
