...
#define THIS_PERFORM    02619
#define PF_START       "02619"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 150\n Tuyệt kĩ phóng ám khí công kích như mưa hoa ngập trời, sát thương tất cả mục tiêu và có 40%% gây choáng trong phạm vi nhìn thấy.", PF_NAME);
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIY" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Tuyệt kĩ phóng ám khí như mưa bão với sức mạnh không thể lường nỗi, tăng %d ngoại công sát thương tất cả mục tiêu và có 40%% gây choáng trong phạm vi nhìn thấy.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Gia tăng %d ngoại công.",
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("02619")==0) return 1;
...
//   	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 5116, 1, OVER_BODY, PF_ONCE );
	...
	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%d%w%w%d%w%c%c%w%c%c", 0x8a, getoid(me), time2(),
                get_x(me), get_y(me), getoid(who), 32531, 1, OVER_BODY  );//51141
