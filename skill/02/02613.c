...
#define THIS_PERFORM    02613
#define PF_START       "02613"
#define PF_NAME        "Song Long Đoạt Châu"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 36\n Sát chiêu tàn độc phóng ám khí 2 lần liên tiếp sát thương mục tiêu, đồng thời làm đối thủ bị Mù Lòa trong 6 giây.", PF_NAME);
        result = sprintf(HIC" %s \n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Sát chiêu tàn độc phóng ám khí 2 lần liên tiếp sát thương mục tiêu, đồng thời làm đối thủ bị Mù Lòa trong 6 giây.", 
...
//              send_user( get_scene_object(z, x, y, USER_TYPE), "%c%w%w%w%c%c%c", 0x4f, x, y, 26121, 1, OVER_BODY, PF_ONCE );
	...
	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 26139, 1, OVER_BODY, PF_ONCE );
        send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 26131, 1, OVER_BODY, PF_ONCE );
