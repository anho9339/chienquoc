...
#define PF_NAME        "Bạo Vũ Tiêu Diêu"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 100\n Tuyệt kĩ phóng ám khí như hoả dược bùng nổ, sát thương tất cả mục tiêu và có 5%% gây choáng trong phạm vi nhìn thấy.", PF_NAME);
		...
		result = sprintf(HIY" %s ( Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Tuyệt kĩ phóng ám khí như hoả dược bùng nổ, tăng %d ngoại công sát thương tất cả mục tiêu và có %d%% gây choáng trong phạm vi nhìn thấy.\n " HIC"Cấp tiếp theo:\n "NOR "Yêu cầu độ tu luyện: " HIR "%d " NOR "\n    Tăng tỷ lệ gây choáng lên %d%%.",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_AP, cur_level*5, cur_level*CAP_SKILL+10000, cur_level*5+5 );
		else if ( cur_level >= 10 )
		result = sprintf(HIY" %s ( %s )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Tuyệt kĩ phóng ám khí như hoả dược bùng nổ, tăng %d ngoại công sát thương tất cả mục tiêu và có %d%% gây choáng trong phạm vi nhìn thấy.",
        	PF_NAME, "giới hạn", SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_AP, cur_level*5 );
	return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
	...
	//	if (me->get_save("02610")==0) return 1;
		...
		{
				printf( ECHO "Phái Đường Môn mới có thể sử dụng \"" PF_NAME "\"" );
                return 1;
		}	
...
int cast_done( object me )
{
		...
		send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 2571, 2, OVER_BODY, PF_ONCE );
	...
	count = 0;
        for( i = 0, size = sizeof(char); i < size; i ++ ) 
                if( who = char[i] ) 
                {
                	...
                	count ++;
                	if (count<=20)
	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 2571, 2, OVER_BODY, PF_ONCE );
                	ret = CHAR_FIGHT_D->throwing_done(me, who, HIT_THROWING, ap);
					ret += CHAR_FIGHT_D->throwing_done(me, who, HIT_THROWING, ap);	
		        if ( who && !who->is_die() )
		        	CHAR_FIGHT_D->set_enmity(me,who,80);	
					
					if (ret>0 && who && random100()<cur_level*5)
	        {	        	
