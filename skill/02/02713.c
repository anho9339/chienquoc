...
#define THIS_PERFORM    02713
#define PF_START       "02713"
#define PF_TIME        "02713#"
#define PF_NAME        "Hấp Hồn Đoạt Phách"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   30
#define SUB_MP          ( 1000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 1 )
#define ADD_CP          ( 400 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 3500 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: 150 cấp\n Triệu hồi vết nứt từ đất đẩy hồn lìa khỏi xác,Tăng 400 Nội Công và có tỷ lệ 45％ làm đối thủ choáng 2 giây.", PF_NAME);
        cur_level = (level - SKILL_LEVEL)/150 + 1;
		size = 9 + (level/ 80 )*400;
        result = sprintf(HIY" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Triệu hồi vết nứt từ đất đẩy hồn lìa khỏi xác,Tăng %d Nội Công và có tỷ lệ 45％ làm đối thủ choáng 2 giây mỗi khi trúng đòn.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n  Tăng %d Nội Công。", 
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_CP, cur_level*150+SKILL_LEVEL, ADD_CP + 3500, 9 + ((cur_level*150+SKILL_LEVEL)/ 80 )*400 );
        return result;  
}
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("02713")==0) return 1;
...
int cast_done( object me )
{
        ...
        set_effect_2(me, EFFECT_PROGRESS, 10, 0);
        me->set_progress_file(__FILE__);
        me->set_enemy(0);        
        send_user(me, "%c%c%w%s", 0x5a, 0, 10, "Hấp Hồn Phách……");
        return 2;    // 执行成功
}
...
//       	send_user( get_scene_object_2(me, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(me), 5116, 3, OVER_BODY, PF_ONCE );
		send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 51142, 2, OVER_BODY, PF_ONCE );
...
//					send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%w%c%c%c", 0x6f, getoid(who), 
 //                       	5116, 1, OVER_BODY, 51142, 2, OVER_BODY, PF_ONCE );
