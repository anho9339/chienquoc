...
#define THIS_PERFORM    02815
#define PF_START       "02815"
#define PF_TIME        "02815#"
#define PF_NAME        "Bá Khí Phi Yến"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   30
#define SUB_MP          ( 3000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 1 )
#define ADD_AP          ( 300 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 500 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n    Xuất khí như mưa bão,Tăng 300 Nội Công và đánh trúng làm mục tiêu choáng trong 5 giây";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Vật phẩm tiêu hao: 1 triệu lá Bùa Giấy\n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Xuất khí như mưa bão,Tăng %d Nội Công và đánh trúng làm mục tiêu choáng trong 5 giây\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng thêm %d điểm Nội Công。", 
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_AP, cur_level*150+SKILL_LEVEL, ADD_AP + 500);     	
        return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("02815")==0) return 1;
...
int cast_done( object me )
{
        ...
        send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%w%c%c%w%c%c%c", 0x6f, getoid(who), 
                34831, 4, OVER_BODY, 5019, 3, UNDER_FOOT, 5011, 4, UNDER_FOOT, PF_ONCE );
...
//	if (who->get_perform("02222#")) return 2;
        ...
        if( ret > 0 ) 
        {
                ...
                set_effect(who, EFFECT_CHAR_FAINT, 5);
