...
#define THIS_PERFORM    04184
#define PF_START       "04184"
#define PF_TIME        "04184#"
#define PF_NAME        "Kim Chung Trạo Thần Công"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   5
#define SUB_MP          300 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/150*4
#define ADD_AP		1000 + (me->get_skill(THIS_SKILL) - 8)/150*15000
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n    Kim Chung Trạo tầng cuối cùng,với uy lực phòng thủ cực mạnh, Sử dụng sẽ buff cho bản thân và toàn nhóm trong phạm vi nhìn thấy.";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Vật phẩm tiêu hao: Bùa Giấy \n    Kim Chung Trạo tầng cuối cùng,với uy lực phòng thủ cực mạnh, Tăng %d Ngoại Kháng và Nội Kháng cho bản thân và toàn nhóm trong phạm vi nhìn thấy.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng %d Ngoại Kháng và Nội Kháng cho bản thân và toàn nhóm trong phạm vi nhìn thấy.",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_AP, cur_level*150+SKILL_LEVEL, ADD_AP+15000 );
	return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
        ...
        if (me->get_save("04184")==0) return 1;
...
int cast_done( object me )
{
	...
	for (i=0;i<size;i++) if (who = char[i])
	{
		...
		if (who->is_npc())
		{
	        ...
	        if ( level > level2+8) add = ADD_AP - (level-level2-8)/150*15000;
	        else add = ADD_AP;
...
void can_perform( object me )
{
	...
	send_user(me, "%c%w%w%c%c%c%c%c%s", 0x64, THIS_PERFORM, SUB_MP, 1, 1, MAGIC_ACT_BEGIN, 0, 5, "Bùa Giấy");
}
