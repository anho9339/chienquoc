...
#define BASIC_SKILL_1   0411
#define BASIC_SKILL_2   0421
#define THIS_SKILL      0429
#define THIS_PERFORM    04291
#define PF_START       "04291"
#define PF_NAME        "龙游四海"

#define SKILL_LEVEL     15
#define SKILL_LEVEL_2   15
#define SUB_MP          20
...
int main( object me, object who, int x, int y, string arg )    // 同步更改“灵犀环”
{
        ...
        if( !me->get_skill(BASIC_SKILL_1) && !me->get_skill(BASIC_SKILL_2) )
        {
                printf( ECHO "使用“" PF_NAME "”得先学会大乘道法。" );
                return 1;
        }
...
//      if( !objectp( who = find_char(arg) ) ) return 1;
        ...
        if( arg[<1] == '#' )    // 本帮总管(NPC)
        {
                ...
                &&    ( p = get_valid_xy( z, x0 + random(p) - p / 2, y0 + random(p) - p / 2, IS_CHAR_BLOCK) ) )
                {
                        ...
                        me->add_to_scene( z, x, y, get_d(me), 42911, 0, 42911, 0 );    // get_front_xy(x, y, x0, y0)
                }
                else    return 1;
        ...
        if(     random(100) < 20 + level * me->get_mp() / ( who->get_mp() + who->get_max_mp() )
        &&    ( p = 18 - level / 10 ) > 0
        &&    ( p = get_valid_xy( z, x0 + random(p) - p / 2, y0 + random(p) - p / 2, IS_CHAR_BLOCK) ) )
        {
                x = p / 1000;  y = p % 1000;
