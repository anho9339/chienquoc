...
#define BASIC_SKILL     0418
#define THIS_SKILL      0418
#define THIS_PERFORM    04185
#define PF_START       "04185"
#define PF_TIME        "04185#"
#define PF_NAME        "Lưu Huyết Thuật"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   3
#define SUB_MP        ( 0 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 1 )
#define ADD_EFFECT    ( 1000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 5000 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 10 cấp\n    Kỳ Thuật trị thương, trong 1 giây hồi phục 1000 điểm Khí Huyết cho đồng đội";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Kỳ Thuật trị thương, trong 10 giây hồi phục %d điểm Khí Huyết cho đồng đội。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Kỳ Thuật trị thương, trong 10 giây hồi phục %d điểm Khí Huyết cho đồng đội。",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_EFFECT*10, cur_level*150+SKILL_LEVEL, (ADD_EFFECT+10)*1000 );
	return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("04185")==0) return 1;
...
int cast_done( object me )
{
	...
	if( get_effect(who, EFFECT_CHAR_GOOD_BAD) )
        ...
        {        	
        	...
        	set_effect_2(who, EFFECT_CON_RECOVER, 10, 0);
