...
#define THIS_PERFORM    04256
#define PF_START       "04256"
#define PF_TIME        "04256#"
#define PF_NAME        "Ngịch Chuyển Tâm Kinh"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   10
#define SUB_MP          ( 0 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 1 )
#define SUB_EFFECT      ( 30 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 1000 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\    Tà thuật ăn mòn xương cốt, khiến đối thủ bị giảm 30％ hiệu quả tấn công đối với mình trong suốt 5 phút。";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Tà thuật ăn mòn xương cốt, khiến đối thủ bị giảm %d％ hiệu quả tấn công đối với mình trong suốt 5 phút。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tà thuật ăn mòn xương cốt, khiến đối thủ bị giảm %d％ hiệu quả tấn công đối với mình trong suốt 5 phút。",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, SUB_EFFECT, cur_level*150+SKILL_LEVEL, SUB_EFFECT + 1000 );
	return result;
}
...
string get_loop_desc( object me )
{
        if( get_effect(me, EFFECT_CHAR_ARMORFALL ) )
                return sprintf( PF_NAME "：\n　　Hiệu quả công kích giảm xuống %d ％。\n", me->get("pf.powerfall") );
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("04256")==0) return 1;
