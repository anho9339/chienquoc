...
#define THIS_PERFORM    04137
#define PF_START       "04137"
#define PF_NAME        "Linh Thiên Hoán Chú"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR" %s \n Võ công yêu cầu: Cấp 60\n Thu lại Linh thú của mình.", PF_NAME);
        result = sprintf(HIC" %s \n " NOR "Võ công yêu cầu: Cấp %d \n Tái sử dụng sau: %d giây\n Thu lại Linh thú của mình.", 
...
int main( object me, object who, int x, int y, string arg )
{
	...
	object * user, mstienlinh, * zombie;
	int flag = 0;
	int i, size;
	who = me->get("mstienlinh");
	if (!who) return 1;
	if (who->get_owner()!=me)
        {
