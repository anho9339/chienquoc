...
#define PF_NAME        "Triệu Hồi Quỷ Tốt"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 1\n Vật phẩm tiêu hao: 2 lá Bùa Giấy\n Triệu hồi Quỷ Tốt từ địa phủ có khả năng dùng Hỏa Diệm tấn công kẻ địch.", PF_NAME);
        cur_level = (level - SKILL_LEVEL)/10 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Vật phẩm tiêu hao: 2 lá Bùa Giấy\n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Triệu hồi Quỷ Tốt từ địa phủ có khả năng dùng Hỏa Diệm tấn công kẻ địch.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n Tăng Quỷ Tốt thêm 20 điểm Khí Huyết,\n Tăng Quỷ Tốt thêm 20 điểm Pháp Lực,\n Tăng Quỷ Tốt thêm 20 điểm Ngoại Công,\n    Tăng Quỷ Tốt thêm 300 điểm Nội Công\n    Tăng Quỷ Tốt thêm 60 điểm Ngoại Kháng:\n    Tăng Quỷ Tốt thêm 60 điểm Nội Kháng:\n    Tăng Quỷ Tốt thêm 55 điểm Thân Thủ.",
...
int main( object me, object who, int x, int y, string arg )
{
		...
		flag = -1;
                ...
                if( flag >= 0 ) printf( ECHO "Bạn thiết lập trạng thái của triệu hồi thú là \"%s\".", flag ? "Hành Động" : "Đợi Lệnh" );
...
int cast_done( object me )
{
        ...
        send_user(me, "%c%c%w%s", 0x5a, 0, 2, "Gọi Quỷ Tốt……");	                
...
int cast_done_npc( object me )
{
        ...
        {
                npc = new( "/npc/std/4131" );
//              npc->set_name( sprintf( "Quỷ Tốt (%d)", me->get_number() ) );
//              npc->set_level(1);
                npc->set_max_hp( INIT_HP );    // 小心准备 me
                npc->set_ap( INIT_AP );
                npc->set_dp( INIT_DP );
