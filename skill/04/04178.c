...
#define THIS_PERFORM    04178
#define PF_START       "04178"
#define PF_TIME        "04178#"
#define PF_NAME        "Pháp Chú Hộ Thể"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   60
#define SUB_MP        ( 5000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 4 )
#define ADD_EFFECT    ( 10000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 400000 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n    Dùng pháp chú niệm thành vòng kim cang hộ thể, trong 60 phút có thể phòng thủ được 10000 điểm sát thương";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Dùng pháp chú niệm thành vòng kim cang hộ thể, trong 60 phút có thể phòng thủ được %d điểm sát thương。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Trong 60 phút có thể phòng thủ được %d điểm sát thương。",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_EFFECT, cur_level*150+SKILL_LEVEL, ADD_EFFECT+400000 );
	return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("04178")==0) return 1;
