...
#define THIS_PERFORM    04255
#define PF_START       "04255"
#define PF_TIME        "04255#"
#define PF_NAME        "Càn Khôn Linh Giáp"

#define SKILL_LEVEL     50
#define TIME_INTERVAL   45
#define SUB_MP        ( 0 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 100 * 1 )
#define ADD_EFFECT    ( 2000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 100 * 500 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 50 cấp\n    Xoay chuyển Càn Khôn tạo ra Chân Khí hộ thể, trong 60 giây có thể hộ thể 2000 điểm sát thương";
        cur_level = (level - SKILL_LEVEL)/100 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Xoay chuyển Càn Khôn tạo ra Chân Khí hộ thể, trong 60 giây có thể hộ thể %d điểm sát thương。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Trong 30 phút có thể hộ thể %d điểm sát thương。",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_EFFECT, cur_level*100+SKILL_LEVEL, ADD_EFFECT+5000 );
	return result;
}
...
int cast_done( object me )
{
	...
	if (who->is_user()) who->set_save("05911", ADD_EFFECT);
        ...
        set_effect(who, EFFECT_CHAR_MISS2, 60);
