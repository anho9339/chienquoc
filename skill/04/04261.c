...
#define THIS_PERFORM    04261
#define PF_START       "04261"
#define PF_NAME        "Vân Vũ Tiêu Diêu"

#define SKILL_LEVEL     30
#define TIME_INTERVAL   3
#define SUB_MP          50 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/30*20
#define ADD_AP          100 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/30*30
...
string get_desc( object me )
{
        ...
        cur_level = (level - SKILL_LEVEL)/30 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Tuyệt kỹ chú trọng về sức mạnh, gia tăng %d ngoại công tấn công nhiều địch thủ và có 10%% gây choáng 2 giây ( Kỹ năng đạt tới cấp 60 sẽ gia tăng phạm vị tấn công ).\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Gia tăng %d ngoại công.",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_AP, cur_level*30+SKILL_LEVEL, ADD_AP+30 );
	return result;
}
...
int cast_done( object me )
{
        ...
        send_user( char, "%c%w%w%w%c%c%c", 0x4f, x, y, 23132, 1, OVER_BODY, PF_ONCE );

//      if( !who->can_be_fighted(me) || !me->can_fight(who) ) return 2;    // 执行成功
	if( ( level = me->get_skill(THIS_SKILL) ) < 60 )
