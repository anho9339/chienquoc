...
#define THIS_PERFORM    04186
#define PF_START       "04186"
#define PF_TIME        "04186#"
#define PF_NAME        "Cuồng Lôi Đại Nộ"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   30
#define SUB_MP          ( 0 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 1 )
#define ADD_AP          ( 200 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 5000 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n Vật phẩm tiêu hao: 1 lá Bùa Giấy\n    Hô gọi Ngự Lôi Thiên Sư tấn công địch thủ, tăng thêm 200 điểm Nội Công khi sát thương và làm mục tiêu bị Choáng trong 5 giây";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Vật phẩm tiêu hao: 1 lá Bùa Giấy\n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Hô gọi Ngự Lôi Thiên Sư tấn công địch thủ, tăng thêm %d điểm Nội Công khi sát thương và làm mục tiêu bị Choáng trong 5 giây\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng thêm %d điểm Nội Công。", 
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_AP, cur_level*150+SKILL_LEVEL, ADD_AP + 5000);     	
        return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("04186")==0) return 1;
...
int cast_done( object me )
{
        ...
        send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%w%c%c%w%c%c%c", 0x6f, getoid(who), 
                42321, 6, OVER_BODY, 42322, 6, UNDER_FOOT, 42323, 6, UNDER_FOOT, PF_ONCE );
