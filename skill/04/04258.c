...
#define THIS_PERFORM    04258
#define PF_START       "04258"
#define PF_TIME        "04258#"
...
#define ADD_EFFECT      ( 2000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 15000 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 10 cấp\n    Tâm Pháp Cao Cấp Côn Luân,Gia tăng 2 ngàn Nội Công cho bản thân hoặc đồng đội";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Tâm Pháp Cao Cấp Côn Luân,Gia tăng %d Nội Công cho bản thân hoặc đồng đội。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tâm Pháp Cao Cấp Côn Luân,Gia tăng %d Nội Công cho bản thân hoặc đồng đội。",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_EFFECT, cur_level*150+SKILL_LEVEL, ADD_EFFECT+200 );
	return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
	...
	object owner;
		...
		if (me->get_save("04258")==0) return 1;
