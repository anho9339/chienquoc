...
#define THIS_PERFORM    04225
#define PF_START       "04225"
#define PF_TIME        "04225#"
#define PF_NAME        "Liên Hoa Tâm Kinh"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   2
#define SUB_MP          ( 0 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 2 )
#define ADD_EFFECT      ( 69500 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 200000 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n    Dùng ngân châm đả thông kinh mạch, trong 60 phút có thể tăng thêm 69500 điểm Ngoại Công cho đồng đội";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Dùng ngân châm đả thông kinh mạch, trong 60 phút có thể tăng thêm %d điểm Ngoại Công cho đồng đội。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Liên tục 60 phút Tăng thêm %d Ngoại Công。",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_EFFECT, cur_level*150+SKILL_LEVEL, ADD_EFFECT+200000 );
	return result;
}
...
int main( object me, object who, int x, int y, string arg )
{
	...
	object owner;
		...
		if (me->get_save("04225")==0) return 1;
