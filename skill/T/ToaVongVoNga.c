...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 10 cấp\n    Dùng Nội Lực hút sát thương khi bị công kích.Hút sát thương liên tục trong 30 phút.";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n   Dùng Nội Lực hút %d sát thương khi bị công kích.Hút sát thương liên tục trong 30 phút. 。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Dùng Nội Lực hút %d sát thương khi bị công kích.Hiệu quả trong 30 phút。", 
...
string get_loop_desc( object me )
{
        if( me->get_perform(PF_START) )
                return sprintf( PF_NAME "：\n　　Hấp thụ %d sát thương，tiêu hao pháp lực thay cho khí huyết。\n", me->get_save("pf.miss") );
        else    return sprintf( PF_NAME "\nThương tổn，tiêu hao gấp đôi pháp lực。\n" );
}
