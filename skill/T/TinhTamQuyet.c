...
#define THIS_PERFORM    04259
#define PF_START       "04259"
#define PF_TIME        "04259#"
#define PF_NAME        "Tịnh Tâm Quyết"
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n    Thần công tẩy tâm tịnh khí, có thể bảo vệ bản thân không bị trạng thái Choáng. Khi đang sử dụng loại tâm pháp này sẽ không thể thay đổi vũ khí.: Sử dụng 2 lần sẽ giải bỏ tác dụng của tâm pháp。\n";
...
int main( object me, object who, int x, int y, string arg )
{
		...
		if (me->get_save("04259")==0) return 1;
...
//        send_user( me, "%c%w%c%w", 0x50, 0, 0xff, 2 );    // 所有出招２秒后能使用
        ...
        me->set_enemy(0);
        send_user(me, "%c", 0x01);
	send_user(me, "%c%c%w%s", 0x5a, 0, 3, "Tịnh Tâm Quyết……");
	send_user( me, "%c%w%c%w", 0x50, THIS_PERFORM, 0xff, 0 );
        me->set_perform( PF_START, time() );
        return 1;
}
