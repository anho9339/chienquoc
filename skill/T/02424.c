...
#define SUB_MP          0
#define SUB_YUAN        0
#define ADD_AP		150 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/150*100
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 150 cấp\n    Chiêu thức võ công nghịch chuyển luân hồi,gia tăng công kích cho bản thân và toàn nhóm。";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Nguyên khí tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Dịch Cân Kinh hắc cấp vô địch thiên hạ. Tăng %d％ sát thương công kích cho bản thân và toàn nhóm。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng %d％ sát thương công kích cho bản thân và toàn nhóm。", 
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_YUAN, TIME_INTERVAL, ADD_AP, cur_level*150+SKILL_LEVEL, ADD_AP+6 );
	return result;  
}
...
int main( object me, object who, int x, int y, string arg )
{
	...
	for (i=0;i<size;i++) if (who = team[i])
	{
		...
		if (who->is_anti_effect()) continue;
	        ...
	        CHAR_FIGHT_D->set_enmity2(me,who,240 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/10*50);
	}        
        return 1;
}
