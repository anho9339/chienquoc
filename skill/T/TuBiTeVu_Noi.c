...
#define THIS_PERFORM    04135
#define PF_START       "04135"
#define PF_TIME        "04135#"
#define PF_NAME        "Từ Bi Tế Vũ"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   10
#define SUB_MP          ( 15000 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 150 * 20 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 10 cấp\n    Dùng kim châm hội tụ nguyên khí, trong 60 phút có thể tăng thêm 20000 điểm Nội Công cho đồng đội";
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Dùng kim châm hội tụ nguyên khí, trong 60 phút có thể tăng thêm %d điểm Nội Công cho đồng đội。\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Liên tục 60 phút Tăng thêm %d Nội Công。",
...
int main( object me, object who, int x, int y, string arg )
{
	...
	object owner;
		...
		if (me->get_save("04135")==0) return 1;
