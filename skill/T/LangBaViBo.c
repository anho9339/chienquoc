...
#define PF_NAME        "Lăng Ba Vi Bộ"

#define SKILL_LEVEL     150
#define TIME_INTERVAL   40
#define SUB_MP          ( 5000 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/150 * 1 )
#define ADD_EFFECT	1000
#define ADD_TIME	( 20 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/150 * 20 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 150\n Liên tục trong 20 giây tăng thêm 1000％ Né Tránh.", PF_NAME);
        cur_level = (level - SKILL_LEVEL)/150 + 1;
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Liên tục trong %d giây tăng thêm %d％ Né Tránh.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n Liên tục trong %d giây tăng thêm %d％ Né Tránh.", 
...
//      me->set_cast_arg( sprintf( "%d", level ) );
        ...
        send_user( user, "%c%d%w%c%c%c", 0x6f, getoid(me), 3642, 10, OVER_BODY, PF_ONCE );
