...
object mazei,xuhengdao;
void clone_npc();

// 函数：获取标识
int get_id() { return 089; }

// 函数：获取标识(客户端)
int get_client_id() { return 089; }

// 函数：获取名字
string get_name() { return "Hổ Lao Quan"; }

// 函数：复位顺序
int get_reset_order() { return 20; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/089.npc" );

        set_fighter_file( ({ "/npc/npc/0250", "/npc/npc/0251",  }) );
        set_boss_file( ({ "/npc/npc/0300",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(mazei) )
        	{
        		mazei = new("npc/task/0108");	
        		mazei->add_to_scene(89,107,77,2);
        	}
        	else if ( get_x(mazei) != 107 || get_y(mazei) != 77 )
        		mazei->add_to_scene(89,107,77,2);
        	if ( !objectp(xuhengdao) )
        	{
        		xuhengdao = new("npc/task/0111");	
        		xuhengdao->add_to_scene(89,122,117,3);
        	}
        	else if ( get_x(xuhengdao) != 122 || get_y(xuhengdao) != 117 )
        		xuhengdao->add_to_scene(89,122,117,3);        	
        }
}
