...
// 函数：获取标识
int get_id() { return 010; }

// 函数：获取标识(客户端)
int get_client_id() { return 010; }

// 函数：获取名字
string get_name() { return "Tề"; }

// 函数：复位顺序
int get_reset_order() { return 118; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/010.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(shijie) )
        	{
        		shijie = new("npc/task/0142");	
        		shijie->add_to_scene(010,357,161,2);
        	}
        	else if ( get_x(shijie) != 357 || get_y(shijie) != 161 )
        		shijie->add_to_scene(010,357,161,2);
...
void reset_call_out()
{
        ...
        MAP_D->clone_fish( this_object(), CITY_QI );
}
