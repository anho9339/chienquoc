...
object kulouyaowang;
// 函数：获取标识
int get_id() { return 291; }

// 函数：获取标识(客户端)
int get_client_id() { return 291; }

// 函数：获取名字
string get_name() { return "Khốn Ma Trận 1"; }

// 函数：复位顺序
int get_reset_order() { return 82; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/291.npc" );

        set_fighter_file( ({ "/npc/npc/0910", "/npc/npc/0911", "/npc/npc/0912",  }) );
        set_boss_file( ({ "/npc/npc/0970",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(kulouyaowang) )
        	{
        		kulouyaowang = new("npc/task/0177");	
        		kulouyaowang->add_to_scene(291,152,163,2);
        	}
        	else if ( get_x(kulouyaowang) != 152 || get_y(kulouyaowang) != 163 )
        		kulouyaowang->add_to_scene(291,152,163,2);
        }
}
