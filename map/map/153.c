...
inherit SCENE;
object hushan,yeshugui;
void clone_npc();

// 函数：获取标识
int get_id() { return 153; }

// 函数：获取标识(客户端)
int get_client_id() { return 153; }

// 函数：获取名字
string get_name() { return "Kênh Trịnh Quốc 3"; }

// 函数：复位顺序
int get_reset_order() { return 52; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/153.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(hushan) )
        	{
        		hushan = new("npc/task/0133");	
        		hushan->add_to_scene(153,106,102,2);
        	}
        	else if ( get_x(hushan) != 106 || get_y(hushan) != 102 )
        		hushan->add_to_scene(153,106,102,2);
        	if ( !objectp(yeshugui) )
        	{
        		yeshugui = new("npc/task/0138");	
        		yeshugui->add_to_scene(153,69,35,2);
        	}
        	else if ( get_x(yeshugui) != 69 || get_y(yeshugui) != 35 )
        		yeshugui->add_to_scene(153,69,35,2);
