...
object jiugui;
// 函数：获取标识
int get_id() { return 050; }

// 函数：获取标识(客户端)
int get_client_id() { return 050; }

// 函数：获取名字
string get_name() { return "Tần"; }

// 函数：复位顺序
int get_reset_order() { return 122; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/050.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(jiugui) )
        	{
        		jiugui = new("npc/task/0157");	
        		jiugui->add_to_scene(50,285,166,2);
        	}
        	else if ( get_x(jiugui) != 285 || get_y(jiugui) != 166 )
        		jiugui->add_to_scene(50,285,166,2);
        }
}
...
void reset_call_out()
{
        ...
        MAP_D->clone_fish( this_object(), CITY_QIN );
}
