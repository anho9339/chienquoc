...
inherit SCENE;
object zhulaosi,shidalong;

void clone_npc();

// 函数：获取标识
int get_id() { return 383; }

// 函数：获取标识(客户端)
int get_client_id() { return 383; }

// 函数：获取名字
string get_name() { return "Huyền Chân Động"; }

// 函数：复位顺序
int get_reset_order() { return 87; }
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(zhulaosi) )
        	{
        		zhulaosi = new("npc/task/0181");	
        		zhulaosi->add_to_scene(383,100,79,2);
        	}
        	else if ( get_x(zhulaosi) != 100 || get_y(zhulaosi) != 79 )
        		zhulaosi->add_to_scene(383,100,79,2);
        	if ( !objectp(shidalong) )
        	{
        		shidalong = new("npc/task/0183");	
        		shidalong->add_to_scene(383,159,141,2);
        	}
        	else if ( get_x(shidalong) != 159 || get_y(shidalong) != 141 )
        		shidalong->add_to_scene(383,159,141,2);
        }
}
