...
// 函数：获取标识
int get_id() { return 312; }

// 函数：获取标识(客户端)
int get_client_id() { return 312; }

// 函数：获取名字
string get_name() { return "Linh Cốc Động 2"; }

// 函数：复位顺序
int get_reset_order() { return 70; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/312.npc" );

        set_fighter_file( ({ "/npc/npc/0770", "/npc/npc/0811", "/npc/npc/0820", "/npc/npc/0771",  }) );
        set_boss_file( ({ "/npc/npc/0812",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(yanzhizhen) )
        	{
        		yanzhizhen = new("npc/task/0168");	
        		yanzhizhen->add_to_scene(312,203,110,2);
        	}
        	else if ( get_x(yanzhizhen) != 203 || get_y(yanzhizhen) != 110 )
        		yanzhizhen->add_to_scene(312,203,110,2);
        }
}
