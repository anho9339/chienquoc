...
inherit SCENE;
object wushanjingling,gumojing,guijingguai;
void clone_npc();

// 函数：获取标识
int get_id() { return 263; }

// 函数：获取标识(客户端)
int get_client_id() { return 263; }

// 函数：获取名字
string get_name() { return "Vu Sơn"; }

// 函数：复位顺序
int get_reset_order() { return 39; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/263.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(wushanjingling) )
        	{
        		wushanjingling = new("npc/task/0121");	
        		wushanjingling->add_to_scene(263,163,111,2);
        	}
        	else if ( get_x(wushanjingling) != 163 || get_y(wushanjingling) != 111 )
        		wushanjingling->add_to_scene(263,163,111,2);
        	if ( !objectp(gumojing) )
        	{
        		gumojing = new("npc/task/0129");	
        		gumojing->add_to_scene(263,133,128,2);
        	}
        	else if ( get_x(gumojing) != 133 || get_y(gumojing) != 128 )
        		gumojing->add_to_scene(263,133,128,2);
        	if ( !objectp(guijingguai) )
        	{
        		guijingguai = new("npc/task/0130");	
        		guijingguai->add_to_scene(263,218,135,2);
        	}
        	else if ( get_x(guijingguai) != 218 || get_y(guijingguai) != 135 )
        		guijingguai->add_to_scene(263,218,135,2);
        }
}
