...
// 函数：获取标识
int get_id() { return 041; }

// 函数：获取标识(客户端)
int get_client_id() { return 041; }

// 函数：获取名字
string get_name() { return "Bờ Vô Định Hà"; }

// 函数：复位顺序
int get_reset_order() { return 17; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/041.npc" );

        set_fighter_file( ({ "/npc/npc/0210", "/npc/npc/0211",  }) );
        set_boss_file( ({ "/npc/npc/0263",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(wuzi) )
        	{
        		wuzi = new("npc/task/0107");	
        		wuzi->add_to_scene(41,190,85,2);
        	}
        	else if ( get_x(wuzi) != 190 || get_y(wuzi) != 85 )
        		wuzi->add_to_scene(41,190,85,2);
        	if ( !objectp(zhaoxun) )
        	{
        		zhaoxun = new("npc/task/0166");	
        		zhaoxun->add_to_scene(41,167,109,2);
        	}
        	else if ( get_x(zhaoxun) != 167 || get_y(zhaoxun) != 109 )
        		zhaoxun->add_to_scene(41,167,109,2);
