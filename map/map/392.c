...
// 函数：获取标识
int get_id() { return 392; }

// 函数：获取标识(客户端)
int get_client_id() { return 392; }

// 函数：获取名字
string get_name() { return "Tru Tiên Trận 2"; }

// 函数：复位顺序
int get_reset_order() { return 99; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/392.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(zanwu) )
        	{
        		zanwu = new("npc/task/0195");	
        		zanwu->add_to_scene(392,156,126,2);
        	}
        	else if ( get_x(zanwu) != 156 || get_y(zanwu) != 126 )
        		zanwu->add_to_scene(392,156,126,2);
	}
}
