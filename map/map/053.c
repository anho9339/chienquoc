...
inherit SCENE;
object qinlaoshi,shuyao;
void clone_npc();

// 函数：获取标识
int get_id() { return 053; }

// 函数：获取标识(客户端)
int get_client_id() { return 053; }

// 函数：获取名字
string get_name() { return "Tần Trường Thành 3"; }

// 函数：复位顺序
int get_reset_order() { return 65; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/053.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(qinlaoshi) )
        	{
        		qinlaoshi = new("npc/task/0158");	
        		qinlaoshi->add_to_scene(53,111,57,2);
        	}
        	else if ( get_x(qinlaoshi) != 111 || get_y(qinlaoshi) != 57 )
        		qinlaoshi->add_to_scene(53,111,57,2);
        	if ( !objectp(shuyao) )
        	{
        		shuyao = new("npc/task/0163");	
        		shuyao->add_to_scene(53,180,45,2);
        	}
        	else if ( get_x(shuyao) != 180 || get_y(shuyao) != 45 )
        		shuyao->add_to_scene(53,180,45,2);
        }
}
