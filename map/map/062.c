...
// 函数：获取标识
int get_id() { return 062; }

// 函数：获取标识(客户端)
int get_client_id() { return 062; }

// 函数：获取名字
string get_name() { return "Sở Phương Thành 2"; }

// 函数：复位顺序
int get_reset_order() { return 7; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/062.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )
        {
        	if ( !objectp(xiahou) )
        	{
        		xiahou = new("npc/task/0103");	
        		xiahou->add_to_scene(62,154,132,2);
        	}
