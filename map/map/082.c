...
// 函数：获取标识
int get_id() { return 082; }

// 函数：获取标识(客户端)
int get_client_id() { return 082; }

// 函数：获取名字
string get_name() { return "Thúy Hoa Sơn"; }

// 函数：复位顺序
int get_reset_order() { return 14; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/082.npc" );

        set_fighter_file( ({ "/npc/npc/0160", "/npc/npc/0171",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(masandao) )
        	{
        		masandao = new("npc/task/0109");	
        		masandao->add_to_scene(82,173,100,1);
        	}
        	else if ( get_x(masandao) != 173 || get_y(masandao) != 100 )
        		masandao->add_to_scene(82,173,100,1);
