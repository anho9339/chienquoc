...
// 函数：获取标识
int get_id() { return 391; }

// 函数：获取标识(客户端)
int get_client_id() { return 391; }

// 函数：获取名字
string get_name() { return "Tru Tiên Trận 1"; }

// 函数：复位顺序
int get_reset_order() { return 98; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/391.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(zenjiu) )
        	{
        		zenjiu = new("npc/task/0194");	
        		zenjiu->add_to_scene(391,61,49,2);
        	}
        	else if ( get_x(zenjiu) != 61 || get_y(zenjiu) != 49 )
        		zenjiu->add_to_scene(391,61,49,2);
	}
}
