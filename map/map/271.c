...
// 函数：获取标识
int get_id() { return 271; }

// 函数：获取标识(客户端)
int get_client_id() { return 271; }

// 函数：获取名字
string get_name() { return "Trác Lộc"; }

// 函数：复位顺序
int get_reset_order() { return 27; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/271.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(liyan) )
        	{
        		liyan = new("npc/task/0178");	
        		liyan->add_to_scene(271,215,89,2);
        	}
        	else if ( get_x(liyan) != 215 || get_y(liyan) != 89 )
        		liyan->add_to_scene(271,215,89,2);
        }
}
