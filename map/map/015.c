...
// 函数：获取标识
int get_id() { return 015; }

// 函数：获取标识(客户端)
int get_client_id() { return 015; }

// 函数：获取名字
string get_name() { return "Đông Hải Đảo"; }

// 函数：复位顺序
int get_reset_order() { return 49; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/015.npc" );

        set_fighter_file( ({ "/npc/npc/0410", "/npc/npc/0411",  }) );
        set_boss_file( ({ "/npc/npc/0463",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(xiaozhu) )
        	{
        		xiaozhu = new("npc/task/0188");	
        		xiaozhu->add_to_scene(15,221,162,2);
        	}
        	else if ( get_x(xiaozhu) != 221 || get_y(xiaozhu) != 162 )
        		xiaozhu->add_to_scene(15,221,162,2);
        }
}
