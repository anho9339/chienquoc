...
// 函数：获取标识
int get_id() { return 393; }

// 函数：获取标识(客户端)
int get_client_id() { return 393; }

// 函数：获取名字
string get_name() { return "Tru Tiên Trận 3"; }

// 函数：复位顺序
int get_reset_order() { return 100; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/393.npc" );

        set_fighter_file( ({ "/npc/npc/1120", "/npc/npc/1121", "/npc/npc/1122",  }) );
        set_boss_file( ({ "/npc/npc/1180", "/npc/npc/1181", "/npc/npc/1182",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(xuxiaosi) )
        	{
        		xuxiaosi = new("npc/task/0196");	
        		xuxiaosi->add_to_scene(393,72,125,2);
        	}
        	else if ( get_x(xuxiaosi) != 72 || get_y(xuxiaosi) != 125 )
        		xuxiaosi->add_to_scene(393,72,125,2);
	}
}
