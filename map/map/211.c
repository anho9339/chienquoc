...
inherit SCENE;
object huxiaosi,huzisan;
void clone_npc();

// 函数：获取标识
int get_id() { return 211; }

// 函数：获取标识(客户端)
int get_client_id() { return 211; }

// 函数：获取名字
string get_name() { return "Mã Đạp Hồ"; }

// 函数：复位顺序
int get_reset_order() { return 23; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/211.npc" );

        set_fighter_file( ({ "/npc/npc/0222", "/npc/npc/0212",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(huxiaosi) )
        	{
        		huxiaosi = new("npc/task/0117");	
        		huxiaosi->add_to_scene(211,141,54,2);
        	}
        	else if ( get_x(huxiaosi) != 141 || get_y(huxiaosi) != 54 )
        		huxiaosi->add_to_scene(211,141,54,2);
        	if ( !objectp(huzisan) )
        	{
        		huzisan = new("npc/task/0118");	
        		huzisan->add_to_scene(211,46,107,2);
        	}
        	else if ( get_x(huzisan) != 46 || get_y(huzisan) != 107 )
        		huzisan->add_to_scene(211,46,107,2);     		
        }
}
