...
// 函数：获取标识
int get_id() { return 311; }

// 函数：获取标识(客户端)
int get_client_id() { return 311; }

// 函数：获取名字
string get_name() { return "Linh Cốc Động 1"; }

// 函数：复位顺序
int get_reset_order() { return 69; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/311.npc" );

        set_fighter_file( ({ "/npc/npc/0761", "/npc/npc/0810", "/npc/npc/0811", "/npc/npc/0760",  }) );
        set_boss_file( ({ "/npc/npc/0821",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(lisan) )
        	{
        		lisan = new("npc/task/0167");	
        		lisan->add_to_scene(311,123,161,2);
        	}
        	else if ( get_x(lisan) != 123 || get_y(lisan) != 161 )
        		lisan->add_to_scene(311,123,161,2);
        }
}
