...
// 函数：获取标识
int get_id() { return 375; }

// 函数：获取标识(客户端)
int get_client_id() { return 375; }

// 函数：获取名字
string get_name() { return "Cô Trúc Tiểu Trúc"; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/375.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(jiangsiwang) )
        	{
        		jiangsiwang = new("npc/task/0187");	
        		jiangsiwang->add_to_scene(375,145,36,2);
        	}
        	else if ( get_x(jiangsiwang) != 145 || get_y(jiangsiwang) != 36 )
        		jiangsiwang->add_to_scene(375,145,36,2);
