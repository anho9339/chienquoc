...
// 函数：获取标识
int get_id() { return 042; }

// 函数：获取标识(客户端)
int get_client_id() { return 042; }

// 函数：获取名字
string get_name() { return "Vô Định Hà 1"; }

// 函数：复位顺序
int get_reset_order() { return 18; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/042.npc" );

        set_fighter_file( ({ "/npc/npc/0210", "/npc/npc/0221",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(masan) )
        	{
        		masan = new("npc/task/0110");	
        		masan->add_to_scene(42,270,138,2);
        	}
        	else if ( get_x(masan) != 270 || get_y(masan) != 138 )
        		masan->add_to_scene(42,270,138,2);
