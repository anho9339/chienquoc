...
inherit SCENE;
object yanlingjun,shougenjun;
void clone_npc();

// 函数：获取标识
int get_id() { return 063; }

// 函数：获取标识(客户端)
int get_client_id() { return 063; }

// 函数：获取名字
string get_name() { return "Sở Phương Thành 3"; }

// 函数：复位顺序
int get_reset_order() { return 8; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/063.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )
        {
        	if ( !objectp(yanlingjun) )
        	{
        		yanlingjun = new("npc/task/0104");	
        		yanlingjun->add_to_scene(63,138,138,2);
        	}
        	if ( !objectp(shougenjun) )
        	{
        		shougenjun = new("npc/task/0105");	
        		shougenjun->add_to_scene(63,49,63,2);
        	}      	
        }
}
