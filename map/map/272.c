...
inherit SCENE;
object youminguihun,yeminggui;
void clone_npc();

// 函数：获取标识
int get_id() { return 272; }

// 函数：获取标识(客户端)
int get_client_id() { return 272; }

// 函数：获取名字
string get_name() { return "Dị Thủy"; }

// 函数：复位顺序
int get_reset_order() { return 28; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/272.npc" );

        set_fighter_file( ({ "/npc/npc/0271", "/npc/npc/0250", "/npc/npc/0251",  }) );
        set_boss_file( ({ "/npc/npc/0352",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(youminguihun) )
        	{
        		youminguihun = new("npc/task/0119");	
        		youminguihun->add_to_scene(272,230,123,2);
        	}
        	else if ( get_x(youminguihun) != 230 || get_y(youminguihun) != 123 )
        		youminguihun->add_to_scene(272,230,123,2);
        	if ( !objectp(yeminggui) )
        	{
        		yeminggui = new("npc/task/0120");	
        		yeminggui->add_to_scene(272,119,181,2);
        	}
        	else if ( get_x(yeminggui) != 119 || get_y(yeminggui) != 181 )
        		yeminggui->add_to_scene(272,119,181,2);
        }
}
