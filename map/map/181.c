...
// 函数：获取标识
int get_id() { return 181; }

// 函数：获取标识(客户端)
int get_client_id() { return 181; }

// 函数：获取名字
string get_name() { return "Ky Quan"; }

// 函数：复位顺序
int get_reset_order() { return 32; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/181.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(qiansanba) )
        	{
        		qiansanba = new("npc/task/0116");	
        		qiansanba->add_to_scene(181,86,165,2);
        	}
        	else if ( get_x(qiansanba) != 86 || get_y(qiansanba) != 165 )
        		qiansanba->add_to_scene(181,86,165,2);
