...
// 函数：获取标识
int get_id() { return 920; }

// 函数：获取标识(客户端)
int get_client_id() { return 920; }

// 函数：获取名字
string get_name() { return "Hỗn Nguyên Trường"; }

// 函数：复位顺序
int get_reset_order() { return 50; } //164
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/543.npc" );

        set_fighter_file( ({ "/npc/phuban/quainl",  }) );
   //     set_boss_file( ({ "/npc/npc/0905", "/npc/npc/0906",  }) );

        set_sizeof_xy( sizeof_xy_point( get_id() ) );

        set_heart_timer( map, set_heart_timer_2( map, ( time() % RESET_TIME ) * 5 ) );
...
void reset() 
{
        ...
        if( time % 100 == 0 )    // 10 分钟
        {
                ...
                set_heart_timer( map, set_heart_timer_2( map, ( time() % RESET_TIME ) * 5 ) ); // 10
        }
    /*    if( time % 200 == 0 )    // 20 分钟
        {
...
void clone_npc()
{
        ...
        if( get_add_amount() ) call_out( "clone_npc", 2 ); //2
}
