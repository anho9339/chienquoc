...
object kuloumo,zhaosushi;
void clone_npc();

// 函数：获取标识
int get_id() { return 141; }

// 函数：获取标识(客户端)
int get_client_id() { return 141; }

// 函数：获取名字
string get_name() { return "Mã Lăng Đạo"; }

// 函数：复位顺序
int get_reset_order() { return 21; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/141.npc" );

        set_fighter_file( ({ "/npc/npc/0221", "/npc/npc/0212",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(kuloumo) )
        	{
        		kuloumo = new("npc/task/0113");	
        		kuloumo->add_to_scene(141,94,41,2);
        	}
        	else if ( get_x(kuloumo) != 94 || get_y(kuloumo) != 41 )
        		kuloumo->add_to_scene(141,94,41,2);
        	if ( !objectp(zhaosushi) )
        	{
        		zhaosushi = new("npc/task/0114");	
        		zhaosushi->add_to_scene(141,241,175,2);
        	}
        	else if ( get_x(zhaosushi) != 241 || get_y(zhaosushi) != 175 )
        		zhaosushi->add_to_scene(141,241,175,2);        		
        }
}
