...
// 函数：获取标识
int get_id() { return 133; }

// 函数：获取标识(客户端)
int get_client_id() { return 133; }

// 函数：获取名字
string get_name() { return "Sơn Động Thần Bí 1"; }

// 函数：复位顺序
int get_reset_order() { return 104; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/133.npc" );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(maning) )
        	{
        		maning = new("npc/task/0165");	
        		maning->add_to_scene(133,102,150,2);
        	}
        	else if ( get_x(maning) != 102 || get_y(maning) != 150 )
        		maning->add_to_scene(133,102,150,2);
        }
}
