...
// 函数：获取标识
int get_id() { return 182; }

// 函数：获取标识(客户端)
int get_client_id() { return 182; }

// 函数：获取名字
string get_name() { return "Tấn Dương"; }

// 函数：复位顺序
int get_reset_order() { return 33; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/182.npc" );

        set_fighter_file( ({ "/npc/npc/0260", "/npc/npc/0261",  }) );
        set_boss_file( ({ "/npc/npc/0353",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(xiongping) )
        	{
        		xiongping = new("npc/task/0172");	
        		xiongping->add_to_scene(182,170,130,2);
        	}
        	else if ( get_x(xiongping) != 170 || get_y(xiongping) != 130 )
        		xiongping->add_to_scene(182,170,130,2);
