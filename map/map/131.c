...
// 函数：获取标识
int get_id() { return 131; }

// 函数：获取标识(客户端)
int get_client_id() { return 131; }

// 函数：获取名字
string get_name() { return "Cự Lộc"; }

// 函数：复位顺序
int get_reset_order() { return 29; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/131.npc" );

        set_fighter_file( ({ "/npc/npc/0270", "/npc/npc/0250", "/npc/npc/0251",  }) );
        set_boss_file( ({ "/npc/npc/0351",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(madalong) )
        	{
        		madalong = new("npc/task/0112");	
        		madalong->add_to_scene(131,90,58,2);
        	}
        	else if ( get_x(madalong) != 90 || get_y(madalong) != 58 )
        		madalong->add_to_scene(131,90,58,2);
        }
}
