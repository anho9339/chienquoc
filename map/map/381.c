...
void clone_npc();
object zhujiu,zhulaowu,weixiaosan;
// 函数：获取标识
int get_id() { return 381; }

// 函数：获取标识(客户端)
int get_client_id() { return 381; }

// 函数：获取名字
string get_name() { return "Hư Lăng Động"; }

// 函数：复位顺序
int get_reset_order() { return 85; }

// 函数：获取地型类别
int get_map_type() { return CAVE_MAP; }

// 函数：获取投掷类数
int get_caster_amount() { return 2; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/381.npc" );

        set_fighter_file( ({ "/npc/npc/0961", "/npc/npc/0962", "/npc/npc/0964", "/npc/npc/0960", "/npc/npc/0963",  }) );
        set_boss_file( ({ "/npc/npc/1000",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(zhujiu) )
        	{
        		zhujiu = new("npc/task/0180");	
        		zhujiu->add_to_scene(381,78,71,2);
        	}
        	else if ( get_x(zhujiu) != 78 || get_y(zhujiu) != 71 )
        		zhujiu->add_to_scene(381,78,71,2);
		if ( !objectp(zhulaowu) )
        	{
        		zhulaowu = new("npc/task/0182");	
        		zhulaowu->add_to_scene(381,203,137,2);
        	}
        	else if ( get_x(zhulaowu) != 203 || get_y(zhulaowu) != 137 )
        		zhulaowu->add_to_scene(381,203,137,2);
		if ( !objectp(weixiaosan) )
        	{
        		weixiaosan = new("npc/task/0184");	
        		weixiaosan->add_to_scene(381,133,25,2);
        	}
        	else if ( get_x(weixiaosan) != 133 || get_y(weixiaosan) != 25 )
        		weixiaosan->add_to_scene(381,133,25,2);
        }
}
