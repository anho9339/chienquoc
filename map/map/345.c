...
// 函数：获取标识
int get_id() { return 345; }

// 函数：获取标识(客户端)
int get_client_id() { return 345; }

// 函数：获取名字
string get_name() { return "Tướng Quân Cổ Mộ 5"; }

// 函数：复位顺序
int get_reset_order() { return 81; }
...
void create() 
{
        ...
        MAP_D->load_npc_file( map, "/map/npc/345.npc" );

        set_fighter_file( ({ "/npc/npc/0860", "/npc/npc/0880", "/npc/npc/0890", "/npc/npc/0870",  }) );
...
void reset() 
{
        ...
        if ( time % 30 == 0 )		//3分钟
        {
        	if ( !objectp(zhangqilong) )
        	{
        		zhangqilong = new("npc/task/0176");	
        		zhangqilong->add_to_scene(345,130,25,2);
        	}
        	else if ( get_x(zhangqilong) != 130 || get_y(zhangqilong) != 25 )
        		zhangqilong->add_to_scene(345,130,25,2);
        }
}
