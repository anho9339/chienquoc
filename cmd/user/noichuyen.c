...
/*
        if( abs( me->get_time("noichuyen") - get_heart_timer(me) ) < 2 )    // 0.2 秒，心跳有影响
        {
                me->set_time( "noichuyen", get_heart_timer(me) );
                notify( "请勿频繁对话。" );
                return 1;
        }
        me->set_time( "noichuyen", get_heart_timer(me) );
*/
        ...
        &&      arg
        &&    ( func = who->get_2( "noichuyen." + key ) ) )
        {
                me->to_front_eachother(who);
                if( functionp(func) ) evaluate(func, arg);
                else send_user( me, "%c%s", ':', func );
        }
        else if( func = who->get_2( "noichuyen." + info ) ) 
        {
                me->to_front_eachother(who);
                if( functionp(func) ) evaluate(func);
                else send_user( me, "%c%s", ':', func );
