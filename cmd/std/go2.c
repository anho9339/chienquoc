...
//              notify( "您在摆摊，不能乱跑." );
	...
	if(  get_effect(me, EFFECT_CHAR_SLOW) || ( get_effect(me, EFFECT_CHAR_LAZY) || (get_effect(me, EFFECT_CHAR_INVISIBLE) && ! get_effect(me, EFFECT_03643) && !me->get_save("no_visible") ) ) )
        ...
        else    if( sizeof(arg) > 3 ) arg = arg[0..1];
        ...
        &&     !inside_screen( x, y, get_x(who), get_y(who) ) )    // 移动受限制  // !inside_screen_2(me, who)
        {
        	...
        	{
	                notify(who->get_name()+"出现，您的行动受到影响.");
	...
	if ( me->get_hp() > me->get("pf.hcmc") ) me->set_hp(me->get_hp() - me->get("pf.hcmc"));	
	else
	{	
			if ( map->get_no_fight() )
			{
		...
		me->set_die(1);	
			...
			else if( map->get_client_id() == 885 || map->get_client_id() == 887 || map->get_client_id()==891 || map->is_wushuang() || map->is_wushuang_palace() )
			{
		me->set_hp(0);	
		me->set_die(5);	
			}
