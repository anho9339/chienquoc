...
*/
		...
		else//Bản án đá giả mạo kết quả phù hợp với thiết bị.
		{
			...
			if((level == 1||level == 0) && index!="1")
			{
				...
				send_user( me, "%c%c%d", 0x25, 3, 0 );    // tắt make2
				return 1;
			}
...
int mix1(object me,object *item)
{
	...
	{
		notify( "Loại đoạn thạch sai quy tắc." );
		return 1;
	}
	if (index=="1")
	{
	...
	if (index=="3")
	{
		if (level>=6)
		{
			notify( "Cao cấp Đoạn thạch chỉ rèn được cao nhất 6 sao." );
			return 1;
		}
	}
	if (index=="4")
	{
		if (level>=10)
		{
			notify( "Cao cấp Đoạn thạch Cường, Thượng chỉ rèn được cao nhất 10 sao." );
			return 1;
		}
	}
	if( me->get_save("passhanhtrang") && me->get_save("ndmkhtk") < 1 )
