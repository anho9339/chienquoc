...
#define BossDM 			"npc/bossmonphai/Bossdm"
...
void check_time()
{
	...
	else
	{
		...
		{
		// 9 giờ sáng và 9 giờ tối
			if ( mixTime[TIME_HOUR] == 1 || mixTime[TIME_HOUR] == 3 || mixTime[TIME_HOUR] == 5 || mixTime[TIME_HOUR] == 7  || mixTime[TIME_HOUR] == 11 
			|| mixTime[TIME_HOUR] == 13 || mixTime[TIME_HOUR] == 15 || mixTime[TIME_HOUR] == 17 || mixTime[TIME_HOUR] == 19  || mixTime[TIME_HOUR] == 23 )
...
void generate_scum()
{
	...
	call_out("check_time2",18000); // Sau 5 tiếng
