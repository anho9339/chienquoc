...
void create()
{
	iStart = mktime(2016,2,29,7,0,0);	
	iFinish = mktime(2016,3,10,24,0,0);	
...
void check_time()
{
	...
	call_out("check_time",3600); // 1 tiếng xuất 1 lần
	generate_money();
//	generate_box();
//	generate_gift();
}
...
//			log_file("distribute.txt",sprintf("%s money %d %d %d %d\n",short_time(),i,z,x,y));	
	...
	log_file("distribute.txt",short_time() + " rớt hoa hồng.\n" );
