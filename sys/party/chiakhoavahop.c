...
/*	if ( mixTime[TIME_WDAY] == 1 || mixTime[TIME_WDAY] == 3 || mixTime[TIME_WDAY] == 5 || mixTime[TIME_WDAY] == 0 ) // t2, t4, t6, CN ko có bảo hộp
	...
	{ */
		...
		{
		// 2h, 8h, 10h, 12h, 14h, 16h, 22h, 24h
			if ( mixTime[TIME_HOUR] == 4 || mixTime[TIME_HOUR] == 6 || mixTime[TIME_HOUR] == 18 || mixTime[TIME_HOUR] == 20	)
...
void generate_scum()
{
	...
	// Xuất hiện 30 phút tiếng -> 15 phút sau sẽ biến mất => 1800 + 900 <<== 15' 
	
	CHAT_D->sys_channel(0,HIY"Quái Vật Đặc Biệt đã xuất hiện tại doanh trại 7 nước, các vị hãy mau mau tương trợ!");
	CHAT_D->sys_channel(0,HIR"Quái Vật Đặc Biệt đã xuất hiện tại doanh trại 7 nước, các vị hãy mau mau tương trợ!");
}
...
{
	int x,y,z,p,i,rate,rate1,level,level2,manh;
	...
	for(i=0;i<1;i++) {	
		item = new("/item/event/ckvahop/manhhop");
		item->set_amount(1+random(2));
		manh = item->move(who,-1);
		item->add_to_user(manh);	
	}
