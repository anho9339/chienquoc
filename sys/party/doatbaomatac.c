...
void win_bonus_callout( object me, object who )
{
	...
	pot = level * 5;
        for( i = 0; i < size; i ++ ) if( team[i] )
        {
				...
				team[i]->add_cash(10000);
				"sys/sys/test_db"->add_yuanbao(team[i],30/size);
				write_user(team[i], sprintf(ECHO "Bạn tiêu diệt Đoạt Bảo Mã Tặc và nhận được %d Kim Bảo !",(30/size)/10));
	}
}