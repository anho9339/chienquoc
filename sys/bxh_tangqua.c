...
void tangqua(object player,object npc)
{
	...
	for(index = 1; index<=10;index++)
	{
		...
		else
		{
			...
			{
			case 1:
				tmp = sprintf(" ""R""%2d: %12s (%d) %12s %d điểm"NOR,1,ben,playerid,"Tích luỹ",get_save_2(sprintf("nguoichoi.%d.cay",id)));					
				break;
			case 2:
				tmp = sprintf(" ""M""%2d: %12s (%d) %12s %d điểm"NOR,2,ben,playerid,"Tích luỹ",get_save_2(sprintf("nguoichoi.%d.cay",id)));					
				break;
			case 3:
				tmp = sprintf(" ""B""%2d: %12s (%d) %12s %d điểm"NOR,3,ben,playerid,"Tích luỹ",get_save_2(sprintf("nguoichoi.%d.cay",id)));			
				break;
			case 4:
				tmp = sprintf(" ""%2d: %12s (%d) %12s %d điểm",4,ben,playerid,"Tích luỹ",get_save_2(sprintf("nguoichoi.%d.cay",id)));			
				break;		
			case 5:
				tmp = sprintf(" ""%2d: %12s (%d) %12s %d điểm",5,ben,playerid,"Tích luỹ",get_save_2(sprintf("nguoichoi.%d.cay",id)));	
				break;	
			case 6:
				tmp = sprintf(" ""%2d: %12s (%d) %12s %d điểm",6,ben,playerid,"Tích luỹ",get_save_2(sprintf("nguoichoi.%d.cay",id)));	
				break;	
			case 7:
				tmp = sprintf(" ""%2d: %12s (%d) %12s %d điểm",7,ben,playerid,"Tích luỹ",get_save_2(sprintf("nguoichoi.%d.cay",id)));	
				break;
			case 8:
				tmp = sprintf(" ""%2d: %12s (%d) %12s %d điểm",8,ben,playerid,"Tích luỹ",get_save_2(sprintf("nguoichoi.%d.cay",id)));	
				break;	
			case 9:
				tmp = sprintf(" ""%2d: %12s (%d) %12s %d điểm",9,ben,playerid,"Tích luỹ",get_save_2(sprintf("nguoichoi.%d.cay",id)));	
				break;	
			case 10:
				tmp = sprintf(" ""%2d: %12s (%d) %12s %d điểm",10,ben,playerid,"Tích luỹ",get_save_2(sprintf("nguoichoi.%d.cay",id)));	
				break;	
			}	
			tmp+="\n";
		}
		...
		result=sprintf(sprintf("Hiện tại bạn đã tích luỹ %d điểm tặng quà.\n",player->get_save("quaqtpn"))+cmd);
