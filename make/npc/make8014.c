...
#define CPP_FILE        __DIR__ "8014.cc"
#define TEXT_FILE       __DIR__ "8014.txt"
...
int main( object me, string arg )
{
        ...
        int id, pic, point, point2, z, x, y, d, i, size;

        cpp = read_file( CPP_FILE );
        line = explode( read_file( TEXT_FILE ), "\n" );

        log_file( LIST_LOG, "# 守卫\n" );

        size = sizeof(line);
        for( i = 0; i < size; i ++ ) 
        {
                if( line[i][0] == '#' || line[i] == "" ) continue;
                if( sscanf( line[i], "%d %s %d %s %d %s %d %d", id, name, pic, city, z, xy, point, point2 ) )
                {
                  ...
                  case "周国" : city2 = "CITY_ZHOU";  break;
                      default : tell_user(me, "%04d 国家: %s", id, city);  break;
                        }

                        log_file( file, sprintf( cpp, pic, name, city, point, point2, city2, city2, city2 ) );
