...
#define CPP_FILE        __DIR__ "33.cc"
#define HTM_FILE        __DIR__ "30.htm"
#define TEXT_FILE       __DIR__ "33.txt"
...
int main( object me, string arg )
{
        ...
        int id, pic1, pic2, combined, value, add, level;
        int i, size;
        ...
        line = explode( read_file( TEXT_FILE ), "\n" );
                        ...
                        log_file( LIST_LOG, "<!-- 小吃 -->\n\n" );

        size = sizeof(line);
        for( i = 0; i < size; i ++ ) 
        {
                if( line[i][0] == '#' || line[i] == "" ) continue;
                if( sscanf( line[i], "%d %s %d %d %d %s %d %d %d %s", 
                        id, name, pic1, pic2, combined, unit, value, level, add, desc ) )
                {
